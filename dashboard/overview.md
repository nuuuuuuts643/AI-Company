...
| 案件 | ブロッカー | 担当 | 経過日数 |
|------|-----------|------|--------|
| 全エージェント有効化 | git push | ナオヤ（home PC） | 6日 ⚠️ |
| P003 HTTPS / flotopic.com | setup-domain.sh 実行 + Squarespace NS 変更 | ナオヤ（home PC） | 6日 ⚠️ |
| P004 Lambda デプロイ | deploy.sh 実行 | ナオヤ（home PC） | 6日 ⚠️ |
| Google OAuth 有効化 | Google Cloud Console で Client ID 取得 → config.js に設定 | ナオヤ（手順: docs/google-oauth-setup.md） | 6日 ⚠️ |
