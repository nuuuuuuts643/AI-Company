...
- ✅ **PR [#38](https://github.com/nuuuuuuts643/AI-Company/pull/38) merged (commit dcbb7c8)** — `perf(T-haiku-full): full mode を Haiku に統一 + プロンプト強化`
  - **背景**: ナオヤ指示「haikuで良い。多分プロンプトの質が低いんだと思うから」。`_generate_story_full` (記事 6 件以上のフルモード) のみが `model='claude-sonnet-4-6'` で走っており、minimal/standard は既に Haiku 4.5 だった。
...
- ✅ **T224a クローズ — 修正不要（既に解決済み）**
  - 前提確認: `grep -rn "allowedEmail\|mrkm\|naoya643" projects/P003-news-timeline/frontend/` → 0 件 (exit 1)。admin.html に PII は残っていない。
  ...
  - **回帰防止**: `scripts/security_audit.sh` (週次 cron + 手動) の A1「個人名パターンスキャン」が `naoya|mrkm|murakaminaoya|村上` を grep。`.github/workflows/security-audit.yml` で毎週月曜 JST 09:00 自動実行。再混入時は CI 赤化で物理ブロック。
...
- ✅ **PR #22 (commit 247e1b8) — docs 追記**
  ...
  - **なぜ**: 「効果が出るまで Dispatch を起動したままにしておく」運用は (a) セッション並走上限を圧迫する (b) コンテキストが切れて引き継ぎ漏れる (c) ナオヤが追加指示できない、の 3 重に詰まる。
...
- ✅ **パターン1 完了** — title markdown strip 漏れ全 write path 適用。
  - `lambda/processor/handler.py` で `gen_title` を pickup する 2 箇所 (line 274 / 287) に `_strip_title_markdown()` を適用。OGP 画像生成・Bluesky pending マーカー・topics-full.json 書き戻し全てに stripped 値が伝播するようになる。
  - `lambda/processor/proc_storage.py:generate_ogp_image()` 冒頭で defensive strip を追加。markdown 文字 (#, **) が画像に焼き込まれる事故を防ぐ。
  - `lambda/fetcher/handler.py:574` の `'generatedTitle': gen_title or g[0]['title']` を `strip_title_markdown(gen_title or g[0]['title'])` に変更。fetcher は processor とは別 Lambda なので `lambda/fetcher/text_utils.py` に `strip_title_markdown()` を新設して import。
  - **commit**: d3fd0f1 / 4 ファイル変更。

- ✅ **パターン2 完了** — AI field min length perspectives/watchPoints/outlook 強制。
  - `lambda/processor/proc_ai.py` で keyPoint のみ持っていた minLength を 3 フィールドに横展開: perspectives (≥60字) / watchPoints (≥80字 = ① + ② 2項目分) / outlook (≥60字)。Tool Use validation で schema 違反として再要求が走る物理ガード。
  - prompt rules (`_STORY_PROMPT_RULES`) と system prompt の出力品質チェックも対応する文字数明示に更新。
  - **基準値**: 本番 watchPoints 平均 38 字 / outlook 41 字 / perspectives 47 字。**目標**: 次回 forceRegenerateAll 後 平均 ≥80/60/60 字。
  - **commit**: c293df9 / proc_ai.py 1 ファイル。

- ✅ **パターン3 完了** — GitHub Actions JSON shorthand 問題は PR #14 (claude/intelligent-goldberg-ca2d0f) が `--targets file://` 形式で既に修正済 (commit 2c8d8e9)。同一 deploy-lambdas.yml への二重編集を避ける (T2026-0429-PR-CONFLICT 運用ルール) ため本ブランチでは別途修正しない。他の workflow (cf-analytics-setup.yml 等) には同パターン未検出。

- ✅ **パターン4 完了** — TASKS.md に残っていた `<<<<<<< HEAD ... >>>>>>> 013d0b2` コンフリクトマーカーを解消 (両側のタスク T2026-0428-K / T2026-0428-Q を保持)。`T2026-0429-PR-CONFLICT` を新設し「同一ファイル並行編集禁止 / 衝突しない順番で merge」を運用ルール化。PR #13 (T237) は別経路で push 完了 (CI run 25081752068 green)。
  - **commit**: f07953f / TASKS.md 1 ファイル。

- ✅ **パターン5 完了** — `scripts/verify_effect.sh ai_quality` の判定軸を強化。
  - 旧: keyPoint 空でない / perspectives 空でない の 2 軸のみ。21 字の短文 keyPoint でも PASS する穴があった。
  - 新: `keyPoint ≥100 字` 充填率 (閾値 30%) と `watchPoints` 存在率 (閾値 30%) を追加判定。閾値は緩く設定し proc_ai.py minLength:100 retry の浸透を Verified-Effect で段階観測。
  - 環境変数 `AI_QUALITY_KP_LONG_THRESHOLD` / `AI_QUALITY_WP_THRESHOLD` で上書き可能。
  - **commit**: 9ce3556 / verify_effect.sh 1 ファイル。

- ✅ **パターン6 完了** — `scripts/session_bootstrap.sh` の `[Code]` 並走 ≥2 件 ERROR 化は T2026-0428-BB で既に実装済 (line 273-282)。`BOOTSTRAP_EXIT=1` + 末尾 `exit "$BOOTSTRAP_EXIT"` で物理ブロックが効いていることをコード検査で確認。緊急 bypass は `ALLOW_CONCURRENT_CODE=1` のみ。
...
- ✅ **T2026-0428-AC system-status.md 数値の自動更新化 (準備)** — `docs/system-status.md` 「P003 技術状態スナップショット」表に `[AUTO]` 接頭辞を 6 行に追加 (AI 要約カバレッジ / keyPoint / perspectives / storyPhase / topics.json 鮮度 / topics.json サイズ)。表上部に「`[AUTO]` 接頭辞付きの行は freshness-check.yml の実測値で自動更新する対象」「手書きで嘘を書ける構造を排除する」を明記。次フェーズで sed 置換 step を freshness-check.yml に追加予定 (T2026-0428-AC 後段)。「Claude が書いたら機能していると誤認するバイアスへの物理ゲート」(T2026-0428-AB Why4 への直接対策)。
- ✅ **T2026-0428-AD PR テンプレに「対応 SLI 追記」チェック欄追加** — `.github/PULL_REQUEST_TEMPLATE.md` を新規作成。「□ 対応 SLI を `docs/sli-slo.md` に追記したか (該当なしは理由を記載)」を含む 6 項目のチェックリスト + Verified 行 / Verified-Effect 行 / なぜなぜ分析 (バグ修正時) 欄を整備。CLAUDE.md の物理ゲート (verify_effect.sh / pre-commit hook) と整合。「新機能ローンチ時に SLI を追加する習慣が無い」(T2026-0428-AB Why4) への構造的対策。
- ✅ **T2026-0428-T AI フィールドカタログと proc_ai schema を CI で突合** — `scripts/check_ai_fields_catalog.py` 新規作成 (133 行)。proc_ai.py を import して `_build_story_schema(mode='full')` から field 一覧を抽出 (Python 3.10+ の `dict | None` 構文に対応)、import 失敗時は AST フォールバック (top-level dict literal のみを抽出し timeline.items.properties の `date`/`event`/`transition` 等の nested key を除外)。`docs/ai-fields-catalog.md` 「フィールド一覧」セクション直下の `| `name` |` 形式の field 名を抽出して diff、乖離があれば exit 1。`.github/workflows/ci.yml` の lint-lambda job に main push 時のみ実行する step を追加。`ALLOW_CATALOG_EXTRA = {'summaryMode'}` で computed フィールドの例外を許容。**カタログも T2026-0428-J/E に合わせて更新** (`background`/`backgroundContext`/`spreadReason` 行を削除し `statusLabel`/`watchPoints` 行を追加・`keyPoint` の cap を 60→400 字に修正)。**ローカル検証**: schema 16 / catalog 17 (summaryMode 例外含む) で OK 通過確認済。「フィールド追加したらカタログも更新」を物理ガード化 (T256 の具体実装)。

### 完了済み（2026-04-28 11:42 JST Code T2026-0428-GRAPH + T265）

- ✅ **T2026-0428-GRAPH スパイク×きっかけ記事アノテーション** — `frontend/detail.js` の「記事数の推移」グラフ (Chart.js) に、前ポイント比 +3 件 or +50% 以上の急増点をスパイクとして検出するロジックを追加。スパイクポイントは `pointBackgroundColor='#f59e0b'` (オレンジ) + `pointRadius=6` で視覚強調し、ツールチップ `callbacks.afterBody` で「📌 きっかけ: {記事タイトル(40字 truncate)}」を追記表示。aggregate モード (3d 以上の日次集計) では各日 SNAP のうち `articleCount` が最大の SNAP の `articles[0].title` を採用し、non-aggregate (24h/72h) では各 SNAP の articles[0] を直接採用。常時表示は不要 (ツールチップのみ) でグラフのノイズを抑制。**設計方針**: 注目度 (グラフの数値スパイク) ×内容 (どの記事がきっかけか) を 1 画面で接続することで「なぜ今動いたか」が即座に分かる UX に。判定は frontend で完結 (新フィールド/Lambda 追加なし)。ナオヤ方針 (2026-04-28) との一致: 「注目度はデータで示す」「グラフ×きっかけ記事は優先度高」。
- ✅ **T265 topics.json 肥大化対応 (card JSON 分割 Step2)** — `proc_storage.py` に `generate_topics_card_json(topics_pub, updated_at)` 関数を新設。`_CARD_INCLUDE_KEYS` で card 表示に必要なフィールド (基本識別 / 分類 / スコア / 日時 / ステータス / keyPoint / latestUpdateHeadline / 親子トピック等 約 28 フィールド) のみを抽出し、重い AI long-text (`generatedSummary`/`perspectives`/`watchPoints`/`outlook`/`forecast`/`predictionMadeAt`/`predictionResult`/`parentTopicTitle`/`relatedTopicTitles`/`trendsData`) を除外。`handler.py` のインライン構築 (旧 `_CARD_KEYS` 14 フィールド) を関数呼び出しに置換。`frontend/app.js` `loadTopics()` を `topics.json` → `topics-card.json` に切替、`trendingKeywords` は別途非同期 fetch (主動線をブロックしない・失敗してもキーワードストリップが消えるだけで一覧表示には影響なし)。**期待効果**: モバイル初回表示で取得する payload を半分以下に圧縮し帯域コストを削減。日付別 shard は将来別タスクへ。

### 完了済み（2026-04-28 11:15 JST Code T2026-0428-AO 全トピック棚卸し＆自己修復基盤）

- ✅ **T2026-0428-AO 全トピック棚卸し＋ZOMBIE 全種一括削除＋自己修復基盤** — ナオヤ要望:「局所パッチではなく、全トピックの棚卸しと一括クリーンアップ」「ゾンビデータ全部処理」「自己修復基盤を実装」「絶対に既存 AI フィールドを上書きしない」。**全件分類**: `scripts/cleanup_all_topics.py` (582 行) で EMPTY / NO_AI / PARTIAL_AI / GOOD / ZOMBIE_FORWARD_ORPHAN / ZOMBIE_REVERSE_ORPHAN / ZOMBIE_STALE / ZOMBIE_NO_ARTICLES / ZOMBIE_BROKEN_META の 9 カテゴリ分類。誤削除防止: FORWARD_ORPHAN は 3 日以上更新なし・NO_ARTICLES は 1 日以上経過のみ対象 (in-flight トピック保護)。**並列実行**: ThreadPoolExecutor max_workers=16 で削除・pendingAI セット並列化 (1.5h → ~10min)、`python3 -u` + `sys.stdout.reconfigure(line_buffering=True)` で進捗即時可視化。**実行結果**: DynamoDB META 9287→1070 件 (約 8200 件のゴミ削除)、ZOMBIE 計 1141 件削除、pendingAI=True 投入 1068 件 (NO_AI 536 + PARTIAL_AI 532)、S3 reverse orphan 9 件 detail JSON/HTML 削除、孤立 HTML 全消去 (existing=646 vs surviving=1068 で残孤立 0)、CloudFront invalidation `I3HKT6ZMNQZ5D3HTZDBUM64XJR` (`/topics/*` `/api/topics*.json` `/sitemap.xml` `/news-sitemap.xml`)、processor lambda regenerateSitemap で topics.json/topics-full.json/topics-card.json 全件再生成 (105 件)。**自己修復基盤の物理ゲート**: ① `proc_config.PROCESSOR_SCHEMA_VERSION = 3` 導入、AI 成功時のみ書き込み、`needs_ai_processing` で `schemaVersion < PROCESSOR_SCHEMA_VERSION` を再処理対象判定。② `update_topic_with_ai` / `update_topic_s3_file` / topics.json マージに **incremental モード** — `existing_meta` 渡された場合、既存値が空でないフィールドは絶対に上書きしない (heal/再処理で蓄積した良い AI 要約を保護する物理ガード)、`handler.py` で `aiGenerated=True` トピックは自動的に incremental に切替。③ `scripts/quality_heal.py` (日次 cron) — keyPoint 空・schemaVersion 古い・statusLabel/watchPoints 欠落を検出し `ConditionExpression='attribute_exists(topicId)'` で安全に pendingAI=True 投入 (META 不在トピックへの誤書き込み物理ガード)。④ `.github/workflows/quality-heal.yml` で daily 06:00 JST に GitHub Actions が自動実行 (workflow_dispatch + dry-run input 対応)。⑤ `scripts/bulk_heal.sh` + `bulk_heal_python.py` で `all`/`no-keypoint`/`empty`/`old-schema` モード別の手動 heal コマンド (`APPLY=1` 環境変数で実行・デフォルト dry-run)。**Verified-Effect**: cleanup_apply 報告 (削除=1141 / キュー投入=1068 / GOOD=0→次サイクルで充填)、production `https://flotopic.com/api/topics.json` topics=105 aiGenerated=91/105 keyPoint=24/105(23%) schemaV3=0 (スキーマ移行直後)、freshness `updatedAt=age 1min PASS`、sitemap.xml + 個別 topic.html ともに HTTP 200。**思想的核**: 「修正したのに過去データが治らない構造的欠陥」を schema_version + 日次自動 heal + 既存データ保護モードで解消。`docs/rules/data-quality-playbook.md` (Cowork 並行作成・T2026-0428-AP) と方法論を共有し、他プロダクト (P002 / P006) に転用可能な汎用フレームワーク化。**1〜2 日後**: processor の 4x/day 自動実行で 1068 pending が順次処理され、keyPoint/statusLabel/watchPoints 充填率が 60%+ に回復見込み。
...
- ✅ **T2026-0428-AI 記事数グラフがトピック内記事数と乖離 修正** — ナオヤ報告:「記事数のグラフなんかおかしくない？トピック内の記事の数とあってないんだけど」。本番調査: topic `c0ccd7d87d35f1f1` で `meta.articleCount=20`/グラフ最終点=20 だが実表示「全 8 件の記事」、SNAP[*].articles 内 unique URL=7。同一 RSS 記事が複数フィードから来る場合、`fetcher/handler.py` の `cnt = len(g)` が水増しされる一方、SNAP.articles 書き込みは URL で dedup される構造的乖離。**Why1**: 同一 URL が複数フィードから来る (Yahoo!ニュース 等の再配信)。**Why2**: `all_articles.extend(fetched)` が dedup 無しで `cluster()` に渡る。**Why3**: 旧コードは `detail.js:558-560` で「グラフ最終点を `meta.articleCount` で補正」する band-aid を入れ、フッター・グラフ・タイムラインリストの 3 表示のうち 1 か所だけ症状を隠していた。**仕組み的対策3点**: ① `fetcher/handler.py` Step2.5 で `all_articles` を URL dedup（`[URL-DEDUP] N → M` ログ可観測） ② `detail.js` で `s.articleCount` 直参照を廃止し `s.articles?.length || s.articleCount` を使う `snapArtCount(s)` ヘルパー＋`displayArticleCount`（旧 S3 データでも実数表示できる二重ガード）band-aid 削除 ③ `tests/test_url_dedup_guard.py` 8件全通過: `[URL-DEDUP]` 文字列存続 / dedup が cluster より前 / inflated_count_scenario_real_world (21→7) / 空入力 / URL 欠如 silently skip 等。**効果検証**: 本番 `c0ccd7d87d35f1f1` で BEFORE グラフ 20 vs リスト 8 (ギャップ 13) → AFTER グラフ 7 vs フッター 8 (ギャップ 1, 概念的に正しい)。**詳細なぜなぜ**: `docs/lessons-learned.md` 2026-04-28 09。
...
- ✅ **T224 個人メールアドレス直書き (Lambda側完了・admin.html分割)** — 根本問題: CLAUDE.md 絶対ルール5「PII・secrets はコードに直書き禁止。env var か AWS Secrets Manager 必須」違反。修正 (lambda/contact/handler.py): `ADMIN_EMAIL = os.environ.get('ADMIN_EMAIL', 'mrkm.naoya643@gmail.com')` → `os.environ.get('ADMIN_EMAIL', '')` に変更し default を空に。さらに `verify_admin_token` 冒頭に `if not ADMIN_EMAIL: return False` を追加し、env var 未設定時は admin 機能を物理的に塞ぐ二重防御。残課題: `frontend/admin.html:296` の `allowedEmail: 'mrkm.naoya643@gmail.com'` は build 時注入機構を作る大改修になるため T224a として分割し、別タスクで対応 (Lambda 側は完了)。
...
- ✅ **docs/rules/global-baseline.md 新設** — ナオヤから繰り返し指摘される全プロダクト共通の前提条件（言語・根本原因・完了の定義・依存確認・PII禁止・なぜなぜ Why1〜Why5・仕組み的対策の質基準）を 1 ファイルに集約。P002・将来 P006 等で `CLAUDE.md` から参照する形で再利用できる。**§6 「AI が動きやすくなるための原則」に「気を付ける禁止」「仕組み的対策は CI / hook / metric / SLI / scripts のいずれかで物理化」を明記**——テキストルールだけで closure しない質基準を物理化。
- ✅ **CLAUDE.md 起動チェックを bootstrap 1 行に簡素化** — 12 行 bash ブロック → `bash /Users/murakaminaoya/ai-company/scripts/session_bootstrap.sh` 1 行 + 7 ステップの責務だけ箇条書き。「規則の置き場所」表に `docs/rules/global-baseline.md` を追加。CLAUDE.md は 132 行で 250 行ガード内維持。
