...
### 完了済み（2026-04-28 17:50 JST Code T2026-0428-AV Bluesky cron→EventBridge+Lambda 移行）

- ✅ **T2026-0428-AV BlueSky 自動投稿のトリガーを GH Actions cron → EventBridge+Lambda に移管** — T2026-0428-AU 調査で「GH Actions 無料枠の cron は 1日3〜5回しか発火しない・4系統併走でも GH 側問題なので根本解決にならない」と確定したのを受け、トリガーレイヤーを AWS 側に移管。**実装**: ① `scripts/bluesky_agent.py` の `main()` から `run(mode, dry_run) -> dict` を抽出し CLI と Lambda で同一ロジック共用 (DRY)。② `projects/P003-news-timeline/lambda/bluesky/handler.py` 新規 — 薄い Lambda ハンドラ、event/環境変数から mode/dry_run 解決して `run()` を呼ぶ。③ `.github/workflows/deploy-lambdas.yml` に bluesky デプロイステップ追加: ビルド時に `scripts/bluesky_agent.py` + `_governance_check.py` を Lambda dir にコピー、`atproto` を `--platform manylinux2014_x86_64` で同梱、関数未作成なら `create-function` + `wait function-active`、env vars (BLUESKY_HANDLE / APP_PASSWORD / SLACK_WEBHOOK / S3_BUCKET / SITE_URL) を毎デプロイ反映。`AWS_DEFAULT_REGION` は Lambda 予約キーのため env から除外。④ `.github/workflows/bluesky-agent.yml` の `schedule:` を全削除し `workflow_dispatch:` のみ残す (緊急の手動投稿フォールバック温存)。**インフラ**: Lambda `flotopic-bluesky` (python3.12 / 384MB / timeout 120s / role: 既存 `p003-lambda-role`) を GH Actions が新規作成。EventBridge `flotopic-bluesky-schedule` (rate(30 minutes) / ENABLED) → Lambda invoke を AWS CLI で作成、`AllowEventBridgeBlueskySchedule` permission も追加。target Input は `{"mode":"daily","dry_run":false}` 固定。**IAM**: `p003-lambda-role` の既存 inline policy (`flotopic-least-privilege`) が DynamoDB (`p003-topics` / `ai-company-bluesky-posts`) と S3 (`p003-news-946554699567`) の必要権限を既に保持していたためポリシー追加なし。**検証**: ① workflow run 25041505589 (`workflow_dispatch`) で Lambda デプロイ完了 → State=Active / LastUpdateStatus=Successful、② `aws lambda invoke --payload '{"mode":"daily","dry_run":true}'` で StatusCode=200 / `{"ok":true,"mode":"daily","dry_run":true}` 受領、③ CloudWatch Logs `/aws/lambda/flotopic-bluesky` で auth 成功 → debut チェック → daily ルート → クールタイム skip まで完走確認、④ 既存 GH Actions cron ('*/30' 等) は workflow yml から削除済 (workflow_dispatch のみ残置)。**初回ハマりどころ (なぜなぜ)**: 初回デプロイで `update-function-configuration` が `ResourceConflictException(State=Pending,Creating)` で失敗 → Lambda は `create-function` 直後 Pending で返るので `wait function-active` を間に挟むのが正解。次に `InvalidParameterValueException` で AWS_DEFAULT_REGION が予約キー判明 → Lambda runtime が自動設定するので env 指定不要。**実 Bluesky 投稿の最終確認**: 直近 daily 投稿 (06:50Z) のクールタイム 2.5h が 09:20Z に明け、次 EventBridge 発火 09:30Z (18:30 JST) で実投稿予定。投稿確認は別フォローアップで append 予定。Verified: 25041505589:success:2026-04-28T17:00+0900 (Lambda デプロイ run) + 50f99125-398a-419f-868a-402fa51ddf56:200:2026-04-28T17:24+0900 (Lambda invoke RequestId)。
