...
| ~~T2026-0502-AD~~ | ✅ 完了 (2026-05-02 PR #242 merged / 完了マーク 2026-05-03) | ~~**fetcher false-split 解消 — entity hierarchy で rule-based 解決**~~ — embedding (multilingual-e5-small) Phase 1 bench で misses=3/6・閾値分離不可能のため pivot。rule-based hierarchy で「欧州⊃ドイツ」「関西⊃大阪」「政府⊃財務省」「米軍⊃駐留米軍」等を shared_entities 拡張で表現。**完了内容**: ① `ENTITY_HIERARCHY` dict (地理/組織/国際機関) を `config.py` に追加 ② `hierarchy_aware_overlap()` を `text_utils.py` に追加 ③ `handler.py` の shared_entities 計算箇所を置換 ④ `tests/test_entity_hierarchy.py` 新設 6 ケース全 PASS ⑤ 既存 `test_title_dedup_guard.py` 35 件 regression なし ⑥ PR #242 merge + Lambda LastModified 2026-05-02 16:04 JST 確認済。**改善継続性**: 新 false-split 観察 → fixture 1 件追加 + dict 1 行追加で恒久解消できる構造確立。**Verified-Effect-Pending: 2026-05-09** — production fetcher_health の suspected_mismerge_count 推移確認 + 「欧州駐留米軍」型 false-split 消滅確認 | `lambda/fetcher/config.py`, `lambda/fetcher/text_utils.py`, `lambda/fetcher/handler.py`, `projects/P003-news-timeline/tests/test_entity_hierarchy.py` | 2026-05-02 |

</details>


### 自動 triage: 2026-05-03 に TASKS.md から移動した取消線済みタスク
