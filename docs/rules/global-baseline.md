...
> POから繰り返し言われている「前提条件」を 1 ファイルに集約。
...
| 全プロダクト共通の前提は本ファイルに集約 | POから繰り返し言われた事は本表 §1 / §6 / §8 に追記する。CLAUDE.md には固有ルールのみ |
...
| **完了の定義** | 効果検証は時間がかかるため、実装＋引き継ぎ登録＋POへの報告の 3 点が揃った時点で当該タスクは「完了」扱い。効果検証結果の確認は別タスクとしてスケジューラー側で発火する |

> **理由**: 「効果が出るまで Dispatch を起動したままにしておく」運用は (a) セッション並走上限を圧迫する (b) コンテキストが切れて引き継ぎ漏れる (c) POが追加指示できない、の 3 重に詰まる。スケジューラーに渡せば人と AI の双方が手を空けられる。
...
## 11. デプロイ後の初回動作チェックはスケジューラー義務（2026-04-30 制定）

**PR merge → deploy 完了後、必ずスケジューラーで「初回動作確認タスク」を仕掛けてからセッションを終了する。CI が green ≠ 本番で動いている。**

| 規則 | 内容 |
|---|---|
| **義務** | `feat:` / `fix:` / `perf:` を含む PR を merge・deploy したら、deploy 完了直後にスケジューラー（scheduled task / GitHub Actions cron）で「N 分後に本番動作確認」を登録する。登録なしでセッション終了は禁止 |
| **確認内容** | ① CloudWatch ログでエラー消滅確認 ② 本番 URL / API エンドポイントで期待値が返るか ③ ユーザー目線でブラウザ実機確認（トップページ・主要機能） |
| **タイミング** | deploy 完了から「処理が走るまでの時間」+ 余裕を加算。cron 駆動なら「次回 cron 実行時刻 + 10 分後」を目安に fireAt を設定 |
| **禁止** | deploy 後にコードセッション内で sleep / polling / Monitor で待機。§10 と同様に「セッションを開いたまま待つ」は禁止 |
| **失敗時** | スケジューラーが「本番で動作不一致」を検出したら `TASKS.md` に緊急タスクを積む。Dispatch に通知が来たら次のセッションで対応 |

> **背景**: 「CI が通ったら完了」のままだと「デプロイ後の初回動作確認」が常に省略され続ける。これはルール違反だが仕組みがないと守れない。スケジューラーを「deploy セットで必ず作る成果物」にすることで、確認を物理的に保証する。
> **良い例**: `done.sh T2026-0430-D url:https://flotopic.com/` → `create_scheduled_task fireAt=<次回cron+10min>` で動作確認 → セッション終了
> **悪い例**: CI green を確認して「完了」報告 → ブラウザ確認せずセッション終了

---

## 改訂履歴

- **2026-04-30** §11 「デプロイ後の初回動作チェックはスケジューラー義務」追加（CI green ≠ 本番動作保証の構造的欠陥を物理ゲートで補う）
...
- **2026-04-28 (v2)** §8/§9 順序バグ修正 + §8 にPO前提 2 行追記（汎用形要請・無理して更新しない）
