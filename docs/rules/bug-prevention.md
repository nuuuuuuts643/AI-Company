...
| **SLIフィールドは実装スキーマと一致させる・CI物理検証** | freshness-check.yml が `t.get('FOO')` で測定するフィールド名は、proc_ai.py の Tool Use schema または handler.py の `ai_updates` dict に必ず存在すること。実装側で削除されたフィールドを SLI が測り続けると 0% を観測し続け「success-but-empty 半壊」を見逃す（過去事例: `situation` 0% / `background` 0% 放置）。`scripts/check_sli_field_coverage.sh` が CI で突合検査する（T2026-0429-M）|
