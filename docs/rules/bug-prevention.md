...
| **コスト増加実装は即スキップ（全タスク共通）** | AIコール回数増加（リトライ・再生成ループ）・外部API追加呼び出し・新規AWS有料リソース作成など、コストが上がる実装は実施前に必ず停止してPOに確認する。確認なしで進めた場合は即ロールバック。「品質向上のため」は理由にならない。プロンプト文言強化・測定スクリプト追加・警告ログ出力はコスト増に該当しない |
