...
| 観点 | 確認項目 |
|---|---|
| **コード直書き禁止** | `ghp_*` `gho_*` `ghs_*` `ghr_*` `github_pat_*` `xoxb-*` `xoxp-*` `xapp-*` `sk-ant-api03-*` `sk-proj-*` `ntn_*` `secret_*` `AKIA*` のいずれも tracked file に書かない (テストではダミー / 例: `sk-ant-dummy-for-test`) |
