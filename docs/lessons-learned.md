...
| freshness-check SLI 8/9/10 | keyPoint 充填率 11.5% 素通り | 同上 | ✅ | — |
| freshness-check SLI 11 sitemap_reach | sitemap 50/50 件 404 | 同上 | ✅ | — |
| quality-heal 日次 cron | ゴミデータ慢性蓄積 | `.github/workflows/quality-heal.yml` | ✅ | — |
| schemaVersion による自動再処理 | 同上 | `lambda/processor/proc_storage.py:286-313` | ✅ | — |
| WORKING.md 8h TTL 自動削除 | 並行タスク事故 | `scripts/session_bootstrap.sh` | ✅ | — |
| needs-push 滞留警告 | Cowork↔Code 連携 | 同上 | ✅ | — |
...
| 横展開チェックリスト自動検証 | 2026-04-28 PM 本セッション | `scripts/check_lessons_landings.sh` | ✗ | 本セッションで TASKS 化 |
| ロールバック手順 runbook | 2026-04-28 PM フェーズ1 完了条件 | `docs/runbooks/rollback.md` | ✗ | 本セッションで TASKS 化 |
| Dispatch セッション並走ゲート | 2026-04-28 PM Bluesky 125 ターン | `CLAUDE.md` Cowork↔Code 連携セクション | ✅ | 本 commit で文章化 + WORKING.md 並走 ≥3 件警告は既存 |
| buildFilters 境界値テスト | 2026-04-28 PM buildFilters バグ | `projects/P003-news-timeline/tests/unit/build_filters.test.js` | ✗ | 次回タスク |
