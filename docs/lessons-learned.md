...
**横展開チェックリスト**:
- [x] `docs/rules/conflict-resolution.md` を新規作成（T2026-0502-H 完了 / `docs/rules/conflict-resolution.md`・shared docs 5 ファイル定義 + 3-way マージ手順 + `--ours/--theirs` 禁止明記）
- [x] `scripts/conflict_check.sh` を新規作成（T2026-0502-H 完了 / `scripts/conflict_check.sh` + `tests/test_conflict_check.sh` 9 ケース pass + `scripts/session_bootstrap.sh` 起動時 §2b で発火・ERROR 時 bootstrap 中断）
