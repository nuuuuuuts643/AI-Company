...
## 2026-05-02 観測フラグだけ立てて自動アクションを実装しない (T2026-0502-N)

**事象**: `suspectedMismerge=true` フラグが DynamoDB に記録されていたが、フロントエンドでの表示もなく、processor での AI 生成スキップもなく、ユーザーへの導線もなかった。SLI で `mismerge_suspected_rate` を集計できる状態だったが、フラグを立てるだけで「何も起きない」状態になっていた。

**Why1**: 検知シグナルは実装したが「検知後にどう動くか」の設計が未完だった。
**Why2**: `suspectedMismerge` は SLI 用フラグとして設計されていたが、それ以上のアクション定義がなかった。
**Why3**: 「まず観測してから対策」の進め方で、観測フェーズで止まっていた。
**Why4**: フラグを「観測インフラ」と位置づけたことで、実装スコープが観測止まりになった。
**Why5**: 観測フラグと物理アクションの実装を同一チケットにしなかった。

**仕組み的対策**:
1. **フラグ実装 = アクション実装を同時に要求**: 新規フラグを追加する PR は「このフラグが立ったときに何が起きるか」を必ず実装する（フロント表示 or processor スキップ or S3 キュー登録）。
2. **30日超 time_gap → split_candidate 物理化** (本 PR T2026-0502-N): `statusLabel="精査中"` + `pendingAI=False` + S3 split 候補キュー登録を実装。
3. **count_spike 誤検知抑制** (本 PR): スポーツ/エンタメジャンルの count_spike 単独フラグを抑制し、精度を向上。

**横展開チェックリスト**:

| アンチパターン | 確認ファイル | 状態 |
|---|---|---|
| 観測フラグだけ立てて自動アクションなし | `lambda/fetcher/merge_audit.py` | ✅ 本 PR で split_candidate アクション追加 |
| ジャンル無視の誤検知 | `merge_audit.detect_mismerge_signals` | ✅ 本 PR で genre 引数追加・スポーツ/エンタメ抑制 |
