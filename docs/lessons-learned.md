...
- 2026-05-02 pre-commit IAM filter (c521a846) が diff 行内容に対して exemption を当てて *.md / lessons-learned.md 内のドキュメント引用まで誤検知 → bypass 強制構造（T2026-0502-IAM-FILTER-FIX・file-path ベースに書換 + 5 ケース回帰テスト + CI 物理化）
...
## pre-commit IAM filter が diff 行内容に対して exemption を当てて誤検知 (2026-05-02・T2026-0502-IAM-FILTER-FIX)

**起きたこと**:
- 2026-05-02 23:00 JST、`feat/T2026-0502-BJ-cowork-token-file` で main からの merge を conclude しようとしたら pre-commit hook (T2026-0502-IAM-DRIFT-FIX2 / commit c521a846 / PR #300) が `docs/lessons-learned.md` 内のドキュメント文字列 `aws iam put-role-policy` を誤検知して reject。
- merge 内容は既に main で CI 通過済 (99831ad7) で、ドキュメント追記しか含まれていなかったが、hook が bypass を強要する構造になっていた。
- `--no-verify` で escape したが、これは「物理ガードのつもりが実は思想ルール」の典型構造。

**なぜなぜ**:

| Why | 答え |
|---|---|
| **Why1** なぜドキュメント追記が IAM hook で reject された？ | hook の exemption regex が `git diff --cached -U0` の **`+` で始まる行内容** に対して `grep -vE 'infra/iam/apply\.sh\|install_hooks\.sh\|\.md:\|lessons-learned\|...'` を適用していたが、diff 出力の `+` 行は **追加された内容そのもの** であり file path は含まれていなかった。`*.md` ファイル内の `aws iam put-role-policy` 引用は exemption が効かず素通しで pattern match まで到達した |
| **Why2** なぜ author は file-path 判定だと信じていた？ | exemption の意図は commit message にも明記されていた（「ドキュメント (.md) 内の説明引用」を例外にする）。author は `\.md:` パターンが diff hunk header の `+++ b/file.md` に効くと思い込んでいた可能性が高いが、直前の `grep -vE '^\+\+\+'` で `+++` 行は既に除外されており、残ったのは content 行だけ。author が実装後に exempt 想定の commit を実際に試さなかった |
| **Why3** なぜ実装後に exempt 想定 commit を試さなかった？ | 当時 (2026-05-02 22:30 JST PR #300) は「`aws iam put-role-policy` 直呼びを物理 reject する」という **真陽性側のテストしかしていない**。回帰テスト (false positive 防止) が無く、CI にも組み込まれていなかった。「reject されること」だけを確認して「reject されないこと」を確認しなかった |
| **Why4** なぜ回帰テストの不在に気付かなかった？ | 同 PR が `docs/lessons-learned.md` を変更していたのに hook が反応しなかったのは、**まだ hook が install されていないリポでの commit だったから**。author 自身が commit したタイミングでは hook が現用化される前で、誤検知が顕在化しなかった。後から `install_hooks.sh` 経由で他クローンに伝搬した時に初めて誤検知が表に出る、時間差攻撃 |
| **Why5** なぜ「物理ガードを足したら反対方向の回帰テストも必須」を制度化していなかった？ | 過去の lessons-learned (2026-05-02 IAM-DRIFT-FIX 末尾「物理ガード追加時は必ず deliberate violation 注入テストで動作確認」) は **真陽性側の確認のみ** を要求していた。「exempt が効くか」を確認する偽陽性側テストの強制が漏れていた。物理ガードの仕様 = (a) 違反を捕捉する + (b) 正当な diff を通す、の 2 軸であることを明文化していなかった |

**仕組み的対策（物理 ★ 優先）**:

| # | 対策 | 強度 | 実装 |
|---|---|---|---|
| 1 | **file-path ベース exemption に書き換え (★物理)** | 物理 | `scripts/install_hooks.sh` の IAM filter を `git diff --cached --name-only --diff-filter=AM` でファイル単位に exempt 判定するループに書き換え。exempt path: `infra/iam/apply.sh` / `scripts/install_hooks.sh` / `*.md` / `tests?/` / `test_*.{py,sh,js,ts,yml,yaml}` / `.git/hooks/`。content 行に対する除外は廃止（regex を path にだけ当てる） |
| 2 | **5 ケース回帰テスト (★物理)** | 物理 | `tests/test_pre_commit_iam_filter.sh` 新設。一時 git repo を tempdir に作って install_hooks.sh を実行し、(Case 1) `*.md` 内ドキュメント引用通過 / (Case 2) `tests/` fixture 通過 / (Case 3) `infra/iam/apply.sh` 自体通過 / (Case 4) `scripts/some_iam_script.sh` 違反 reject / (Case 5) workflow YAML 違反 reject — を全部 assert |
| 3 | **CI 物理化 (★物理)** | 物理 | `.github/workflows/ci.yml` の hook-tests job に `bash tests/test_pre_commit_iam_filter.sh` を追加。次に install_hooks.sh の IAM filter を編集する Claude が exempt 系/violation 系のどちらかを壊すと CI で必ず落ちる |
| 4 | **物理ガード追加時の回帰テスト 2 軸を制度化 (補助思想)** | 思想 | 本ファイル冒頭の「物理ガード追加時は必ず deliberate violation 注入テストで動作確認」を「(a) 違反 → reject されること (b) 正当な exempt 対象 → reject されないこと の **両方** を deliberate に注入してテスト」に書き換え (本 Edit で同時に実施。1〜3 が回避された場合の補助テキスト軸) |

**横展開チェックリスト**:

- [x] `scripts/install_hooks.sh` IAM filter を file-path ベースに書換（実装ファイル: `scripts/install_hooks.sh` L124-167・**物理**・本 task で完了）
- [x] `tests/test_pre_commit_iam_filter.sh` 5 ケース回帰テスト追加（実装ファイル: `tests/test_pre_commit_iam_filter.sh`・**物理**・本 task で完了）
- [x] `.github/workflows/ci.yml` で回帰テストを CI 強制（実装ファイル: `.github/workflows/ci.yml` の hook-tests job 内・**物理**・本 task で完了）
- [ ] 他の pre-commit/commit-msg hook の filter (PII / cost-discipline anti-pattern / Verified-Effect 等) も同じ構造的欠陥を持っていないか監査（タスク化候補: T2026-0503-HOOK-FILTER-AUDIT・補助思想・別 task で着手）
- [ ] `scripts/check_lessons_landings.sh` に本 lesson の 3 物理 landing (install_hooks.sh L124-167 / tests/test_pre_commit_iam_filter.sh / ci.yml の hook-tests job) の grep 検証を追加（タスク化候補: T2026-0503-LANDING-CHECK-EXTEND・**物理**）

**この教訓から導かれる一般原則**:

- **物理ガードは「違反を block できるか」と「正当な diff を通すか」の 2 軸テストで初めて完成**。片方だけだと、もう片方の失敗が「全員に bypass を強制する」形で運用負荷を撒き散らす。
- **diff 出力の `+` 行は file-path を含まない**。file path で exempt したいなら `git diff --cached --name-only` でファイル列挙してからループするのが唯一正しい設計。`grep -vE 'path-pattern'` を `+` 行に当てるのは構造的にバグ。
- **install_hooks.sh で配布する hook は author の手元で必ず exempt 想定 commit を再現テストする**。手元で誤検知に当たらなかった理由が「author が hook install 前に commit していたから」というケースが本事例。配布前に install→exempt commit→pass を毎回手で踏むこと（または本事例のように tests/ 経由で機械化）。
