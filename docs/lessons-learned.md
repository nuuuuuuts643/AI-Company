...
| 3 | **Lambda 間 invoke 定期棚卸し（★観測）** | 観測 | `p003-haiku` の月次チェックに `grep -rn "boto3.client.*lambda\|InvocationType" lambda/` を追加し、invoke 経路の一覧を TASKS.md に自動記録。残存 invoke が PO 承認済みかを確認するフロー化 |
