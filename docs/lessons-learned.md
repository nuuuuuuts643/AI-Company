...
- [ ] **PO 操作 (Mac で 1 回・本 PR merge 後)**: `gh auth token > ~/ai-company/.cowork-token && chmod 600 ~/ai-company/.cowork-token`
- [ ] **PO 操作 (Mac で・PO 判断)**: SEC2 ERROR 完全解消したい場合は `git -C ~/ai-company remote set-url origin https://github.com/nuuuuuuts643/AI-Company.git` で URL から token 剥がし
- [ ] 効果検証 (Eval-Due 2026-05-03): Cowork セッションから cowork_commit.py で PR を出せることを実証
- [ ] 同 pattern (Cowork sandbox と Mac 環境差分) の他の認証経路に潜在問題がないか棚卸し: AWS auth (現状 IAM access key) / Slack token / Anthropic API key 等
