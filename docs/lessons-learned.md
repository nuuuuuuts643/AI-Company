...
## workflow YAML が repo に存在しないファイルを参照していた (T2026-0502-WORKFLOW-DEP-PHYSICAL・2026-05-02 22:55 JST 制定)

**起きたこと**:
- c521a846 (PR #300) で `infra/iam/apply.sh` が `scripts/iam_canon.py` を参照する形に書き換わったが、実体ファイルが commit されず ENOENT
- 918621d3 (PR #302 / 私の LINT-YAML-FIX2) で `iam-policy-drift-check.yml` の canon() も同 helper 経由化 → 同じく ENOENT
- IAM policy drift check workflow が **~50 分 failure** 続行 (c521a846 13:30 → 2932b46c 13:42 で復旧)
- `git log -- scripts/iam_canon.py` → 0 commits (歴史を通じて 1 回も commit されていない)

**なぜなぜ**:

| Why | 答え |
|---|---|
| **Why1** なぜ参照先ファイルが存在しないまま merge された？ | PR レビューで「workflow YAML が参照するファイルが repo にあるか」を機械的に確認する CI が無かった (思想ルールのみ) |
| **Why2** なぜ思想ルールだけで運用されていた？ | 「commit すれば全ファイルが含まれる」前提が暗黙化。新規ファイルの commit 漏れを workflow 側で検出する物理 hook 不在 |
| **Why3** なぜ apply.sh の存在チェック (L49-51) が CI で発火しなかった？ | apply.sh は CI で実行されない (manual + IAM 権限要)。drift check 側は実行されるが、ENOENT で 0 秒 fail するだけで「missing reference」と人間に伝わらない |
| **Why4** なぜ 2 回目 (LINT-YAML-FIX2) で同じ罠を踏んだ？ | drift-check workflow で canon() を `python3 scripts/iam_canon.py` に書き換える前に、ファイルが repo に存在するか **私が確認しなかった**。「apply.sh が同 path で参照しているから問題ないだろう」という暗黙仮定 |
| **Why5** なぜ事前確認の物理ガードが無かった？ | workflow YAML から外部スクリプトを呼ぶ際に「参照先が repo に commit されているか」を CI 側で機械検証する rule 自体が**存在しなかった** (思想止まり) |

**仕組み的対策 (本 PR で landing)**:

1. **`scripts/ci_check_workflow_script_refs.sh` 新設** — `.github/workflows/*.yml` 内で `python3 scripts/foo.py` / `bash infra/foo.sh` / `./scripts/foo.sh` 等の参照を抽出し、実体が repo に存在しなければ exit 1
2. **`.github/workflows/lint-yaml-logic.yml` に新ステップ追加** — 既存「No inline logic in YAML」と並列で `ci_check_workflow_script_refs.sh` を毎 push/PR で実行
3. **`scripts/check_lessons_landings.sh` に landing 物理検証追加** — 上記 2 ファイルの存在 + workflow 内 step 呼び出しを grep verify。物理ガード自身が消されたら CI が即 fail
4. **(思想・補助) PR レビューチェックリスト** — `.github/PULL_REQUEST_TEMPLATE.md` に「workflow YAML を編集した場合、参照する scripts/* / infra/* が同 PR に含まれるか確認」を追加検討 (T2026-0502-PR-TEMPLATE 別タスク化候補)

**横展開チェックリスト**:
- [x] `scripts/ci_check_workflow_script_refs.sh` 新設・ローカル動作確認済 (51 workflows checked + 検出力テスト pass)
- [x] `.github/workflows/lint-yaml-logic.yml` に新ステップ追加・既存 step と並列実行
- [x] `scripts/check_lessons_landings.sh` に T2026-0502-WORKFLOW-DEP-PHYSICAL の landing 物理検証 grep block 追加
- [ ] 効果検証: 本 PR merge 後の lint-yaml-logic.yml run で新 step が success・かつテスト用 missing ref を仮 commit したら fail することを次セッションで確認 (Eval-Due 2026-05-09)
- [ ] 同 pattern: `infra/iam/apply.sh` 含む全 shell script も「参照する scripts/* / infra/* が repo に存在するか」を CI で物理 reject 化検討 (T2026-0502-SHELL-DEP-PHYSICAL 別タスク化候補)
- [ ] 同 pattern: pre-push hook で `lint-yaml-logic.yml` 相当を local 実行 (上記 IAM-DRIFT-FIX2 横展開と統合・T2026-0502-LOCAL-LINT-PRE-PUSH)

**この教訓から導かれる一般原則**:
- **「思想ルール」は physical 化されるまで「気をつける」と同強度**。本件は「commit 忘れに気をつけて」が 2 回連続で破綻し ~50 分の本番 CI 障害に直結した
- **workflow YAML から script を呼ぶ = repo に存在 commit されている契約**。契約を機械検証しないと commit miss が時限爆弾化する
- **既存の `[ -f "$CANON" ] \|\| exit 1` チェックは "reactive" であって "preventive" ではない**。preventive (PR を block) と reactive (実行時に exit 1) の差を区別する
