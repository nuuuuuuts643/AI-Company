...
- 2026-05-02 Lambda 間 invoke のコスト盲点 — fetcher → processor 即時トリガーが月 +$17 を無音で消費（乗算コスト視点の欠如・T2026-0502-J）
...
**横展開チェックリスト**:
- [ ] `docs/rules/conflict-resolution.md` を新規作成（タスク化: T2026-0502-CONFLICT-DOC）
- [ ] `scripts/conflict_check.sh` を新規作成（タスク化: T2026-0502-CONFLICT-SCRIPT）
- [ ] `done.sh` に PR merged + main green チェック追加（タスク化: T2026-0502-DONE-GATE）
...
## Lambda 間 invoke のコスト盲点 — fetcher → processor 即時トリガーが月 +$17 を無音で消費（2026-05-02 T2026-0502-J）

**起きたこと**:
- fetcher が新規ペンディングトピック発生ごとに `_lambda.invoke('p003-processor', InvocationType='Event')` を実行していた
- CloudWatch Logs Insights 実測 (2026-05-02): Anthropic API calls = 164/24h のうち 111 calls (68%) が fetcher trigger 由来
- 設計意図は「新規トピック → AI 要約のレイテンシを 6h → 30min に短縮」だったが、コスト試算は誰も行っていなかった
- PO が異常 API call 数を Cowork 観測で発見して初めて発覚。月 +$17/month の想定外支出が継続していた

**なぜなぜ**:

| Why | 答え |
|---|---|
| **Why1** なぜコスト超過が続いた？ | fetcher → processor の即時 invoke が毎 fetch サイクルで実行され、1日 53 calls のはずが 164 calls に膨らんでいた。しかし誰も Anthropic API call 数を SLI で監視していなかった |
| **Why2** なぜ SLI が存在しなかった？ | Anthropic API コストは「Lambda 課金」として扱われ、Anthropic usage dashboard での月次確認のみ。日次・時次の call 数を CloudWatch で観測する仕組みがなかった |
| **Why3** なぜ Lambda 間 invoke をコスト軸で設計しなかった？ | レイテンシ改善の目的のみで実装され、「invoke が増えると下流 Lambda の API call が増加する」という乗算コストの視点がレビューになかった |
| **Why4** なぜレビューで気付けなかった？ | 新機能実装時のコスト試算テンプレートが存在しない。「Anthropic API call は processor のみが発生させる」という暗黙前提で、fetcher が間接的に呼び出す経路は考慮外だった |
| **Why5** なぜ Lambda 間 invoke の棚卸しが行われていなかった？ | `boto3.client('lambda').invoke(...)` パターンを定期的に grep して「想定通りか / コスト正当化できるか」を確認する運用がない |

**仕組み的対策**（最低 3 つ）:

| # | 対策 | 強度 | 実装 |
|---|---|---|---|
| 1 | **Anthropic API call 数の日次 SLI 観測（★物理）** | 観測 | `freshness-check.yml` または `p003-haiku` にて CloudWatch Logs Insights で `[METRIC] claude_cache` を集計し、閾値 (例: 80 calls/24h) 超過で Slack alert + TASKS.md 起票。コスト異常を翌日以内に検出 |
| 2 | **Lambda 間 invoke 実装時のコスト試算 requirement（★物理）** | 物理 | `.github/PULL_REQUEST_TEMPLATE.md` の Lambda 変更セクションに「cross-Lambda invoke を追加する場合: invoke 頻度 × 下流 API call 数 × 月コスト試算を記載」を必須フィールドとして追加。PR 本文に試算なければ CI が fail |
| 3 | **Lambda 間 invoke 定期棚卸し（★観測）** | 観測 | `p003-haiku` の月次チェックに `grep -rn "boto3.client.*lambda\|InvocationType" lambda/` を追加し、invoke 経路の一覧を TASKS.md に自動記録。残存 invoke 一覧を PO 承認スタンプ付きで TASKS.md に記録し、未承認エントリーは月次棚卸しで再評価 |
| 4 | （補助）新機能実装時の乗算コスト確認 | 思想 | 「A が B を呼ぶ設計」の場合、A の呼び出し頻度 × B のコスト単価 の乗算を設計レビュー時に明示。Lambda invoke は「1回の安さ」ではなく「総呼び出し数」で判断する |

**横展開チェックリスト**:
- [x] fetcher → processor 即時 invoke 削除（T2026-0502-J・PR #146 / 2026-05-02）
- [ ] Anthropic API call 日次 SLI を freshness-check.yml または p003-haiku に追加（タスク化推奨・**観測**）
- [ ] PULL_REQUEST_TEMPLATE.md の cross-Lambda invoke コスト試算フィールド追加（**物理**）
- [ ] `p003-haiku` 月次チェックに invoke 棚卸し grep 追加（**観測**）
