...
1. **物理 (主要ページ noindex 物理ガード)**: `scripts/check_seo_regression.sh` を新設。`frontend/{index,topic,catchup,storymap,about,contact,terms}.html` に `noindex` が混入したら CI で exit 1。`.github/workflows/ci.yml` に組込。本 PR で landing。タスク: **T2026-0502-AQ**
2. **物理 (Cloudflare PV 前日比急変 SLI)**: `freshness-check.yml` に SLI 15 追加。`api/cf-analytics.json` の `cf.daily` を読んで前日比 0.7 未満で WARN・0.5 未満で ERROR を Slack 通知。重複 alert 防止のため UTC 22 時台 (cf-analytics 更新直後) のみ判定。本 PR で landing
3. **物理 (主要ページ noindex 物理ガード) の 2 重防御**: 上記 #1 に加え、frontend の各 HTML に「T2026-0502-AQ: noindex 削除」コメントを残し将来の AI セッションが安易に再追加しないよう注意喚起。本 PR で landing
4. **思想 (フェーズ前提を壊す原則の明記)**: `docs/product-direction.md` に「フェーズ N の前提 (例: 流入・観測点・主要 KPI 取得経路) を壊す可能性のある PR は、PR description に `Phase-Impact-Conflict:` 行で明示する」を明記。物理化困難 (運用判断) のため 1 ヶ月運用後に PR template への組込を検討
...
1. **物理 (auto-merge.yml の PAT 化)**: `GH_TOKEN: ${{ secrets.AUTO_MERGE_PAT || secrets.GITHUB_TOKEN }}` に変更 (本 PR で実装済)。PAT (= user identity) で merge することで GITHUB_TOKEN 制約を回避し、push event / pull_request closed event が他 workflow を正常に trigger する。**PAT 未設定時は GITHUB_TOKEN にフォールバック + warning ログ** (壊さない設計)。タスク: **T2026-0502-AT**・**物理**
2. **物理 (PAT scope 最小化 + expire 90 日)**: PAT の権限は `repo` + `workflow` のみ・expire 90 日設定で rotation 強制。secret-scan.yml の `gh[ops]_` パターン検出で漏洩は CI で物理ガード済 (T2026-0502-SEC2)
3. **物理 (横展開チェックリスト追加)**: lessons-learned.md の本セクションに「auto-merge bot 由来の trigger 失敗を Lambda 用に直したら frontend / その他にも横展開する」を明記 + 確認パターン (workflow runs API で `event=pull_request` 由来の run が total=0 なら異常)
4. **思想 (中期 GitHub App 化)**: PAT は user 紐付きで離脱リスクあり。GitHub App は Bot identity で長期運用に向く。1 ヶ月運用後に検討 (T2026-0502-AV タスク化)
