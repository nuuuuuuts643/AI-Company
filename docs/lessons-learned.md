...
- 2026-05-02 lifecycle Lambda が SK を FilterExpression に使い ValidationException 連発（DynamoDB PK/SK は FilterExpression 禁止）
...
## 2026-05-02 lifecycle Lambda が SK を FilterExpression に使い ValidationException 連発（T2026-0502-B）

**症状**: p003-haiku アラートで `ValidationException: Filter Expression can only contain non-primary key attributes: Primary key attribute: SK` が直近24h複数回発生。lifecycle Lambda が毎週次の Scan で失敗していた。

| ステップ | 内容 |
|---|---|
| **Why1** なぜ ValidationException が出た？ | `table.scan(FilterExpression=Attr('SK').eq('META'))` で SK（sort key = primary key の一部）を FilterExpression に指定したため |
| **Why2** なぜ SK を FilterExpression に使えないのか？ | DynamoDB の FilterExpression は **非 PK 属性のみ**を対象とする仕様。PK/SK のフィルタは KeyConditionExpression（Query 時のみ）で行う |
| **Why3** なぜ実装時にそのルールを見落とした？ | 「Scan + FilterExpression で項目を絞れる」という認識があったが「PK/SK は使えない」という制約まで意識していなかった |
| **Why4** なぜ早期に気付かなかった？ | ローカルテストで DynamoDB Local / LocalStack を使わなかったため、実際のデータがある本番環境でのみエラーが顕在化した |
| **Why5** なぜ横展開がなかった？ | `FilterExpression` に PK/SK を使う禁止パターンの grep ルールが存在しなかった。同様のコードが lifecycle 以外にも 4 箇所存在していた（proc_storage.py × 2、api/handler.py、fetcher/storage.py） |

**仕組み的対策:**

1. **根本修正（本コミット実装済み）**: `FilterExpression` から SK を除去し、全件 Scan 後に Python 側で `item.get('SK') == 'META'` フィルタ（案A）。対象5箇所を同時修正: `lifecycle/handler.py:149`、`proc_storage.py:505`、`proc_storage.py:1273`、`api/handler.py:36`、`fetcher/storage.py:49`。

2. **横展開ガード（思想ルール）**: DynamoDB Scan/Query で `FilterExpression` を書く際は「SK や topicId（PK）が含まれていないか」を必ずチェックする。PK/SK を絞りたい場合は Query + KeyConditionExpression、または全件 Scan 後 Python フィルタ（小テーブル向け）。

3. **将来的な恒久対処**: TODO(T2026-0502-B-followup) — テーブル規模が増大した場合は SK を GSI（Global Secondary Index）に投影して Query に切り替える。

### 横展開チェックリスト

| 対策名 | 適用対象 | 実装ファイル | 状態 |
|---|---|---|---|
| SK を FilterExpression から除去 (Python フィルタに変更) | lifecycle Scan | `projects/P003-news-timeline/lambda/lifecycle/handler.py` | ✅ 本コミット |
| SK を FilterExpression から除去 | proc_storage pendingAI replenish | `projects/P003-news-timeline/lambda/processor/proc_storage.py:505` | ✅ 本コミット |
| SK を FilterExpression から除去 | proc_storage get_all_topics_for_s3 fallback | `projects/P003-news-timeline/lambda/processor/proc_storage.py:1273` | ✅ 本コミット |
| SK を FilterExpression から除去 | api all_topics fallback | `projects/P003-news-timeline/lambda/api/handler.py:36` | ✅ 本コミット |
| SK を FilterExpression から除去 | fetcher get_all_topics fallback | `projects/P003-news-timeline/lambda/fetcher/storage.py:49` | ✅ 本コミット |
