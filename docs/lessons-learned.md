...
## T2026-0504-REFACTOR-SPLIT-CI: ファイル分割後の CI 参照先更新漏れと修正セッションの暴走 (2026-05-04)

**What**: proc_ai.py をファイル分割（proc_formatter.py / proc_genre.py / proc_prompts.py 分離）したとき、CI スクリプトの hardcode 参照が更新されず main CI が落ちた。CI 修正セッションが暴走して merged PR ブランチに無用な push を繰り返した。

**Why1**: ファイル分割 PR に「CI スクリプトの参照先を確認する」手順がなかった
→ PR 出す前に既存スクリプトが古いファイル名を参照していないか grep する確認が欠落。

**Why2**: `scripts/` 内に特定ファイル名を hardcode している箇所が把握されていなかった
→ `check_ai_fields_catalog.py`・`check_ai_fields_coverage.py` など複数スクリプトが `proc_ai.py` を直接 import していた。関連スクリプト一覧を PR レビュー時に明記していなかった。

**Why3**: CI 修正セッションへの指示が「CI を直す」と広すぎて、スコープが無限に広がった
→ 本来は「test_*.py の proc_ai import を proc_formatter 等に変更する」程度の修正で済むはずが、「既存の全 CI 失敗も修正しろ」という指示で修正対象が拡大。

**Why4**: セッションの暴走を止める物理的な仕組みがなく、Dispatch が気づいたときには大量の push 済み
→ 1 セッション 1 PR（またはそれ以下）というルールが物理化されていなかった。merged PR ブランチへの追加 push を物理的に禁止する pre-push hook がない。

**Why5**: 「既存失敗も全部直せ」という指示がセッションのスコープをさらに広げた
→ PO 指示「proc_formatter.py 参照更新」に対して「ついでに他も直せ」という拡張指示が Cowork（LLM） により追加解釈された。セッションの初期スコープと実行スコープの乖離が検出されない。

**仕組み的対策**:
1. **ファイル分割/リネームの PR を出す前に grep 確認を必須化** — `bash scripts/check_file_references.sh <旧ファイル名>` を手動実行し、参照箇所を PR description に明記。Dispatch チェックリストに追加。
2. **セッションへの指示は「このファイルのこの行だけ」レベルまで絞る** — 「CI を直す」「既存失敗も修正」という広い目的指示は禁止。修正対象ファイルと期待される commit diff を明示。
3. **1 セッション 1 PR（またはそれ以下）を物理化** — merged PR ブランチへの後続 push を pre-push hook で reject（T2026-0502-COST-DISCIPLINE に追加・実装済）。
4. **CI 修正セッションへの指示テンプレート化** — [docs/code-session-prompts/ci-fix-template.md](docs/code-session-prompts/ci-fix-template.md) を作成し、「何を修正するか」「修正しないこと」「検証方法」を明示。

**横展開チェックリスト**:
- [ ] ファイル分割を含む PR → 出す前に `grep -r <旧ファイル名> scripts/ .github/ tests/` を実行。参照箇所を PR description に「**Scripts affected**」セクションで列挙し、レビュイーが参照漏れを検出可能に（実装ファイル：出す側が記載責任）
- [ ] 「直せるだけ直す」は禁止。セッション指示に書かれていないファイルは touch しない（Eng Claude 自走ルール違反の一類・教訓 COWORK-WORKING-MD-MISSING の拡張）
- [ ] CI 修正セッション起動時には [docs/code-session-prompts/ci-fix-template.md](docs/code-session-prompts/ci-fix-template.md) をコピペで渡す
