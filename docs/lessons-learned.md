...
- 2026-04-28 08 scheduled-task-protocol とPO前提 §8 の矛盾（保留禁止 vs 効果見えない時は保留 OK）
...
- 2026-04-29 aiGenerated=True なのに keyPoint 充填率 1.3% — title-only 成功で aiGenerated=True が立つ + handler/storage の keyPoint 判定不整合の 2 層原因（T2026-0429-KP）
- 2026-04-29 ランキング age decay フォールバックがフルウェイトに倒れる + PR#34 で hasAI sort 消失 + buildFilters() が空 allTopics で走りジャンル消失 — 3 層原因（T2026-0429-J）
- 2026-04-30 keyPoint ≥100字 充填率が 2.2% で停滞（プロンプト強化のみで止まり length validation/retry/SHORT_FALLBACK が未整備）— 3 層原因（T2026-0430-A）
- 2026-04-30 fetcher の DynamoDB バッチ書き込みが chunk 3/4 失敗で topics 88% 72h+ 停滞（diversityScore に float を入れて Decimal ラップ漏れ）— 4 層原因（T2026-0430-D / PR #46）
- 2026-04-30 perspectives 充填率が 45% で頭打ち（minimal mode 永久 None + mode 別 SLI 分解不在 + 集計 1 値による天井誤読）— 4 層原因（T2026-0430-G / PR #51）
...
- 2026-05-01 Dispatch 違反多発（手動invoke提案・パラメータ適当・実機未確認・効果検証なし・違反無視）
...
3. **(ルール・任意) `global-baseline.md` §8 に「warning→ERROR 昇格 trigger」を追記しない** — PO前提「効果が見えない間は保留」「無理して更新しない」に従い、抽象的な昇格ルールはまだ書かない。本件は具体な観測事故が 1 件確認できた段階で個別に ERROR 化する方針で十分
...
## scheduled-task-protocol とPO前提 §8 の矛盾 (2026-04-28 08:10 制定)

**起きたこと**: 本日 08:10 schedule-task で「運用ルール検討」を求められ過去 commit log を観察したところ、直近の schedule-task が **毎回ほぼ何かを実装** している。`scheduled-task-protocol.md` フェーズ 2 で「最低 1 件以上必ず実装」を強制し、§「保留が許される条件」を 3 件 (PO判断 / core path / 復旧優先) で exhaustive に絞り、「効果が見えない」を保留理由として明示的に禁止しているのが原因。一方 `global-baseline.md §8` は PO前提として「効果が見えない間は保留でかまいません」「無理して更新しない」「念のためルール追加は禁止」を定めている。**両ルールが正面から矛盾し、Claude は protocol 側に従って毎回 CLAUDE.md / global-baseline / docs/rules を肥大化させてきた。**

| Why | 答え |
|---|---|
| **Why1** なぜ矛盾が放置された？ | protocol は「発見偏重 → 実装ゼロ」事故 (2026-04-28 04:15) への過剰反応として書かれ、PO前提 §8 と並べて読まれていない |
...
| **Why5** なぜシグナルが補正されない？ | PO前提 §8 と protocol を整合性チェックする物理ガードが無い。両者を突き合わせて読む頻度が低く、矛盾が「言わずもがな」として温存される |

**仕組み的対策（本 commit で 1 つ実装、2 つは保留と判断）:**

1. **(構造・本 commit で実装) `scheduled-task-protocol.md` §「保留が許される条件」に 1 件追加** — 「unblocked タスクが残っていても、低価値 / 効果事前不可視 / PO前提 §8 と矛盾する施策しか無い場合」を 4 件目として追加。ただし escape hatch 濫用防止のため「保留判断の理由を 3 行以内で報告に書く」を必須化。これでPO前提 §8 と protocol の整合が取れる。
2. **(物理・保留) commit-msg hook に `implemented=0` 時の `reason=hold:<具体>` 必須化** — 即時実装可能だが、現在 `implemented=0` の commit が物理的に存在せず、効果が事前に観測できない。施策 1 を入れた後 1〜2 週間観測し、`implemented=0` commit が複数現れて雑な保留が観測されてから検討する。**PO前提「効果見えない間は保留」を本ルール自身にも適用**。
...
- PO前提 §8「無理して更新しない」を読んだ後でも、protocol §「保留禁止」が脳内で優位になり、**「最小限の実装は何か」と探す方向**にバイアスがかかった
- 結果として「lessons-learned の追記 + protocol の最小修正」を選んだ。これはPO前提に沿うが、**「実装ゼロ」が選択肢として最初から排除されていた**事実は変わらない
...
- PO前提「無理して更新しない」を **新規ルール自身にも適用** することが、ルール肥大化を防ぐ最強の歯止めになる。本件で私が施策 2/3 を保留したのはこの原則の自己適用
...
## 記事数グラフがトピック内記事数と乖離 — fetcher の cnt 水増し（2026-04-28 09 PO報告 T2026-0428-AI）

**起きたこと**:
- PO報告:「記事数のグラフなんかおかしくない？トピック内の記事の数とあってないんだけど」
...
## 空トピック量産 — fetcher が META と detail JSON を非同期生成（2026-04-28 PO緊急報告 T2026-0428-AK）

**起きたこと**:
- PO緊急報告:「空トピックが量産され続けている。ユーザーが離れる原因になっている。即刻根本修正」
- 本番調査結果:
...
- ユーザー目線の SLI (「クリックしたら何か表示される確率」) を verify_effect.sh の物理計測項目に入れていなかったのが Why5 の本質。次はPO報告を待つ前に「ユーザー体験 SLI を追加」する習慣を持つ。
...
**起きたこと**:
- PO報告「空トピックがフロントに並んで品質が悪い」→ 局所修正で fetcher / lifecycle にガードを足した（T2026-0428-AK）
...
5. **セッション多数並走・判断コスト増大** — WORKING.md に 3 件の needs-push:yes が滞留（L / AU / AV）、Cowork 1 件 + Code 2 件が同時走行、POが「止める？再開する？」を判断する往復が頻発
...
8. **機能していないルール（形骸化）** — CLAUDE.md「変更前に副作用確認（声に出す）」「実装前に全体影響マップ必須（POに確認）」が物理ガードを伴わず、buildFilters バグ・about.html ラベル変更時のいずれにも作用しなかった。「POに確認」を Claude が自走モード（feedback_autonomy）で省略するため、テキストルールだけでは動作しない
...
| **Why2** なぜ書いた対策が検証されない？ | landing 後の「**外部から実行され、結果が観測される**」ループが対策ごとに作られていない。書き手（Claude）は書いた瞬間に完了とみなし、次の事象を追う。POは個別事象を通報するが、対策の永続的な実装状態は誰も見ない |
...
| T256 AI フィールド層忘れ CI 検出 | proc_ai 出力 vs handler/proc_storage 双方の突合 | ✅ landing (PR #53, 2026-04-30) | `scripts/check_ai_fields_coverage.py` + `scripts/test_ai_fields_coverage.py` 13 ケース。CI lint-lambda で `proc_ai._normalize_story_result` の Return Dict キーが両層で `gen_story.get('K')`/`gen_story['K']` 参照されているか物理ガード |
...
3. **外部観測（CLAUDE.md フェーズ 1 完了条件の Dispatch 運用化）**: `docs/project-phases.md` フェーズ 1 完了条件に Dispatch 運用項目（コードセッション同時 1 件・並走 0 件・PO判断介入ゼロ）を追加。WORKING.md の `[Code]` 行が 2 件以上ある瞬間を `session_bootstrap.sh` が WARNING で出すゲートは既存だが、本表で「フェーズ 1 完了条件」として明示することで、満たすまで次フェーズへ行かない構造ガードを敷く
...
| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| pre-commit ads.txt 整合 | 2026-04-27 ads.txt 欠落 | `scripts/install_hooks.sh` | ✅ | 実体は heredoc 内。clone 直後 1 回 `bash scripts/install_hooks.sh` で `.git/hooks/pre-commit` を生成 |
| commit-msg Verified 行 | 2026-04-27 完了の定義 | `scripts/install_hooks.sh` | ✅ | 実体は heredoc 内。`.git/hooks/commit-msg` を生成 |
| commit-msg [Schedule-KPI] | 2026-04-28 schedule-task v3 | `scripts/install_hooks.sh` | ✅ | 実体は heredoc 内。6 箇所のうち 1 つ |
...
| AI フィールド層忘れ CI 検出 | T249 keyPoint merge 漏れ | `scripts/check_ai_fields_coverage.py` | ✅ | T256 で landing (PR #53, 2026-04-30)。proc_ai._normalize_story_result の Return Dict キーが proc_storage / handler 双方で参照されているか AST + grep で物理検査。boundary test 13 ケース同梱 |
...
| age decay missing-data フォールバック | 2026-04-29 T2026-0429-J | `projects/P003-news-timeline/frontend/app.js` | ✅ | EXILE_DECAY=0.10 で lastUpdated 欠落と 72h 超を最低ウェイトに |
| age decay 健全性 SLI 観測 | 2026-04-29 T2026-0429-J | `scripts/check_age_decay.sh` / `.github/workflows/freshness-check.yml` | ✅ | 本番 topics.json で missing 率と 上位30件 stale 率を 1h 周期で観測 |
| refreshTopics 後 buildFilters 再実行 | 2026-04-29 T2026-0429-J | `projects/P003-news-timeline/frontend/app.js` | ✅ | データ依存 DOM 操作はデータ確定後に必ず再実行 (ジャンルバー総合固定バグ対策) |
...
| minimal mode cnt-aware perspectives 生成 | 2026-04-30 T2026-0430-G perspectives 45% 天井 | `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0430-G で landing (PR #51 merged 2026-04-30 21:15)。`_build_story_schema(mode, *, cnt)` で minimal + cnt>=2 のとき perspectives のみ schema 追加。tests `tests/test_minimal_perspectives.py` 11 ケース pass |
...
| handler/storage keyPoint 判定統一 | 2026-04-29 keyPoint 充填率 1.3% | `projects/P003-news-timeline/lambda/processor/handler.py` | ✅ | T2026-0429-KP で landing。`_required_full_fields` の keyPoint 判定を `bool()` から `_is_keypoint_inadequate` (100字閾値) に切替。proc_storage と判定基準を物理共有して再生成 skip の不整合を排除。境界値テスト同梱 (`tests/test_keypoint_consistency.py` 19 ケース) |
| ai_succeeded を story 成功にゲート | 2026-04-29 aiGenerated=True で全フィールド空 149件 | `projects/P003-news-timeline/lambda/processor/handler.py` | ✅ | T2026-0429-AISG で landing。`title_succeeded` / `story_succeeded` を独立追跡し `ai_succeeded = story_succeeded` で集約。title-only 成功で `aiGenerated=True` / `aiGeneratedAt` が立つ事故を物理排除 |
| ゴーストID 駆除 + ConsistentRead 検証 | 2026-04-29 14:00 fetcher_trigger 7件指定→0件処理対象 / keyPoint 永久未生成 | `projects/P003-news-timeline/lambda/fetcher/storage.py`, `lambda/fetcher/handler.py`, `lambda/processor/proc_storage.py` | ✅ | T2026-0429-H で landing。fetcher の batch_writer 並列書き込みで silently drop された topicId が saved_ids に残る問題を、① validate_topics_exist の skip_tids 撤廃 + ConsistentRead=True ② ThreadPool f.result() で例外顕在化 ③ processor 側 _drop_ghost_topics() で過去ランからの幽霊も駆除 — の 3 層で物理ガード。tests/test_ghost_topic_cleanup.py 8 ケース + tests/test_stub_meta_guard.py 更新 (全 195 件 pass)。本番効果: total 99→92 / no keyPoint 7→0 / aiCallsUsed 0→4 |
| keyPoint length validation + 1回 retry + SHORT_FALLBACK | 2026-04-30 keyPoint ≥100字 充填率 2.2% 停滞 | `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0430-A で landing。`_retry_short_keypoint` (新アーキ) と `_process_keypoint_quality` を追加し、初回 keyPoint < 100字 のとき 1 回だけ retry → 失敗時は SHORT_FALLBACK (空にしない・長い方を残す)。`keyPointLength` / `keyPointRetried` / `keyPointFallback` を DDB に記録 + `[KP_QUALITY]` ログを CloudWatch に固定フォーマットで出力。境界値テスト 31 ケース同梱 (`tests/test_keypoint_retry.py` rewrite) |
| KP_QUALITY ログによる短文 keyPoint 観測 | 2026-04-30 短文 keyPoint の発生原因が後追い不能 | `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0430-A で landing。`[KP_QUALITY] mode=X cnt=Y topic=Z orig_len=N final_len=M retried=0/1 fallback=0/1 ge100=0/1` の固定フォーマットで CloudWatch Logs Insights から集計可能。後続の verify_keypoint_quality cron が読み込み「retry 後も < 100 字」のトピックを TASKS.md に積む routine の入力にする |
| DDB-bound dict の Decimal ラップ漏れ AST 検査 | 2026-04-30 PR #46 / T2026-0430-D fetcher 停滞 | `scripts/check_decimal_wrap.py` + `scripts/test_decimal_wrap.py` | ✅ | T2026-0430-D で landing。fetcher/processor の Python AST を歩き、Dict literal で value のいずれかが `Decimal(...)` で wrap されている dict を「DDB 行き」と判定。残値に bare float / constant float / unwrapped float-func 呼び出し / Div BinOp が混在すれば CI で物理 fail。再代入トラッキング (`score = apply_time_decay(...)` のあと `score = max(1, int(...))` で safe 化) も実装。`.github/workflows/ci.yml` の lint-lambda job に組み込み済 + 境界値テスト 11 ケース同梱 |
| minimal regime の legacy storyPhase 正規化 | 2026-04-30 storyPhase 発端率 48.6% (ac=2 の 49 件が legacy) | `projects/P003-news-timeline/lambda/processor/proc_storage.py` + `projects/P003-news-timeline/lambda/fetcher/handler.py` | ✅ | T2026-0429-G で landing。T219 (2026-04-28) で minimal mode AI 出力を `phase=None` に変更したが、それ以前に「発端」固定で書き込まれた DDB レコードが永続化していた (AI 再生成は ac>=3 でしか走らない)。`normalize_minimal_phase()` を proc_storage.py に新設し、`generate_topics_card_json` と handler.py の topics_pub 構築で適用。fetcher.handler.py の `_pub` でも同等ロジックを inline で適用 (別 Lambda で proc_storage を import 不能)。境界値テスト 22 ケース同梱 (`tests/test_normalize_minimal_phase.py`)。idempotent・コスト 0・DDB 直接書き換えなし。新記事到来で ac>=3 になれば既存の AI 再生成パスが正しい phase を上書きする |
| topic 個別 JSON empty meta スキップ | 2026-04-28 success-but-empty 横展開 (T2026-0428-Q) ⑦ topic 個別 JSON の meta=2 フィールド | `projects/P003-news-timeline/lambda/processor/proc_storage.py` | ✅ | T260 で landing。`update_topic_s3_file` の merge 後 meta が `aiGenerated=False` かつ `generatedTitle` 不在/空白なら S3 PUT をスキップ + `[SKIP_EMPTY_JSON] tid=... reason=aiGenerated=False` ログ。本番 https://flotopic.com/api/topic/8f81be6586cbea09.json のような 2 フィールドだけの空 JSON 量産を物理排除。DynamoDB レコードは変更しないので将来 AI 生成時には正常に書かれる。境界値テスト 9 ケース同梱 (`tests/test_skip_empty_topic_json.py`) |
| keyPoint retry schema minLength=60 物理ガード | 2026-05-01 keyPoint>=50字 充填率 38.6% 停滞 | `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0501-D で landing。`_retry_short_keypoint` の Tool Use schema を `minLength: 0` → `minLength: _KEYPOINT_RETRY_MIN_CHARS=60` に切替 + description から「空文字は許容するが」「書ける場合は」軟化文言を除去。メイン schema (`_build_story_schema`) の minLength=0 は PO 指示「書けない場合は生成しない」物理化のため維持し、retry のみ強化する設計分離。outlook (60) / perspectives (80) / watchPoints (80) と同じ「Tool Use API がスキーマ違反を再要求 = 物理ガード」方式。境界値テスト 4 ケース同梱 (`tests/test_keypoint_retry.py::RetrySchemaMinLengthTest`)。本番 SLI で keyPoint>=50字 が 38.6%→60%+ に到達するか scheduled task で観測 |
| 海外ニュース 国際/政治 境界キーワード網羅 | 2026-05-01 トピック 1017d101df11d48b (ミャンマー恩赦) が genre='政治' 誤分類 | `projects/P003-news-timeline/lambda/fetcher/config.py`, `projects/P003-news-timeline/lambda/fetcher/text_utils.py`, `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0501-F で landing。fetcher の `GENRE_KEYWORDS['国際']` に米中露欧中東のみだったため「ミャンマー」「スーチー」「ASEAN」が 0 ヒット → 「政府」1 ヒットの政治へ倒れる構造的バグ。ASEAN 諸国 / 南西アジア / 中東広域 / アフリカ / 中南米 / 欧州主要国 / クーデター / 海外要人 を網羅 (~80 語追加)。`GENRE_STRONG_KEYWORDS['国際']` にも ×3 ボーナス語を拡充。`_OVERRIDE_GENRE_RULES` の `'国際'` に「クーデター/軍事政権/ASEAN加盟/ミャンマー/スーチー/ガザ/プーチン/ゼレンスキー/ネタニヤフ/金正恩」追加・`'政治'` から「政府/大臣」相当の海外混同語を排除。AI 側 `_GENRES_PROMPT` の「ジャンル境界の明確化」にも 政治=日本国内 / 国際=海外 の主語境界を例示追加。境界値テスト 8 ケース同梱 (`tests/test_genre_classification.py`) |
| AI 出力 人名・組織略称ガード (情報の地図原則) | 2026-05-01 トピック 1017d101df11d48b keyPoint「スーチー氏」初出が前提知識を要求 | `projects/P003-news-timeline/lambda/processor/proc_ai.py` | ✅ | T2026-0501-F で landing。`_WORD_RULES` (cache 対象 system prompt) に「人名は初出時に肩書き＋正式名称必須」「略称・通称・苗字一語禁止」「組織略号は ASEAN→東南アジア諸国連合 (ASEAN) 形式」「situation/keyPoint は段落単位で初出扱い」ルール追加。`_build_story_schema` の `aiSummary` / `keyPoint` description にも同等制約を inline。「情報の地図」原則 (前提知識を要求した時点で地図ではなくなる) を物理ガードではなくプロンプト思想ルールで反映。Tool Use schema による物理ガードは構造化困難なため、効果は qualitative_eval / 朝 SLI で観測する |
| 収益・PV 週次 SLI 観測 | 2026-04-30 T2026-0430-REV 品質改善 × 収益相関不可視 | `scripts/revenue_check.sh` + `.github/workflows/revenue-sli.yml` + `docs/revenue-log.md` | ✅ | T2026-0430-REV で landing。cf-analytics.json 週間PV / DDB META 件数 / docs/revenue-log.md 最終転記日チェックの3観測を統合。月曜07:00 JST cron で実行し8日以上未転記なら Slack 警告。品質SLI (keyPoint/perspectives 充填率) と収益 (忍者AdMax CTR/CPM/収益¥) の週次相関を手動転記で観測する基盤 |
| cooling トピック lifecycle 再計算 (stale48h SLI 修正) | 2026-05-01 stale48h 39%>30% (T2026-0501-SLI-AGE) | `projects/P003-news-timeline/lambda/fetcher/score_utils.py` + `projects/P003-news-timeline/lambda/fetcher/handler.py` + `projects/P003-news-timeline/tests/test_stale_topic_lifecycle.py` | ✅ | T2026-0501-F2 で landing。`compute_lifecycle_status` が 48h〜7日を velocity 問わず cooling に固定していたため、72h+ 無更新 85 件が topics.json に滞留。修正: 72h 超 + velocity_decayed=0 → archived。さらに handler.py の topics_deduped 構築前に非更新トピックの velocity を apply_velocity_decay で減衰させて lifecycle を再計算 (memory only)。境界値テスト 12 ケース同梱 |
...
- **「POに判断を仰ぐ」は仕組み的対策ではない。** CLAUDE.md「実装前に全体影響マップ必須（POに確認）」は形骸化していた。対策は CI/hook/SLI のいずれかで物理化する。「確認する」「気を付ける」「意識する」は本ファイルの仕組み的対策セクションに書かない（global-baseline §6）
...
## aiGenerated=True なのに keyPoint 充填率 1.3% — 2 層原因（2026-04-29 制定・T2026-0429-KP）

**事象**: 本番 DDB `p003-topics` で `aiGenerated=True` のレコードが 536件あるが、`keyPoint` が schema 要件 (200〜300字) を満たすのは 7件のみ (1.3%)。529件が滞留。
内訳:
- 149件: 全 AI フィールド (storyPhase / statusLabel / perspectives / outlook) が空。`generatedTitle` と `generatedSummary` だけ埋まっている
- 280件: storyPhase 等は埋まっているが `keyPoint` 空 (旧スキーマ時代の処理結果)
- 38件: 全フィールド埋まっているが `keyPoint` が < 100字 (短いタイトル風 — 例: 「ロシア撤退と過激派攻勢の同時進行が新段階へ」21字)

### なぜなぜ（独立した 2 層原因）

**Layer A — title-only 成功で aiGenerated=True が立つ (149件の根本原因)**

- **Why1**: `aiGenerated=True` なのに storyPhase まで含めて全フィールド空のレコードがある
- **Why2**: `update_topic_with_ai` は `ai_succeeded=True` で `aiGenerated=True` を書くが、`gen_story=None` のときは story フィールドを 1 つも書かない (条件 `if gen_story:` で全部スキップ)
- **Why3**: `handler.py` で `ai_succeeded` は title 生成成功と story 生成成功の両方で `True` に立つ単一フラグ。title-only 成功 + story API エラー (5xx / timeout) のとき、`ai_succeeded=True` のまま `gen_story=None` で `update_topic_with_ai` が呼ばれる
- **Why4**: `ai_succeeded` の意味論が「Claude が何か成功した」と曖昧。「AI 要約として保存可能なフィールドが揃った」 (= story 生成成功) という storage 側の意図と乖離していた
- **Why5**: 「flag は 1 つで十分」という素朴な構造選択が、後から成功条件が複合化したときに対応できなかった。state を分離しなかった

**Layer B — handler/storage で keyPoint の充足判定が不整合 (38件の根本原因)**

- **Why1**: `proc_storage._is_keypoint_inadequate` は < 100字を不十分と判定し再生成キューに乗せるが、トピックは何度処理しても短いまま
- **Why2**: `handler.py` の `_required_full_fields` は keyPoint について `bool(topic.get('keyPoint'))` 判定。21字でも `True` になり「充足済」扱いで `needs_story=False` → generate_story を call せず skip
- **Why3**: 「proc_storage が再処理キューに乗せる条件」と「handler が実際に再処理する条件」が独立コードで実装され、判定基準が ずれていた
- **Why4**: 充足判定ロジックが 2 箇所にコピーされ、片方 (`KEYPOINT_MIN_LENGTH=100`) を後から追加したときに もう片方 (handler の `bool()`) を更新し忘れた
- **Why5**: 共通の判定関数 (`_is_keypoint_inadequate`) を作っておきながら、handler から import せず inline ロジックで書いていた。「同じ意味の判定は 1 関数に集約」という基本原則が守られていなかった

### 仕組み的対策（最低 3 つ・物理ゲート / 観測 を含む）

1. **物理ゲート — handler/storage 判定統一 (Layer B 対策)**: `handler.py` で `from proc_storage import _is_keypoint_inadequate` し、`_required_full_fields` の keyPoint 判定を `(not _is_keypoint_inadequate(topic.get('keyPoint')))` に置換。判定関数を物理的に 1 箇所に集約し、閾値変更が両方に伝搬する構造に変える。回帰防止のため `tests/test_keypoint_consistency.py` を新設し「proc_storage が再処理キューに載せた topic を handler が skip しないこと」を 4 ケースで assert (空 / None / 短い / 充分)
2. **物理ゲート — `ai_succeeded` を story 成功にゲート (Layer A 対策)**: `handler.py` で `title_succeeded` / `story_succeeded` を独立に追跡し、`ai_succeeded = story_succeeded` で集約する形に refactor。`update_topic_with_ai` の `aiGenerated=True` / `aiGeneratedAt` 更新は story 生成成功時のみ走る。title-only 成功時は `gen_title` だけが書き込まれ、`aiGenerated` は False のまま → 次回 invocation で再処理対象に含まれる。回帰防止のため `tests/test_keypoint_consistency.py` の `AiSucceededSplitTest` で 4 ケース (title-only / story-only / both / neither) を assert
3. **境界値テスト同梱 (Layer A+B 対策)**: 新設した `_is_keypoint_inadequate` の境界値 (None / 空文字 / whitespace / 99字 / 100字 / 長文) を `IsKeyPointInadequateTest` で 7 ケース assert。CLAUDE.md「新規 formatter は boundary test 同梱」を判定関数にも適用

### 横展開チェックリスト

本セッションで 横展開チェックリスト 表に 2 行追加 (handler/storage keyPoint 判定統一・ai_succeeded を story 成功にゲート)。CI `check_lessons_landings.sh` が実装ファイルの存在を物理検証する。

### メタ教訓

- **「同じ意味の判定」を 2 箇所に書くと閾値変更時にズレる。** 共通ヘルパー関数を作ったら必ず import で参照する。inline コピーは「一見 1 行で済む」誘惑があるが、後で必ず分岐するロジックの burden になる
- **複合化した成功条件を単一フラグで表現してはいけない。** `ai_succeeded` のように「何かが成功した」抽象 flag は、後から成功条件が増えると意味が壊れる。最初から `title_succeeded` / `story_succeeded` のように成功源ごとに分け、合成は明示的に書く
- **DDB の充填率はコードを読まずに数えれば一発で分かる。** 1.3% という極端な値は、コード読みより先にスキャンして Counter で内訳を出すべきだった (本セッションでは最初に scan で内訳を出し、パターンから根本原因に当たれた)

---

## ランキングの age decay が効かず古いトピックが上位に出る — 3 層原因（2026-04-29 制定・T2026-0429-J）

**事象**: フロントの app.js でランキング sort を行っているが、24h〜72h 経った古いトピックが上位 10 件に大量混入。さらに PR#34 で「hasAI=true (AI解説あり) を最優先」のロジックが消失。さらに `buildFilters()` が初回 `allTopics=[]` のときに走り、ジャンルバーが「総合」のみで固定される事故も同時発生。

### なぜなぜ（独立した 3 層原因）

**Layer A — age decay フォールバックが「フルウェイト」に倒れる (古いトピック上位の根本原因)**

- **Why1**: 24h〜72h 経った topic が score を decay されないまま上位に出る
- **Why2**: `ageDecay(t)` は `lastUpdated` パース失敗時に `return 1.0` していた (`if (age <= 0 || !toUnixSec(t.lastUpdated)) return 1.0`)
- **Why3**: フォールバック値の選択が「データがないから安全側でフルウェイト」という発想だった。が、本来は逆 — 「データがないなら信用できないから最低ウェイト」が正
- **Why4**: 「missing data の扱い」を「中央値」「平均」「フルウェイト」のどれにするか選ぶ際に「ペナルティ方向に倒す」という ranking の基本原則を忘れた
- **Why5**: 境界値テスト（lastUpdated=null / "" / "invalid" / 未来日付 / 0）を書かなかった。書いていれば「null だと return 1.0 になる」と即気付く

**Layer B — hasAI sort が PR#34 で消失（リグレッション）**

- **Why1**: PR#34 (perf: コスト削減 + 要約なしトピック下位固定) で `hasAI` ソートが除去された
- **Why2**: PR レビュー時に「hasAI 優先 → 要約なし下位固定」が同義に見えたが、実際は「hasAI=true を最優先」+ 「decay/score で並び替え」という 2 段ソート構造が壊れた
- **Why3**: ranking ロジックの sort 順番を変える PR で「ソート規則の固定」テストを書いていなかった (sort の前後を assert する snapshot test が無い)
- **Why4**: app.js の sort は inline arrow function で、「ソート規則を読む唯一の場所」がコード本体しかない。意図 (= AI 解説の濃度を最優先) が doc 化されておらず、PR で消えても誰も気付けない
- **Why5**: 「ranking 規則は仕様」という認識が欠けていた。仕様 = テストで固定するもの、なのにソートロジックには test が無かった

**Layer C — buildFilters() が allTopics=[] のときに走り、ジャンルバーが「総合のみ」で固定される**

- **Why1**: 初回ロードで「総合」しかジャンルボタンが表示されない（テクノロジー / 経済 / 政治等が消える）
- **Why2**: `BuildFilters.computeVisibleGenres(allTopics, currentGenre, GENRES)` が空配列を受け取ると `counts={}` 経由で `visible=['総合']` を返す（仕様通り）
- **Why3**: `buildFilters()` は DOMContentLoaded 直後の line 1476 で実行されるが、その時点で `refreshTopics()` はまだ走っていない → `allTopics=[]`
- **Why4**: `refreshTopics()` 完了後に `buildFilters()` が呼び直されない設計だった (`renderTopics()` だけ呼ばれる)。「DOM 描画」と「フィルタ可視リスト計算」が一つの画面でどちらも `allTopics` に依存するのに片方しか refresh していない
- **Why5**: 「ロード前提のロジックをロード前に呼ぶリスク」を init 順序の review で見落とした。`buildFilters()` は副作用ありの DOM 操作なので、データ依存箇所はデータ確定後に呼ぶのが自然

### 仕組み的対策（最低 3 つ・物理ゲート / 観測 を含む）

1. **物理ゲート — age decay フォールバックを「最低ウェイト」に倒す (Layer A)**: `app.js` で `EXILE_DECAY=0.10` を導入し、lastUpdated 欠落 / パース失敗時はこれを返す。さらに 72h 超も同値で強制 exile。`scripts/check_age_decay.sh` を新設し、本番 topics.json を fetch して「lastUpdated 欠落率 > 5%」「上位 30 件の 24h 超比率 > 50%」を観測 → freshness-check workflow に組込み、CI 実行ごとに job summary に書き出す
2. **観測 — age decay 健全性を SLI に昇格 (Layer A)**: `.github/workflows/freshness-check.yml` に「age decay 健全性 (T2026-0429-J)」step を追加。1h おきの cron で `bash scripts/check_age_decay.sh` を実行し、閾値超で `::warning::` を出す。Slack 通知は出さない (ノイズ過多回避) — Actions UI と Step Summary で気付ける構造に
3. **物理ゲート — refreshTopics() 後の buildFilters() 再実行 (Layer C)**: `app.js#refreshTopics` で `allTopics` を populate した直後に `buildFilters()` を呼び直す。「データ依存の DOM 操作はデータ確定後に必ず再実行」という単純な不変条件を物理化

### 横展開チェックリスト

本セッションで 横展開チェックリスト 表に 3 行追加 (age decay フォールバック修正・check_age_decay.sh の SLI 化・refreshTopics 後 buildFilters 再実行)。CI `check_lessons_landings.sh` が実装ファイルの存在を物理検証する。

### メタ教訓

- **missing data のフォールバックは「ペナルティ方向」が default。** ranking / scoring 系で `null → 安全側でフルウェイト` は事故の元。「データがないなら信用できない」 = 最低ウェイトが ranking の基本原則
- **sort 順序は仕様。** 「動いていれば良い」ではなく snapshot test / unit test で固定する。PR で順序が変わったときに diff で気付ける構造を作る
- **DOM 描画とデータロードは順序に依存する。** init で「全部初期化 → データロード」と書きたくなるが、データ依存の DOM 操作はデータ確定後に必ず再実行する。`buildFilters()` のような関数は内部で「データ未着なら何もしない」も検討に値する

---

## keyPoint ≥100字 充填率 2.2% 停滞 — 3 層原因（2026-04-30 制定・T2026-0430-A）

**事象**: 本番 SLI (topics.json updatedAt 2026-04-30 18:05 JST 観測) で `aiGenerated=True` 母集団の keyPoint ≥100 字充填率が **2.2% (2/92)**、平均長 **35.6 字** で停滞。T2026-0429-J で「コスト抑制のため retry 撤去 → プロンプト強化のみで品質改善を狙う」方針に転換したが、processor 1 サイクル経過後も数値が動かなかった。

直前のタスク群:
- T2026-0428-BH: `KEYPOINT_MIN_LENGTH=100` 物理ガード追加（DDB 上書き許可・再処理キュー投入の判定）
- T2026-0429-KP: handler/storage 判定統一（`_is_keypoint_inadequate` 共通化）
- T2026-0429-J: retry 撤去 + worked example 差替え + schema minLength=0 で空文字許容
- T-keypoint-prompt (2026-04-30): フェーズ判定方式（記事数で書き方を分岐）に転換

これだけ手を入れても充填率が動かない = プロンプト strategy 単独では LLM の応答揺らぎを抑制できないことが確定。

### なぜなぜ（独立した 3 層原因）

**Layer A — プロンプト強化単独では LLM の「短く返す」癖を抑えられない（充填率停滞の根本原因）**

- **Why1**: keyPoint プロンプトに「100 字以上必須・worked example・フェーズ判定」を明記したのに、本番応答の 97.8% が < 100 字
- **Why2**: Tool Use API の schema minLength=0 が「書けない場合は空文字」のエスケープを許容しており、LLM はこれを「短くして良い理由」と解釈して使う
- **Why3**: T2026-0429-J で「コスト 2x 増を許容しない」前提で retry を撤去したが、撤去前の状態（自然に短文が出続ける）を観測してから判断していなかった。「強いプロンプトで品質を上げる」が原則として正しくても、LLM が schema escape を活用する性質に勝てなかった
- **Why4**: 「プロンプト改善で品質を上げる」と「プロンプト + 後処理 (retry / validation) で品質を上げる」は別戦略。前者だけで終わらせず、後者を併用する判断を遅らせた
- **Why5**: 「コストを増やさない」が暗黙の優先制約として存在し、品質改善の選択肢を狭めていた。本来は「許容コスト + 品質目標」を先に決めて選択肢を比較すべきだった

**Layer B — length validation がコード側に無く、短文応答を素通りさせた（観測ギャップ）**

- **Why1**: 初回応答が 30 字でも、proc_ai.py は `[WARN] keypoint_short` を 1 行 print するだけで、応答を「成功」として保存していた
- **Why2**: schema validation は `minLength=0` で空文字を許容し、Tool Use API が短文を返しても tool_use レスポンスとして valid。サーバ側 (proc_ai) で別途 length check を行わなかった
- **Why3**: 「schema は valid だから保存できる」と「品質要件を満たしたから保存すべき」を分離していなかった。schema = 形式契約、length = 品質契約 という二層構造が必要だった
- **Why4**: WARN ログを出すだけで失敗扱いにせず、再生成も呼ばなかった。「気付くこと」と「対処すること」を別レベルで実装する発想が欠けていた
- **Why5**: 過去の T2026-0429-J で retry 撤去時に「短文時の挙動」を「ログだけ出す」で固定したが、これは仕様ではなく単なる「現状維持の妥協」だった

**Layer C — 短文発生の品質メタが DDB に保存されず、後追い分析が不能（再発防止の盲点）**

- **Why1**: 本番で短文 keyPoint が発生してもログを CloudWatch から集計するしかなく、DDB スキャンでは「どのトピックが retry されたか・なぜ短いままか」を判別できなかった
- **Why2**: keyPoint だけ DDB に保存しており、品質メタ（length / retried / fallback）を保存していなかった
- **Why3**: 「品質改善 = keyPoint 本体を改善」とのみ捉え、「改善過程の観測データを残す」が後回しになった
- **Why4**: 後で「短文が出るパターンの分析」をしようとしたとき、CloudWatch から collected な事実 (mode / cnt / orig_len / final_len) を再構成する手間が大きく、分析が始まらなかった
- **Why5**: 「データを残せば後で分析できる」という習慣が、品質メトリクスでは確立されていなかった (Verified-Effect 行で SLI を残す習慣はあるが、トピック単位のメタデータは抜けていた)

### 仕組み的対策（最低 3 つ・物理ゲート / 観測 を含む）

1. **物理ゲート — `_retry_short_keypoint` を新アーキで再導入 (Layer A 対策)**: `proc_ai.py` に `_retry_short_keypoint(articles, cnt, mode, original_keypoint)` を追加し、初回 keyPoint < 100 字のとき 1 回だけ Claude を再呼出する。retry プロンプトは「前回が短すぎたので最低 100 字以上、具体数字・固有名詞を含めて拡張」を明示し、keyPoint だけ生成する縮小スキーマ + max_tokens=400 でコスト最小化。system prompt は同一文字列で cache hit 維持。コスト試算: pendingAI 30〜50 件/サイクル × 2 サイクル/日 × ~98% 短文 ≈ +100 retry call/日 × ~$0.001 = +$0.10/日 ≒ +$3/月 で許容範囲。回帰防止: `tests/test_keypoint_retry.py` を rewrite し、短文時 2 call / 長文時 1 call / retry 失敗時 fallback フラグ立て / keyPoint を空にしない を全モード (minimal/standard/full) で 31 ケース assert
2. **物理ゲート — length validation + SHORT_FALLBACK フラグ + DDB 永続化 (Layer B+C 対策)**: `proc_ai.py` に `_process_keypoint_quality(result, articles, cnt, mode)` を新設し、length validation → 必要に応じ retry → 結果に応じて `_kpRetried` / `_kpFallback` / `_kpLength` を内部フラグとして立てる。`_normalize_story_result` で `keyPointLength` / `keyPointRetried` / `keyPointFallback` を output dict に含め、`proc_storage.update_topic_with_ai` で DDB に書き込む（keyPoint と同時更新で incremental 整合性を保つ）。retry も短い場合は `keyPointFallback=True` を立てるが keyPoint 自体は空にせず、original / retry の長い方を残す（情報量保持）。これにより「短文が発生した理由・retry の効果」を DDB スキャン 1 発で観測可能にする
3. **観測 — `[KP_QUALITY]` 固定フォーマットログ + 横展開チェックリスト追記 (Layer C 対策)**: CloudWatch Logs に `[KP_QUALITY] mode=X cnt=Y topic=Z orig_len=N final_len=M retried=0/1 fallback=0/1 ge100=0/1` を 1 トピック 1 行で出力。後続の `verify_keypoint_quality` cron (タスク化) が CloudWatch Logs Insights から集計し「retry 後も < 100 字」のトピックを TASKS.md に積む routine の入力にする。本セッションで横展開チェックリスト 表に 2 行追加（keyPoint length validation + 1回 retry + SHORT_FALLBACK / KP_QUALITY ログ）し、CI `check_lessons_landings.sh` が proc_ai.py の存在を物理検証する

### 横展開チェックリスト

本セッションで 横展開チェックリスト 表に 2 行追加（keyPoint length validation + 1回 retry + SHORT_FALLBACK / KP_QUALITY ログによる短文 keyPoint 観測）。CI `check_lessons_landings.sh` が `projects/P003-news-timeline/lambda/processor/proc_ai.py` の存在を物理検証する。

### メタ教訓

- **「プロンプトで品質を上げる」と「プロンプト + 後処理で品質を上げる」は別戦略。** プロンプト改善が頭打ちになった時点で後処理 (validation / retry / fallback) を併用する判断を遅らせない。コスト制約は「許容コスト + 品質目標」を先に決めて選択肢を比較する形で管理する
- **schema valid と品質要件は別レイヤー。** Tool Use API の schema validation は形式契約であって品質契約ではない。サーバ側で品質要件 (length / 内容) を独立に検証し、不足時は再生成 / フラグ立て / フォールバックの選択肢を持つ構造にする
- **WARN ログだけは「気付くこと」止まり。** 観測 = 気付く + 対処する の二段構造で実装する。「ログを出すだけ」は妥協であって仕様ではない。仕様化するなら ① 何が起きたら ② どう対処するか を明示する
- **改善過程のメタデータは DDB に残す。** 「結果」だけでなく「プロセスの観測データ」(retry 回数・fallback 発動率・初回 vs 最終長さ) を保存し、後で分析可能にする。1 行の DDB フィールド追加コストより、後追い分析の手間を防ぐ価値が大きい
- **過去の判断を再評価する勇気。** T2026-0429-J で「retry 撤去」した判断は「コスト抑制」という当時の優先制約には正しかったが、本番 SLI が動かない事実を見て再評価する。「過去の判断を変えない一貫性」より「観測事実に基づいて方針を更新する誠実さ」を優先する

---

## fetcher の DynamoDB バッチ書き込みが chunk 3/4 失敗で topics 88% 72h+ 停滞 — 4 層原因（2026-04-30 制定・T2026-0430-D / PR #46）

**事象**: 本番 flotopic.com で「topics の 88% が 72h 以上前から更新されていない」と外形観測。fetcher Lambda は EventBridge cron で正常に invoke され完走していたが、CloudWatch Logs に以下が固定フォーマットで出続けていた:

```
[dynamo-batch] chunk write 失敗: Float types are not supported. Use Decimal types instead.
[dynamo-batch] WARN: 3/4 chunks 失敗
[ghost] saved_ids から DDB 不在の 37件を除去
```

実損: 1 サイクルあたり新規/更新トピック ~159 件が DDB に保存できず、saved_ids にだけ残ってゴースト化 → processor がそれら topicId を再処理しに行っても DDB 不在で skip → keyPoint も spreadReason も生成されず → topics.json で全部古いトピックが上位を占有。

**事故の根本コミット**: 2026-04-27 `59eaf649` で META item に `'diversityScore': _div_mult` (= float) を追加した行。velocity / velocityScore は同じコミット内ですら `Decimal(str(...))` でラップされていたのに、`_div_mult` だけ規約から漏れた。

### なぜなぜ（独立した 4 層原因）

**Layer A — Decimal ラップ規約の物理ガード不在（バグ混入の根本）**

- **Why1**: `_div_mult` (float) を `Decimal(str(...))` でラップせず DDB に渡したのに、commit 時点で誰も気付かなかった
- **Why2**: 隣の行の `velocity` / `velocityScore` は規約通りラップされており、コミット差分を見れば「diversityScore だけ規約逸脱」と一目瞭然なはずだったが、PR レビューで見逃された
- **Why3**: 「DDB に渡す数値は必ず Decimal」という規約は思想ルールでしか書かれておらず、CI/hook の物理ガードが無かった。pre-commit / CI の static analysis に「DDB-bound dict literal の float 検出」が組まれていなかった
- **Why4**: 過去にも同種の事故 (Float types are not supported) を踏んだはずだが、再発防止が「commit 時に気を付ける」という思想ルール止まりで、AST ベースの構造検査まで到達していなかった
- **Why5**: 「規約が確立 = チーム全員が守る」と暗黙に信じていた。LLM (Claude) が新規フィールドを追加する際、隣接フィールドのパターンを 100% 真似しない揺らぎを構造的に防ぐ仕組みが無かった

**Layer B — DDB 書き込み失敗が ERROR にならず WARN で素通り（観測ギャップ）**

- **Why1**: `[dynamo-batch] WARN: 3/4 chunks 失敗` が出続けていたのに、CloudWatch アラームも Slack 通知も発火しなかった
- **Why2**: chunk write 失敗のログレベルが `[WARN]` で、Lambda の return 値も成功扱い。SLI 集計対象に「DDB write failure rate」が含まれていなかった
- **Why3**: `freshness-check` SLI 1 (updatedAt) は本番 topics.json の `lastUpdated` を見るが、「古いトピックが上位を占有して全体の中央 lastUpdated は新しく見える」状態を救えなかった (= ranking 上位の古さは検知するが、`新規記事が DDB に届いているか` を直接観測する SLI が無い)
- **Why4**: 「fetcher が完走した = データが入った」と暗黙視。実際は「fetcher が完走 + chunk write が成功した」が真の完了条件。両者のギャップを埋める SLI を作っていなかった
- **Why5**: SLI を増やすと運用負荷が増えるという感覚 (true-positive を逃す代わりに false-positive も減らせる) で、`new_topics_persisted_per_run` のような根本指標を後回しにしていた

**Layer C — boto3 batch_writer の例外が ThreadPoolExecutor で silently drop（黙殺ループ）**

- **Why1**: chunk write 失敗が連続したのに fetcher が完走 ext code 0 で終わった
- **Why2**: T2026-0429-H で fetcher の batch_writer 並列書き込みを ThreadPool に分割したが、`f.result()` での再 raise を一部経路で取り損ねていた (PR #46 と並行で修正済)
- **Why3**: 「並列化で速度を稼ぐ」と「失敗を観測する」が同時に成立しないコード設計だった。失敗が future に閉じ込められたまま catch されず、メイン thread に伝播しなかった
- **Why4**: ThreadPool 使用時の例外伝播はベストプラクティスがあるが、ad-hoc 並列化で省略された
- **Why5**: 並列化 PR のレビュー時に「失敗時の挙動」を確認するチェックリストが無かった。「速くなる」だけを成功条件にして、「失敗が観測可能」を成功条件に含めなかった

**Layer D — saved_ids ゴースト残留で処理対象が幻のまま増殖（自己治癒不全）**

- **Why1**: chunk write 失敗で DDB 不在のはずの topicId が saved_ids に大量に残り、次回 processor が対象として拾うがいつまでも処理できない
- **Why2**: saved_ids は「fetcher が保存しようとした topicId のリスト」であって「保存に成功した topicId」ではないが、後続コードはそれを暗黙に「保存済」と扱った
- **Why3**: 「書き込みリクエストを発行 = 書き込み成功」と扱う経路と、「ConsistentRead で実在を確認してから処理対象にする」経路が混在していた
- **Why4**: T2026-0429-H で `_drop_ghost_topics()` を導入したが、ゴーストを駆除した後の re-fetch 流路までは閉じていなかった (= ゴースト除去後、その topicId はもう新着扱いされない)
- **Why5**: 「失敗を黙殺しない」設計と「失敗から自己回復する」設計の両方が必要だが、後者は事象が起きてから後追い実装になりがちだった

### 仕組み的対策（最低 3 つ・物理ゲート / 観測 を含む）

1. **物理ゲート — `scripts/check_decimal_wrap.py` で AST 検査 + CI 必須化 (Layer A 対策)**: `lambda/fetcher/handler.py` `lambda/fetcher/storage.py` `lambda/processor/handler.py` `lambda/processor/proc_storage.py` `lambda/processor/proc_ai.py` の各 Python AST を歩き、Dict literal で value のいずれかが `Decimal(...)` で wrap されている dict を「DDB 行き」と判定。残りの value に bare float (関数スコープで `source_diversity_score` / `calc_velocity` / `apply_time_decay` 等の float-returning 関数から代入された Name)・constant float リテラル・unwrapped float-func 呼び出し・unwrapped Div BinOp が混在していれば CI で物理 fail。再代入トラッキング (例: `score = apply_time_decay(...)` のあと `score = max(1, int(...))` が来れば safe) も実装し、現状コードは全 5 ファイル pass。`.github/workflows/ci.yml` の lint-lambda job に組み込み、PR で reject される構造にする。境界値テスト同梱: `scripts/test_decimal_wrap.py` で 11 ケース (PR #46 の実バグ pattern / 正しいラップ / int 再代入 / literal float / unwrapped function call / Div BinOp / Decimal 不在 dict / int wrap / nested list-dict / str constant / 実 lambda コード回帰) を assert
2. **観測 — `[dynamo-batch]` ログを CloudWatch Logs Insights で集計 + freshness-check SLI に "新規 topic 永続化率" 追加 (Layer B 対策)**: 既存の `[dynamo-batch] chunk write 失敗` / `[dynamo-batch] WARN: N/M chunks 失敗` ログを Logs Insights で日次集計し、`freshness-check.yml` workflow 内で `chunk_failure_rate > 5%` 検出 step を追加する。さらに `new_topics_persisted_per_run` (= 直近 fetcher run で saved_ids に追加された topicId のうち DDB に実在する件数 / saved_ids 追加件数) を SLI 12 として導入し、< 90% で `::error::` を出して main 通知を発火させる。本セクションで横展開チェックリスト表に 1 行追加し landing を物理検証
3. **物理ゲート — boto3 batch_writer 失敗を ThreadPool で必ず再 raise (Layer C 対策・既存 T2026-0429-H の補強)**: fetcher/handler.py 内の並列 batch_writer 呼び出し全箇所で `f.result()` を呼び切り、未消費 future を ThreadPool に閉じ込めない構造を unit test で固定。`tests/test_ghost_topic_cleanup.py` の延長で `tests/test_batch_writer_exception_propagation.py` を新設し、batch_writer が `ClientError` を投げたとき lambda_handler が ext code != 0 で終了する (= EventBridge / CloudWatch で alarm が発火する) ことを assert (※ 次タスクで実装、本タスクではログ観測 SLI で代替)

### 横展開チェックリスト

本セッションで 横展開チェックリスト 表に 1 行追加 (`scripts/check_decimal_wrap.py` + `scripts/test_decimal_wrap.py`)。CI `check_lessons_landings.sh` が `scripts/check_decimal_wrap.py` の存在を物理検証する。

### メタ教訓

- **「規約が確立されている = 全員が守る」は LLM 時代には成り立たない。** Claude が新規フィールドを追加するたびに隣接パターンを 100% 真似する保証は無い。規約は CI/hook の AST 検査まで物理化して初めて「守られる」状態になる。思想ルールは LLM に対しては「気を付ける」と同等の強度しか持たない (global-baseline §6 の通り)
- **WARN ログは「気付かない」ことの言い換え。** chunk write 失敗が WARN で出続けても、SLI/アラームに紐付かなければ存在しないのと同じ。「失敗の発生」と「失敗の観測」は別レベル。観測まで含めて初めて再発防止と呼べる
- **完走 ≠ 成功。** Lambda が ext code 0 で終わっても「やるべき仕事が全部終わった」とは限らない。並列化導入時は「失敗が main thread に伝播するか」「失敗時の終了コード」「失敗時の SLI 反映」を全部チェックリスト化する
- **fetcher の本質は I/O 成功率。** 何をフェッチして何を保存できたか、を直接測る SLI が「fetcher が完走したか」より上位の指標。今後は `new_topics_persisted_per_run` を fetcher の primary SLI として運用する
- **隣の行をコピーするだけで防げる事故ほど CI で物理化する価値が高い。** PR diff を見て「隣 1 行と違う」が一目で分かるバグは、AST diff でも一発検出できる。「人間がレビューで気付く」より「CI で reject」が速くて安い

---

## perspectives 充填率が 45% で頭打ち — mode 切替の閾値で品質指標が天井を作っていた（2026-04-30 制定・T2026-0430-G / PR #51）

**事象**: 本番 flotopic.com の verify_effect.sh ai_quality 観測で perspectives 充填率が 45/100 = 45.0% に張り付き、目標 60% に到達しなかった。SLI 履歴は 4.31% (4/28) → 40.2% (4/30 07:10) → 45.0% (4/30 21:00) と段階改善しているように見えたが、実は **mode 別に分解測定すると以下のように完全に二極化** していた:

```
mode=full:     16/16  (100%) — 全件 perspectives 生成成功
mode=standard: 29/29  (100%) — 全件 perspectives 生成成功
mode=minimal:   0/55    (0%) — 全件 perspectives=None 強制
```

minimal mode (articleCount<=2) は「perspectives は記事 1〜2 件では差分が出ない」として `perspectives=None` を強制していたため、minimal が aiGenerated 母集団の 55% を占める構造下では perspectives 充填率の上限が **構造的に 45%** に天井設定されていた。49/55 の minimal 件は ac=2 + uniqueSourceCount=2 で、2 媒体の論調差を表現する素材は十分あったにも関わらず。

### なぜなぜ（独立した 4 層原因）

**Layer A — mode 別 SLI 分解の不在（観測ギャップ）**

- **Why1**: perspectives 充填率は集計値しか観測しておらず「45% で停滞している」としか分からなかった
- **Why2**: verify_effect.sh ai_quality は `aiGenerated=True 全体 ÷ perspectives 非空件数` の単純平均しか計測していない。mode (minimal/standard/full) で割って分布を出すロジックを作っていなかった
- **Why3**: 「集計 SLI は単純平均」という暗黙合意があり、母集団の構造的偏りを発見する SLI を別途持たなかった
- **Why4**: T2026-0428-Y で perspectives>=60% を目標値に設定した際、母集団の前提 (=どの mode で生成されているか) を SLI 設計に書き込まなかった
- **Why5**: 「品質指標は連続値」と暗黙に思い込んでいた。実際には mode が discrete に切り替わる構造下では、指標が連続的に向上することはなく、mode 別に別々の階段を登る挙動になる。最大値が決まる SLI は連続最大化の対象にならない

**Layer B — mode 切替閾値が品質指標の天井を決めていた（設計ミス）**

- **Why1**: minimal mode (cnt<=2) が `perspectives=None` を強制しており、aiGenerated 母集団の 55% に永久に 0 を返す構造だった
- **Why2**: minimal mode の設計理念「記事 1〜2 件では差分が出ない」が、cnt=2 で 2 媒体ある場合にも一律適用されていた。1 媒体しかない cnt=1 と、2 媒体ある cnt=2 を同一視するのが間違い
- **Why3**: mode 設計時 (T2026-0428-J) は「コスト最小化」を主目的にしており、品質指標 (perspectives 充填率) との両立を後追いで検討していなかった。「コスト境界」と「品質生成境界」が同じ閾値 (cnt<=2) で混ざっていた
- **Why4**: 後段の verify_effect.sh が「aiGenerated 全体に対する充填率」を計測する設計になっていたため、minimal mode の永久 None が指標の天井として現れることを SLI 側で検出できなかった
- **Why5**: 「ある時点の局所最適」と「全体最適」を分けて扱う設計判断ができていなかった。コスト最適化のために mode 分割をしたのに、その mode 分割が品質 SLI の天井を作るとは設計時に予測していなかった

**Layer C — SLI が一定値で停滞している事象を「進捗あり」と誤読（観測解釈ミス）**

- **Why1**: 4/28 4.31% → 4/30 07:10 40.2% → 4/30 21:00 45.0% という変動を「順調に向上中」と解釈し、minimal mode の天井に張り付いていることに気付かなかった
- **Why2**: 観測タイミングが粗い (1 日 1〜2 回) ため、過渡的な数値変動なのか天井張り付きなのかを区別する観測点が不足していた
- **Why3**: 「目標 60% に向かう進捗」というナラティブ前提で SLI を読んでおり、「天井に張り付いている可能性」という対立仮説を立てていなかった
- **Why4**: SLI が改善している時に「なぜ改善しているか」「改善は持続するか」を問わなかった。原因が外部要因 (T2026-0429 系副作用) で偶発的だった可能性を排除できていなかった
- **Why5**: 観測値の変動を見たら「上限と下限を推定する」「持続性を問う」「分母を分解する」の 3 段思考を踏むべきだったが、トレンドラインを引いて延長線上で目標到達を予測する 1 段思考で済ませていた

**Layer D — minimal mode の存在自体を SLI denominator から除外する選択肢を検討しなかった（測定設計ミス）**

- **Why1**: 「perspectives 充填率」を本番計測する際、minimal mode のように構造的に perspectives を出さない群を分母に含めるかは設計判断だが、暗黙に含める運用をしていた
- **Why2**: 「perspectives は全 aiGenerated topic で生成すべき」という暗黙仕様と、minimal mode 設計の「perspectives は cnt<=2 では生成しない」という実装仕様が矛盾していたが、矛盾自体が発見されなかった
- **Why3**: SLI 設計時に「分母をどう定義するか」を docs/sli-slo.md に明文化していなかった。"aiGenerated 全体" としか書かれておらず、minimal/standard/full の生成有無を区別できなかった
- **Why4**: 仕様の矛盾を検出するには「実装仕様」「期待仕様」「SLI 仕様」の三つ巴比較が必要だが、これを定期的に走らせる仕組み (SLI 仕様レビューや CI 比較) が無かった
- **Why5**: SLI を「現状観測の道具」としてのみ扱い、「仕様の整合性チェッカー」としての役割を活かしていなかった。SLI 値が天井に張り付くのは仕様矛盾のシグナルでもある

### 仕組み的対策（最低 3 つ・物理ゲート / 観測 を含む）

1. **観測 — verify_effect.sh ai_quality に mode 別分解出力を追加 (Layer A 対策)**: 現在の `perspectives=XX%(N/M)` 1 行に加え、`perspectives_minimal=A/B perspectives_standard=C/D perspectives_full=E/F` を 1 行追加出力する。jq クエリは `[(.topics // .)[] | select(.aiGenerated == true) | select(.summaryMode == "minimal") | select(.perspectives != null and (.perspectives | length // 0) > 0)] | length` を 3 mode 分実行。CI / scheduled task 両方で出力が SLI として可視化されるため、mode の生成失敗率と全体集計が乖離した瞬間に検出できる
2. **物理ゲート — `_build_story_schema(mode, *, cnt)` で mode と cnt を両方受け、minimal でも cnt>=2 で perspectives を生成する構造に変更 (Layer B 対策・本タスクで landing 済)**: `projects/P003-news-timeline/lambda/processor/proc_ai.py` で minimal mode の schema 生成を `cnt>=2` 分岐に変更。`_normalize_story_result(minimal)` で result['perspectives'] が文字列で非空なら propagate。tests `tests/test_minimal_perspectives.py` 11 ケースで schema 分岐 / max_tokens 増量 / prompt 指示文の物理確認。本セクションで横展開チェックリスト表に 1 行追加し landing を物理検証
3. **観測 — SLI 値が一定範囲に張り付く現象を検出する SLI ヘルスチェック (Layer C 対策)**: scheduled task `verify-T2026-0430-G-perspectives` が翌朝 06:30 JST に走り、perspectives 充填率と mode 別内訳を再測定する。60% 未達なら次手 (perspectives min_length 緩和 / cnt=1 への拡張等) を TASKS.md に積む。「指標が改善している」という観測解釈に対する反証として、改善後 N サイクル経っても天井に張り付くなら mode 別分解 SLI を即チェックする運用を実装

### 横展開チェックリスト

本セッションで 横展開チェックリスト 表に 1 行追加 (`projects/P003-news-timeline/lambda/processor/proc_ai.py` の minimal mode cnt-aware schema)。CI `check_lessons_landings.sh` が該当ファイルの存在を物理検証する。

### メタ教訓

- **mode 切替がある SLI は集計 1 値では読めない。** discrete な mode (minimal/standard/full) 分割をしている指標は、必ず mode 別分解で観測する。集計平均は mode 分布の影響で天井 / 床が動くため、生成成功率と母集団の構造的偏りを区別できない
- **「指標が改善 = 順調」は危険な推論。** 改善トレンドを見たら必ず「上限はどこか」「持続性はあるか」「分母を分解したらどうか」の 3 段思考を踏む。トレンド延長線上での目標到達を 1 段で予測すると、天井に張り付く事象を見逃す
- **コスト境界と品質境界を同じ閾値で混ぜない。** mode 設計でコスト最適化のために cnt<=2 で minimal を採用するのは合理的だが、品質指標の生成境界を同じ閾値で混ぜると指標の天井を作る。「コストはここで切る」「品質はここまで担保する」を別軸で設計する
- **SLI 仕様 = 「分母の定義」は明文化が必要。** 「aiGenerated 全体」のような曖昧な分母定義は、後段の実装で構造的偏りを生む。docs/sli-slo.md に「分母の構造」(mode 分布 / 生成成功条件) を明記し、SLI 値の天井 / 床を予測可能にする
- **指標が天井に張り付いたら仕様矛盾を疑う。** SLI が一定値で停滞しているのは「もう少しで届く」のサインではなく、「実装仕様と SLI 仕様が矛盾している」のサインの可能性がある。1 段の改善努力ではなく、仕様の三つ巴比較に切り替える判断を遅らせない

---

## fetcher 0 件保存が 3 日間検知されず — SLI 周期が壊れの平均寿命より長かった（2026-04-30 制定・T2026-0430-H / PR #52）

### 背景

PR #46 で fetcher の Decimal 未ラップで chunk 3/4 失敗が 3 日連続。記事 0 件保存が 72h 続いたが、誰も気付かず PO の手動確認で発覚。修正後 (PR #46) も「次に同じ事故が起きたら何時間で気付けるか」を計算すると最大 72h で、観測穴が同じ位置にある状態だった。

### Why1〜Why5

**Why1**: なぜ 3 日間気付かなかったか
→ 既存 SLI (freshness-check.yml) は updatedAt 差分を 12h 周期で観測していた。fetcher が動作しているか自体は別観測しておらず、updatedAt は Decimal バグでも一部 chunk が成功する run では更新されていた

**Why2**: なぜ updatedAt だけで fetcher 健全性を測れないと判断していなかったか
→ T263 「production 鮮度 SLI が無くて 14h 壊れに気付かなかった」事故で freshness-check.yml を建てた時、updatedAt 差分が「fetcher が動いているか」を間接的にカバーしていると暗黙的に仮定してしまった。fetcher が部分成功する故障モード (chunk 単位失敗) は当時の故障モードリストに含まれていなかった

**Why3**: なぜ故障モードリストが網羅できなかったか
→ 故障モードを「事後の事故」からしか追加していない。「どんな失敗が起きうるか」を事前に列挙する設計レビューが運用前提になっていなかった。Lambda の出力 = 入力件数 × 成功率 という最小モデルから「成功率 0% は updatedAt も 0 件保存も同時に折れる」「成功率 1〜99% は updatedAt は折れないが 0 件保存は起きうる」を派生させる思考が抜けていた

**Why4**: なぜ「fetcher 自体の健全性 SLI」が単独で立ってなかったか
→ Lambda Errors / Duration / Throttles の AWS 標準 metric には依存していたが、これらは「Lambda 関数の例外」しか拾わない。本件のように「関数は完走するが業務的には失敗 (=一部 chunk が except で握りつぶされる)」を拾えない。アプリケーションレベルの構造化健全性ログ (`[FETCHER_HEALTH]` のような) を出す習慣が無かった

**Why5**: なぜアプリケーションレベル健全性ログを既定にしてこなかったか
→ Lambda は短命・stateless なため「健全性は AWS 標準 metric で十分」というクラウドネイティブ前提に乗っかった。だが業務指標 (saved_articles=N) を直接 metric 化する CloudWatch Embedded Metric Format / Metric Filter の運用パターンを採用していなかった。lessons-learned 「success-but-empty 横展開」(2026-04-28) で抽象パターン自体は認識していたが、各コンポーネントへの適用 (T2026-0428-Q) が未完だった

### 仕組み的対策（3 つ landing）

1. **構造化健全性ログ + Metric Filter** — `[FETCHER_HEALTH] {"event":"fetcher_health","saved_articles":N,...}` を fetcher の両 return path で emit。CloudWatch Logs Metric Filter で `$.saved_articles` を `P003/Fetcher.FetcherSavedArticles` に変換。**T2026-0428-Q success-but-empty 横展開の fetcher 分への landing**

2. **CloudWatch Alarm 2h 検知** — `P003-Fetcher-Zero-Articles-2h` (period=30min × 4 evaluation, Sum<1, treat-missing-data=breaching) で連続 2h 0 件保存を検知し SNS 経由 email 通知。**検知遅延 72h → 2h**

3. **Slack 並走通知** — `.github/workflows/fetcher-health-check.yml` (毎時 23 分 UTC) で `aws cloudwatch get-metric-statistics` をポーリングし Slack Webhook POST。SNS 経由 email との 2 系統 belt-and-suspenders。**alarm 1 系統で済ませない設計**

### 横展開チェックリスト追記

| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| fetcher 健全性 SLI (saved_articles) | 2026-04-30 T2026-0430-H | `projects/P003-news-timeline/lambda/fetcher/handler.py`, `scripts/setup_fetcher_alarm.sh` | ✅ | T2026-0430-H で landing (PR #52)。`[FETCHER_HEALTH]` JSON ログ + CloudWatch Metric Filter `FetcherSavedArticles` |
| fetcher 0 件 2h alarm | 2026-04-30 T2026-0430-H | `scripts/setup_fetcher_alarm.sh` | ✅ | CloudWatch Alarm `P003-Fetcher-Zero-Articles-2h` 本番設置済 |
| fetcher Slack 並走通知 | 2026-04-30 T2026-0430-H | `.github/workflows/fetcher-health-check.yml` | ✅ | 1h 周期で metric ポーリング。SNS email との 2 系統検知 |

### メタ教訓

- **SLI 周期は壊れの平均寿命より短く設計する**。72h 沈黙する故障モードに対して 12h 周期 SLI は遅すぎ、6h 周期でも危うい。「壊れたら何時間で誰が気付けるか」を SLI 1 つごとに数値で問い、壊れの寿命 / 2 以下の周期に揃える
- **AWS 標準 metric は「関数 healthy」しか語れない**。業務指標 (=saved_articles のような件数) はアプリケーションが構造化ログで明示的に emit し、Metric Filter で抽出する。クラウドネイティブの「マネージド観測」だけに依存しない
- **アラート経路は最低 2 系統**。SNS email だけでは spam fall や PO 不在で見逃す。Slack 並走の belt-and-suspenders を最初から設計する
- **故障モードを事前に列挙する**。「Lambda が例外を吐く」だけでなく「Lambda は完走するが業務指標 0」「上流が空」「下流が壊れて消費されない」を Why ツリーで派生させ、各々に SLI を立てる

---

## AI フィールド層忘れ CI 物理ガードの landing — 「書いた対策が物理的に動く状態」への昇格（2026-04-30 制定・T256 / PR #53）

### 背景

T249 (2026-04-28) で `proc_ai._normalize_story_result` が emit する `keyPoint` / `backgroundContext` が `handler.py` の `ai_updates` dict と `proc_storage.update_topic_with_ai` の片方に載っておらず、AI 生成成功でも一部フィールドが DDB / S3 に届かない事故。手動調査で発見したが、「proc_ai が出すキーが両層で参照されているか」は CI で機械検査可能なはずだった。T256 として横展開チェックリスト (lessons-learned 818 行・863 行) に「✗ 未 landing」で 2 日間滞留していた。

### Why1〜Why3（短縮版・なぜ滞留したか）

**Why1**: なぜ T249 直後に物理ガードが landing しなかったか
→ 個別事故 (keyPoint 充填率 1.3%・perspectives 45% 天井 etc.) の対症療法 PR が高頻度で走っており、構造ガード PR は「いつかやる」キューの後ろに沈み続けた。

**Why2**: なぜ滞留が可視化されなかったか
→ 横展開チェックリストの ❌ 行を CI で物理検査する `check_lessons_landings.sh` は landing 済 (T2026-0428-BC) だが、❌ 行の存在自体は ERROR にしていなかった (✗ 列が free-form 注記のため)。表は「TODO リスト」として機能していたが「期限切れの ❌ を強制 close する」圧は無かった。

**Why3**: なぜ T256 のような「層を 1 つ忘れる」パターンを type system で防げないのか
→ Python は dict literal キーの型を持たず、`gen_story.get('keyPoint')` の有無を型検査で見つけられない。TypedDict / Pydantic 化すれば防げるが既存コード改造コストが大きい。代替として「proc_ai 出力スキーマと proc_storage / handler の参照キー集合」を AST + grep で突合する物理検査が最小コストの解。

### 仕組み的対策（3 つ landing）

1. **AST 抽出ベースの coverage check** — `scripts/check_ai_fields_coverage.py` が `proc_ai._normalize_story_result` を AST で読み、Return Dict literal の string キー集合を取得。各キーが `proc_storage.py` と `handler.py` で `gen_story.get('K')` または `gen_story['K']` として参照されているか正規表現で確認、欠落で exit 1。

2. **STORAGE_ONLY allowlist による意図表明** — keyPoint 品質メタ (`keyPointLength` / `keyPointRetried` / `keyPointFallback`) は DDB 観測専用で S3 topic file (handler.ai_updates) には流さない。意図的な非伝播は `STORAGE_ONLY` set で明示し、handler 欠落を許容する。**「層をスキップして良いケース」を allowlist で明示する設計**。

3. **境界値テスト同梱 + CI 統合** — `scripts/test_ai_fields_coverage.py` 13 ケース。T249 そのもの (proc_ai に新キー追加・handler 追加忘れ) を再現する `test_t249_pattern_keypoint_missing_in_handler_caught` で「propagation 忘れたら CI が必ず落ちる」ことを物理検証。`.github/workflows/ci.yml` lint-lambda ジョブに `python3 scripts/check_ai_fields_coverage.py` + unittest を追加。

### 横展開チェックリスト追記

| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| AI フィールド層忘れ coverage check | T249 keyPoint / backgroundContext merge 漏れ | `scripts/check_ai_fields_coverage.py` + `scripts/test_ai_fields_coverage.py` | ✅ | T256 で landing (PR #53, 2026-04-30)。AST + grep で proc_ai → proc_storage / handler の双方参照を物理検査。13 ケース境界値テスト同梱 |

### メタ教訓

- **「✗ 未 landing」を期限付きで管理する**。横展開チェックリストの ❌ / ✗ 行は「いつまでに landing するか」を別列で持たず滞留しやすい。今後は「✗ → 起票 ID」を必ず添え、起票後 X 日経過したら CI で WARN を出す案を T2026-0428-BD 系列で検討する
- **対症療法の前に「型 / スキーマで防げないか」を 30 秒問う**。Python TypedDict 化は既存改造コストで採用しなかったが、AST 抽出という代替策が機能した。新規バグを見たら「型で防げる？防げないなら CI で機械検査できる？」を 2 段階で問う習慣を持つ
- **T249 の二の舞いを「allowlist で意図表明する」**。STORAGE_ONLY のように「層をスキップする意図的なケース」を allowlist 化することで、新規開発者が「これも skip して良いのか？」を悩まずに済む。allowlist は単なる例外ではなく、意図のドキュメント

---

## Dispatch 並走ルール違反 + CI 連続失敗（2026-05-01 T2026-0430-REV セッション記録）

### 違反内容（2026-05-01）

1. `local_4b22469a`（Opus・267ターン）と `local_185fe0ed` を同時並走させた
2. WORKING.md に `local_4b22469a` の行がなかった（物理ルール違反）
3. main の CI が PR #80/#81 マージ後に2回連続失敗した
4. Dispatch が `start_code_task` 前に WORKING.md を確認しなかった

### Why1〜Why5

**Why1**: なぜ 2 セッションが並走したか
→ Dispatch が `start_code_task` を呼ぶ直前に WORKING.md の [Code] 行を確認しなかった（手順スキップ）

**Why2**: なぜ WORKING.md を確認しなかったか
→ CLAUDE.md の「Dispatch 起動時」セクションに「start_code_task 前に WORKING.md [Code] 行が 0 件であることを確認」という明示的ステップが書かれていなかった。session_bootstrap.sh の [Code] ≥2 ERROR は Code セッション起動後に動くため、Dispatch 側の確認圧がなかった

**Why3**: なぜ `local_4b22469a` は WORKING.md に行を書かなかったか
→ 267ターン・長期セッションでコンテキスト圧縮が発生し、冒頭の「WORKING.md 宣言 → push → 着手」という物理ルールが圧縮で参照されなくなった。かつ WORKING.md 宣言なしで着手しても session_bootstrap.sh は「0件 → 問題なし」と判定するため、外部からも違反が見えなかった

**Why4**: なぜ CI が PR #80/#81 で2回連続失敗したか
→ PR #80 マージ後に CI 失敗を確認しないまま PR #81 をマージした。Dispatch が「CI 待ちは即クローズ」ルールを Code セッション向けと誤解し、次タスク起動前の CI 確認ステップを省略した

**Why5**: なぜ Dispatch は CI 失敗を見落としたか
→ CLAUDE.md の CI 待ちルールが「Code セッション側の Monitor ポーリング禁止」として書かれており、「Dispatch 側の次タスク起動前 CI パス確認」が明文化されていなかった。p003-haiku（朝 7 時）は翌朝まで確認しない設計のため、即時フィードバックループが存在しなかった

### 仕組み的対策（4 つ）

1. **CLAUDE.md「Dispatch 起動時」セクションに明示ステップ追加**（即時・本セッションで実施）
   → `start_code_task` 呼び出し前の必須確認として「`cat WORKING.md` で [Code] 行が 0 件であることを確認」を手順ステップに追加。Dispatch の手順書を最短ルートで物理化

2. **session_bootstrap.sh に「WORKING.md 自己宣言リマインダー」を追加**（即時・本セッションで実施）
   → 起動時に [Code] = 0 の場合でも「このセッションは WORKING.md に [Code] 行を追記してから着手すること」を明示出力。宣言なしで着手できないわけではないが、見落とし率を下げる

3. **CLAUDE.md に「PR merge 前 CI 確認」を Dispatch 手順に追加**（即時・本セッションで実施）
   → 「次タスクを Dispatch が起動する前に `gh run list --branch main --limit 3` で直近 CI がすべて green であることを確認する」を Dispatch 起動前チェックに明記

4. **lessons-learned.md 横展開チェックリストに 1 行追加**（本セッション・本 commit）

### 横展開チェックリスト追記

| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| WORKING.md 自己宣言リマインダー | 2026-05-01 local_4b22469a 267ターン無宣言 | `scripts/session_bootstrap.sh` | ✅ | 本セッション (T2026-0430-REV) で landing。[Code]=0時に「WORKING.md に [Code] 行を追記してから着手」を出力 |
| Dispatch start_code_task 前チェック | 2026-05-01 WORKING.md 未確認並走 | `CLAUDE.md` | ✅ | 本セッション (T2026-0430-REV) で Dispatch 起動時セクションに明示追記 |

### メタ教訓

- **物理ガードは「侵入後」ではなく「侵入前」に置く**。[Code]≥2 ERROR は「2件目が起動した瞬間」に作動するため、1件目が WORKING.md 宣言なしで動いていると永遠に検知されない。予防的チェックポイント（Dispatch 側の確認ステップ）が必要
- **コンテキスト圧縮は「長期セッションの敵」**。267ターン超のセッションでは冒頭の物理ルールが圧縮消滅する。重要な前提（WORKING.md 宣言）は session_bootstrap.sh の出力として「毎回見える場所」に置く
- **CI 待ちルールの読み手を明記する**。「Code セッション向け」と「Dispatch 向け」でルールの内容が違う。文書に書く際は「誰が読む前提か」を冒頭に明示し、対象外の読み手が誤解しないようにする

---

## PR #86 22箇所一括変更がなぜなぜなし・物理ガードをすり抜けたか（2026-05-01 制定・T2026-0501-E）

### 背景

T2026-0501-E は「GitHub Actions YAML 内の `python3 -c` インラインロジック (22箇所) を `scripts/ci_*.py|sh` へ一括移行し、`lint-yaml-logic.yml` CI ガードを追加する」リファクタリング PR（PR #86）。PR はマージされたが、なぜなぜ分析なし・変更ファイル 36 件という広範囲変更のままPRテンプレのチェックをすり抜けた。後日レビューで「なぜこの規模の変更になぜなぜがないか」が問題として浮上。

### Why1〜Why5

**Why1**: なぜセッションはなぜなぜをせずに実装したか
→ PR テンプレの「バグ修正の場合のみ」セクションに `docs/lessons-learned.md に Why1〜Why5 + 仕組み的対策 3 つ以上を追記した` チェックボックスがある。T2026-0501-E は「refactor」カテゴリとして起票されていたため、「バグ修正でない = なぜなぜ不要」と判断された。変更規模（36ファイル・22箇所）に応じたプロセス分岐が PR テンプレに存在しない。

**Why2**: なぜ check_lessons_landings.sh が防げなかったか
→ `check_lessons_landings.sh` は「lessons-learned.md の横展開チェックリスト表に書かれたファイルが repo に存在するか」を検査する *出口ガード*。PR #86 は lessons-learned.md に新エントリを追加していないため、チェックリスト表に新行も生まれず、このスクリプトは 0 件検査で pass した。「なぜなぜエントリが書かれているか」を検査する *入口ガード* が存在しない。

**Why3**: なぜ 22 箇所一括というパターンを選んだか
→ タスク定義 T2026-0501-E が「全 YAML のインラインロジックを一括移行」という最大スコープを含んでいた。「1 タスク 1 スコープ」ルールを守ったまま、タスク定義そのものに「22 箇所すべて」が組み込まれていた。「変更ファイル数 N 超の PR には段階分割または追加評価を要求する」という CI ガードが存在しなかった。

**Why4**: なぜ Dispatch はセッション起動前になぜなぜを要求しなかったか
→ Dispatch の起動プロトコル（CLAUDE.md「Dispatch / Cowork 起動時」セクション）に「なぜなぜ要求」ステップが存在しない。なぜなぜは「バグ修正後」のプロセスとして設計されており、「広範囲 refactor 実装前の予防的なリスク評価」としての位置付けが未定義。タスクに `[scope:large]` のような広範囲変更マーカーがなく、Dispatch が変更規模を事前に把握する手段がなかった。

**Why5**: なぜ「ルールより仕組み」というルール自体が仕組みになっていないか
→ CLAUDE.md の絶対ルール表に「物理 = CI/hook/scripts で物理ガードあり」「思想 = テキストルールのみ」の分類がある。しかしこの分類を強制する仕組みがない。新しいルールが「物理」として追加されても「そのガードが既存の全ファイル/全フローを実際にカバーしているか」を検証するステップが PR テンプレに存在せず、新規 CI ガードが将来のコードのみを守り過去の蓄積には適用されないパターンが生まれやすい。

### 仕組み的対策（3 つ・未実装・要タスク起票）

1. **PR テンプレに「変更ファイル数 >= 10」ガードを追加（CI + テンプレ改修）**:
   - `.github/workflows/ci.yml` に `gh pr diff --name-only | wc -l` が 10 超の場合 `::warning::` を出す step を追加。警告時は PR テンプレの「大規模変更の場合のみ」セクション（なぜなぜ/影響範囲/段階分割の検討を必須）に明示的に誘導する。
   - 実装ファイル候補: `.github/workflows/ci.yml`, `.github/pull_request_template.md`

2. **新規 CI ガード追加時に既存ファイルへの後方適用を PR テンプレで強制**:
   - PR テンプレに「新規 CI ガード追加の場合のみ: `bash <新ガードスクリプト> .` を main branch 全対象ファイルに実行して 0 件検出を確認した」チェックボックスを追加。これにより「新規 CI ガードが既存の違反を含まずに 0 件 pass している」ことを PR マージ前に明示的に証明させる。
   - 実装ファイル候補: `.github/pull_request_template.md`

3. **TASKS.md の `[scope:large]` タグを session_bootstrap.sh が検査**:
   - TASKS.md のタスク定義で推定変更ファイル数が 10 件超のものには `[scope:large]` タグを付ける規則を設ける。`session_bootstrap.sh` が着手前に `[scope:large]` タグ付きタスクを grep し「大規模変更タスク: PR を機能単位に分割したか・なぜなぜ/影響評価を実施したか」を出力する。
   - 実装ファイル候補: `scripts/session_bootstrap.sh`, `TASKS.md` テンプレ追記

### 横展開チェックリスト

| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| PR 大規模変更 CI ワーニング (>=10ファイル) | 2026-05-01 PR#86 ガード不備 | `.github/workflows/ci.yml` + `.github/pull_request_template.md` | ✗ | 要タスク起票 |
| 新規 CI ガード後方適用チェックボックス | 2026-05-01 PR#86 ガード不備 | `.github/pull_request_template.md` | ✗ | 要タスク起票 |
| session_bootstrap.sh scope:large 検査 | 2026-05-01 PR#86 ガード不備 | `scripts/session_bootstrap.sh` | ✗ | 要タスク起票 |
...
## Dispatch 違反多発 なぜなぜ分析（2026-05-01 制定）

**起きたこと**: Dispatch セッション内で複数ルール違反が同時発生。①手動 invoke を提案 ②コード読まずにパラメータを埋めてプロンプト送信 ③実機確認なしで完了報告 ④効果検証なしで完了報告 ⑤CI失敗・ルール違反・stale エントリーに気づいて無視。PO指摘「壊れたまま進むな」「仕組みにしろ・癖はダメ」。

| Why | 答え |
|---|---|
| **Why1** なぜ複数ルール違反が同時発生したか | Dispatch が長期継続（20+往復）でコンテキスト喪失状態に陥り、CLAUDE.md の「絶対ルール」をリアルタイム確認できなくなった |
| **Why2** なぜコンテキスト喪失が起きたか | Dispatch セッション = 会話式のため、「現在どこまでやったか」を会話履歴に依存していた。会話が進むにつれ、前半の決定事項・ルール確認が視界から消える |
| **Why3** なぜそれに気づかなかったか | CLAUDE.md に「Dispatch 絶対禁止パターン」が明文化されていなかった。代わりに「絶対ルール」表に分散していたため、「Dispatch に特有の禁止事項」と「全セッション共通ルール」の区別が曖昧だった |
| **Why4** なぜ禁止パターンが明文化されていなかったか | 過去の Dispatch セッションが短期（3-5往復）で終わっていたため、「長期継続時のコンテキスト喪失」という現象自体が認識されていなかった。今回が初めて露見した |
| **Why5** なぜ過去の Dispatch セッションが短かったか | WORKING.md の「Dispatch 継続性」セクション導入（2026-05-01）以前は、Dispatch が長期化する際の「状態引き継ぎ仕組み」が存在しなかった。セッション継続を判断する明示的なトリガーも無かった |

**仕組み的対策:**

1. **CLAUDE.md に「Dispatch 絶対禁止パターン」テーブルを明文化（2026-05-01 実装）**:
   - 禁止行為と代替手段をセットで記載。禁止: 「手動 invoke 提案」「パラメータ適当」「実機未確認」「効果検証なし」「違反無視」「長期継続」。
   - 実装ファイル: `CLAUDE.md`（新セクション追記済）

2. **WORKING.md の「Dispatch 継続性」セクション + 8h stale 自動削除の運用継続**:
   - Dispatch が往復 20 回を超えたら、状態を WORKING.md に記入して現在のセッションを exit。新セッション起動で「状態引き継ぎ」を物理的に強制。
   - session_bootstrap.sh が開始時に WORKING.md を読むことで、前セッションの「次のアクション」「未解決項目」が自動的に視界に入る
   - 実装ファイル: `WORKING.md`（既存ルール）

3. **Dispatch → Code セッション起動時の「根拠ファイル Read 必須化」**:
   - コードセッション起動前に Dispatch が根拠ファイル（TASKS.md / lambda/ 関連コード等）を最低 1 つ Read してから、それに基づくパラメータをプロンプトに記載する。「コードを読まずにパラメータを埋めるな」という禁止ルールを仕組み化。
   - チェックリスト: Dispatch → Code プロンプト作成時に自問「このパラメータに根拠ファイルを貼っているか」
   - 実装ファイル: `CLAUDE.md`（Dispatch → Code 起動時セクション）

4. **効果検証・実機確認をスケジューラーに委ねるフロー確立（運用ルール・2026-05-01 既存）**:
   - 「完了 = 実装完了」ではなく「完了 = 効果検証済み」。効果検証がまだなら、Code セッション exit 後 one-time scheduler task（p003-sonnet）を登録して数値確認を委ねる
   - 「実機確認は絶対に Dispatch が やる」をルール化。Code セッションが「完了した」だけでは報告させない
   - 実装ファイル: `CLAUDE.md`（「完了の流れ」セクション）

### 横展開チェックリスト

| 対策名 | 出所 | 実装ファイル | 状態 | 備考 |
|---|---|---|---|---|
| Dispatch 絶対禁止パターンテーブル | 2026-05-01 違反多発 | `CLAUDE.md` | ✅ 実装済 | セクション「Dispatch 絶対禁止パターン」追記 |
| WORKING.md Dispatch 継続性 + 8h TTL | 2026-05-01 違反多発 | `WORKING.md` / `scripts/session_bootstrap.sh` | ✅ 実装済 | 既存ルール・動作確認済 |
| Code セッション起動時「根拠ファイル Read 必須化」チェック | 2026-05-01 違反多発 | `CLAUDE.md`（Dispatch → Code セクション） | ⚠️ 運用ルール化のみ | 物理ガード（スクリプト検査）は未実装・要タスク |
| 効果検証・実機確認を Dispatch が実施するフロー確立 | 2026-05-01 違反多発 | `CLAUDE.md`（「完了の流れ」） | ✅ 実装済 | 既存ルール・実行方式の周知強化 |

### メタ教訓

- **Dispatch のコンテキスト喪失は「長期継続時の必然的現象」**。会話履歴の長さ ∝ 過去の決定事項がスクロール外。物理的に「状態ファイル」（WORKING.md）に外出しして、セッション開始時に自動的に読ませる設計が本質的解決
- **「禁止事項」を「絶対ルール表の 1 行」に埋めるな。Dispatch/Code 各セッション種別に特有の禁止パターンは視認しやすい独立テーブルで明文化する**。分散ルールは長期継続時に確認されない
- **「効果検証は完了条件」なのに、スケジューラーに委ねるのは「投げ出し」ではなく「コンテキスト保護」**。Dispatch/Code の短期セッション内で時間待ちをしてコンテキスト喪失するより、状態をファイルに記録してスケジューラーに委ねることで、次回セッションが対処できる
