...
## T2026-0502-MU-FOLLOWUP-ARCHIVED: high-value archived トピックが quality_heal から無条件除外されていた（2026-05-02）

**起きたこと**:
- `quality_heal.py` の `find_unhealthy()` は `lifecycleStatus='archived'` を無条件スキップしていた
- `4bf3a46568f1189c` (EU関税 score=201, ac=13, summaryMode=standard) が archived 状態で
  4軸全 False のまま放置。quality_heal もスキップし続けたため AI 再処理キューに入らなかった

**なぜなぜ**:

| Why | 答え |
|---|---|
| **Why1** なぜ 4bf3a4 が放置された？ | quality_heal が archived を一律スキップしていた。pendingAI=True にならず AI 再処理が起動しなかった |
| **Why2** なぜ一律スキップしていた？ | 「archived = lifecycle Lambda の管轄 = 触らない」という設計原則を、AI 品質修復も含む全ての heal から除外していた |
| **Why3** なぜ "全除外" が正しいと思われていた？ | archived トピックは表示されないため「heal しても意味なし」という暗黙の前提があった。しかし score=200 超・ac=13 の高価値トピックは archived 解除後に即表示されうる |
| **Why4** なぜ archived の高価値判定がなかった？ | 設計時に archived 分布を実測していなかった。実測すると 275/2283 件中 high-value (ac>=6 && score>=100) は 1 件のみで、コスト爆発リスクがないことが判明 |
| **Why5** なぜ分布実測が設計に反映されなかった？ | lifecycle 除外ロジックを追加した時点でスキャンデータがなく、「全除外が安全」とデフォルト判断した。実測駆動の閾値設定が欠落していた |

**仕組み的対策**（最低 3 つ）:

1. **物理（コード修正）**: `find_unhealthy()` の archived 除外を `ac >= 6 and score >= 100` の高価値条件付きに変更。`scripts/quality_heal.py:108-115`
2. **物理（境界テスト）**: `tests/test_quality_heal_mode_upgrade.py` に lifecycle × (ac, score) 境界テストを追加。archived/legacy/deleted の rescue 条件を網羅
3. **思想（実測駆動の閾値設定）**: lifecycle 除外などコスト影響があるフィルタ追加時は事前に DDB scan で分布実測し、対象件数を設計記録に書く。「全除外が安全」のデフォルト判断禁止

**横展開チェックリスト**:
- [x] `scripts/quality_heal.py` の archived 除外を高価値条件付きに修正（**物理** / 本 PR で実装）
- [x] `tests/test_quality_heal_mode_upgrade.py` に境界テスト追加（**物理** / 本 PR で実装）
