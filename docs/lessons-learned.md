...
- 「chore: マージコンフリクト解決 (upstream版を採用)」で **CLAUDE.md を 156→117 行に圧縮** + `~/ai-company/...` を `/Users/murakaminaoya/ai-company/...` に置換
- 結果: ① Cowork 用の重要セクション（Dispatch 起動時手順 / Dispatch 絶対禁止パターン）が消失 ② `murakaminaoya` を含む PII が main に流入 ③ main の PII 検査 CI が真っ赤化 ④ 自分の PR (#125) も auto-merge できず `mergeable_state=blocked` で詰まった
...
2. **物理ゲート（done.sh の PR merged + main green ゲート）**: `done.sh <task_id>` 内で「自分の PR が merged 済み」「main の最新 CI が green」両方を確認するゲート追加。`Verified-Merge: PR #N merged @ <JST>` を必須化し、未 merge なら ERROR で完了拒否
...
- [ ] auto-merge 詰まり監視 schedule task を作成（タスク化: T2026-0502-AUTOMERGE-WATCHER）
