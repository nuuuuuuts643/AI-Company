...
1. https://qiita.com にアクセスしてログイン（アカウントなければ無料登録）
2. 右上の「投稿する」ボタンをクリック
3. **タイトル欄**に以下をコピペ：
   ```
   AIでニュースの「流れ」を可視化するサービスを個人で作った話（技術編）
   ```
4. **本文欄**に「---」から下の本文全部をコピペ（このメモの「はじめに」から「ソースコードは非公開ですが」までの部分）
5. **タグ**に以下を設定：
   ```
   Python  AWS  DynamoDB  個人開発  AWSLambda
   ```
6. 「公開する」→「全体公開」で投稿完了

### Zennに投稿する場合（Qiitaより少しエンジニア寄り）

1. https://zenn.dev にアクセスしてGitHubアカウントでログイン
2. 「投稿する」→「記事を書く」
3. タイトルと本文を同じようにコピペ
4. トピックに `python` `aws` `個人開発` を設定
5. 「公開する」

### どちらに投稿するか
- **Qiita優先**：エンジニア人口が多くて流入が期待しやすい
- 両方に投稿してもOK（内容が全く同じでも問題なし）
