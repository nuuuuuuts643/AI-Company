...
### Step 6: インクリメンタル・ストーリー型処理（コスト削減＋品質向上）

> ⚠️ モデル使い分け（旧Step6）はPOより「Haikuのままで良い」と確認済み。スキップ。

**PO指示（2026-05-04）**: 「新しい記事が出るたびに全文書き換えはおかしい。追加記事は追加分だけ書けば良い。こうなって→こうなった、という形にしたい。記事リンクは別ページに逃がしてストーリーとして読めるようにしたい。予測セクションも欲しい。まず政治経済で試す。」

**問題意識**:
- 現在: 記事が増えるたびに Claude がストーリー全体を書き直す（全トークン消費）
- 結果: 「今日時点のスナップショット」にしかならない。「トランプ米軍削減→イラン絡む→EU関税と連動」という因果の流れが保持されない

**目標設計（章型ストーリー）**:

```
topic.story = [
  { date: "2026-04-30", summary: "トランプ、ドイツ駐留米軍の削減を検討", articleIds: [...] },
  { date: "2026-05-01", summary: "英国王夫妻が敬意を表明、ウイスキー関税も撤廃へ", articleIds: [...] },
  { date: "2026-05-02", summary: "EUが自動車関税25%を表明、イランは依然として対話拒否", articleIds: [...] },
  ...
]
topic.outlook = "米欧の関税交渉と中東情勢が連動する展開が続く見込み"
```

**コスト削減の仕組み（物理的）**:
- 新規チャプター生成時: 当日の新着記事分のみを Claude に投げる（差分処理）
- 既存チャプターへの書き直しは原則禁止（追記のみ）
- outlook のみ定期更新（全チャプターのサマリーから再生成）

**実装フェーズ**:

1. **Phase S1（スキーマ拡張）**: DynamoDB に `chapters` 配列フィールド追加。既存 `aiSummary` は互換フォールバックとして保持。
2. **Phase S2（processor 変更）**: 新着記事検出 → 差分チャプター生成 → `chapters` 配列に append。全書き直しロジックを廃止。
3. **Phase S3（フロントエンド変更）**: 章型タイムラインビュー・記事一覧は別ページへ・outlook セクション追加。
4. **Phase S4（政治経済カテゴリで試験）**: genre=politics/economics のみ章型を有効化。他カテゴリは従来フォーマット維持。

**切り戻し**: `USE_CHAPTER_MODE=false` 環境変数でトピックごとに旧フォーマットにフォールバック。

**設計前に確認が必要な事項**:
- 既存トピック（163件のバックログ）の移行方針: 新規章のみ追記 or 初回は要約から変換？
- チャプター上限（最大何日分 = 何件のチャプター）を持つか
- proc_ai.py のどの関数を新設/変更するか → Step 7（ファイル分割）が先に完了しているほうが実装しやすい

**実装は Step 7（ファイル分割）完了後に着手する。** ← Step 7 完了済み（PR #396）。以下に詳細設計を追記。

---

#### PO指示まとめ（2026-05-04）

- 全文書き換えをやめ、新着記事の追加分だけ書く
- 過去の事実は変わらない → チャプターは **append-only**（書き直し禁止）
- 記事リンクは別ページに逃がす。ストーリーとして読ませる
- 事実だけでなく「なぜ重要か・何が変わるか」の解説を各チャプターに入れる
- 「〜の可能性」「〜かもしれない」を最小化。断定的に書く（間違えたら次のチャプターで訂正）
- スカスカな一般論の要約は不要。多少尖ってよい（リーガルライン遵守）
- 速報性は不要。記事が集まってからバッチ処理で正解を作る
- まず政治経済カテゴリの、記事数最多トピックで1件「正解サンプル」を手動設計してからプロンプトに落とす
- UI/UXへの波及を意識して慎重に進める

---

#### 新データ構造（DynamoDB + S3 スキーマ）

```json
{
  "topicId": "xxx",
  "background": "このトピックが生まれた前提・文脈（既存記事群から初回に生成。以後変更不可）",
  "keyPoint": "このトピックで何を見ていけばよいか（注目点。初回生成・以後変更不可）",
  "chapters": [
    {
      "date": "2026-04-30",
      "summary": "トランプがドイツ駐留米軍を5000人以上削減すると発表",
      "commentary": "これはNATOへの欧州の負担増要求と対EU関税交渉の圧力カード。欧州が応じれば関税緩和、拒否すれば経済的締め付けが続く",
      "prediction": "欧州はNATO分担金を増やす代わりに自動車関税の撤廃を求める交渉に移行する",
      "articleIds": ["a1", "a2"]
    }
  ],
  "outlook": "（最新チャプターから自動更新。全チャプターを踏まえた次の動きの予測）",
  "relatedTopicIds": [],
...
| 既存フィールド | 新構造での扱い |
|---|---|
| `aiSummary` | フォールバック用に保持（フロント互換）。新規は `chapters[0].summary` |
| `perspectives` | `chapters[N].commentary` に統合。廃止方向 |
| `outlook` | 最新チャプターから定期再生成（内容は変わる唯一のフィールド） |
| `background` | 復活（初回生成・以後append-only）|
| `keyPoint` | 初回生成・以後変更不可 |

---

#### コスト削減の仕組み（物理的ゲート）

現在: 記事10件 → 毎回10件全部を Claude に投げる
新構造: 前回処理後の新着記事のみを投げる（差分処理）

- `lastChapterDate` より新しい記事が 0件 → Claude 呼び出しゼロ（物理スキップ）
- 1件以上 → 差分だけで新チャプター生成
- 全体の再生成は禁止（コード上で物理ブロック）
- outlook の更新: 全チャプターの summary だけを連結して Claude に投げる（記事本文不要・低トークン）
...
```
禁止表現: 「〜の可能性がある」「〜かもしれない」「〜と見られる」
推奨表現: 「〜になる」「〜だ」「〜した」
commentary に必須: なぜこれが重要か + 何が変わるか（読者への影響）
prediction に必須: 次に何が起きるか（断定形・外れたら次チャプターで訂正）
禁止コンテンツ: 特定個人への中傷・未確認情報の断定・株価予測の具体的数値
```
...
| フェーズ | 内容 | 切り戻し |
|---|---|---|
| **S0（設計検証）** | 政治経済の記事最多トピック1件で「正解チャプター」を手書き。プロンプト設計に反映 | なし（データ変更なし） |
| **S1（スキーマ拡張）** | DynamoDB に `chapters`/`background`/`relatedTopicIds` フィールド追加。既存フィールドは保持 | フィールド削除するだけ |
| **S2（processor）** | 差分処理ロジック + 新チャプター生成プロンプト実装。`CHAPTER_MODE_GENRES=politics` で政治経済のみ有効化 | 環境変数を空にする |
| **S3（フロント）** | 章型タイムラインビュー。`chapters` がない場合は `aiSummary` にフォールバック | フォールバックがある |
| **S4（全genre展開）** | `CHAPTER_MODE_GENRES=*` で全カテゴリ有効化 | 環境変数で戻す |

**S0 → S1 → S2 → POレビュー → S3 → S4 の順。S2 完了後に必ずPOレビュー。**

---

#### 制約・リスク

- **初動問題**: 既存トピックの「本当の初動」は取れない。初回チャプターは「現時点での状況まとめ」として生成する
- **チャプター上限**: 1トピック最大30チャプター（約1ヶ月分）。それ以上は古いものをアーカイブ（ライフサイクル管理）
- **DynamoDB サイズ制限**: chapters配列が増えると400KB/アイテム制限に近づく可能性 → 必要なら chapters を S3 に移す
- **UI/UX波及**: フロントの変更は大きい。S3 フェーズで別セッションを立てる
- **ブラックボックス禁止**: 全チャプターの生成ロジック・プロンプトは `proc_prompts.py` に一元化し可視化する
