...
- `commentary` の中で関連トピックに言及する場合、文中に `[topicId:xxxx]` プレースホルダーを埋め込む。フロントがリンクとしてレンダリングする（文脈の中にリンクを置くことで回遊を促す）

#### フロントエンド画面設計（S3 向け・2026-05-04 確定）

詳細設計: `docs/decisions/002-frontend-2axis-navigation.md` を参照。

**基本画面（全ユーザーが見る）**
- チャプターを時系列で並べる（縦の地図）
- 各チャプター: 事実の箇条書き（summary）＋ 予測1行（prediction）
- 関連トピックへの内部リンク（relatedTopicIds）を基本画面に表示（横の地図）

**「詳しく見る」展開時（興味ある人のみ）**
- 解説（commentary）— 文中の `[topicId:xxxx]` は押しやすいリンクとしてレンダリング
- 元記事リンク一覧（articleIds）

**トピック一覧画面（変更しない）**:
- 既存の「流行ってる / 流行ってない」表示（盛り上がり度・記事増加トレンド）は必ず残す
- チャプター型に変えても `lastChapterDate` が直近かどうかで「今また動いてるトピック」を自然に表現できる
- 一覧 → 詳細への動線はそのまま維持

**設計思想**:
- ニュースを普段読まない人が「世の中の流れ」をサクッと確認できる場所
- 事実を流し読み → 予測で引っ張られて次チャプターへ → 気になれば解説を開く
- 解説の中の関連リンクが文脈の分岐点として機能し、読んでるうちに地図の中を動いてる体験になる
- 解説 + 元記事は同じ展開エリアに置くことで「深掘り」の出口を一箇所に集約
...
| **POレビュー** | S0 の正解サンプルを確認してプロンプト方針を確定 → **✅ 完了 2026-05-04: 画面設計確定（箇条書き事実＋予測＋内部リンク基本、解説＋元記事は展開式）** | S0完了後 |
| **S1** | DynamoDB スキーマ拡張: `chapters` / `background` / `relatedTopicIds` フィールド追加。読み取り互換性確認 → **✅ 完了 PR #398** | POレビュー通過後 |
| **S2** | processor 変更: 差分処理・`lastChapterDate` 以降の記事のみ送る・`CHAPTER_MODE_GENRES=politics` 環境変数でジャンル限定 → **✅ 完了 PR #399** | S1完了後 |
| **S2.5** | ソースフィルタリング: 一次ソース優先・転載除外・per-source上限・設計記録（docs/decisions/001） → **✅ 完了 PR #402** | S2完了後 |
| **S3** | フロントエンド変更: チャプター型タイムライン表示・2軸双方向ナビ（docs/decisions/002） | S2完了・POレビュー後 |
