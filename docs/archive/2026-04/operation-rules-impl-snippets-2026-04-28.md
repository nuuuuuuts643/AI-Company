...
> 本ファイルは write 操作なし。POが Code セッションで内容をそのままコミットすることを想定。
...
03:30 レポートは提案表 (OR-1〜OR-17) で止まり、CLAUDE.md は依然 440 行 (上限超過)。提案を「読む」段階のままだとまた肥大化するだけなので、最低限の物理ゲート 3 種を実装ドラフトに落とした。意思決定はPO、コピペ実行は Code セッション、で完結する形にする。
...
> POが Code セッションで上から実行することを想定。各ステップ単独で動作確認できる粒度に分けた。
...
- PO手元で 1 回試す（自分で fix: のテスト commit を作って Verified 行なしで reject されるか確認）
...
- 03:30 レポート OR-4 のみ。Step 1〜3 完了後にPO判断で着手
...
- **CLAUDE.md の直接編集**: PO未承認のうちに単一情報源を破壊するリスクが大きい。本ファイルはドラフト止まり
