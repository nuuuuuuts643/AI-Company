...
```bash
bash /Users/OWNER/ai-company/scripts/session_bootstrap.sh
```
...
| **実装前に全体影響マップ必須** | 新機能追加・修正前に ①影響ファイル一覧 ②依存方向 ③副作用シナリオ を箇条書きで列挙し、POに確認してから着手。「とりあえず実装」禁止。調査→報告→承認→着手の順を物理ルールとする |
