...
### PR作成後のCI確認+マージ（2026-05-03 追加）

Code（実装セッション）: PR 作成 → 即 exit。Dispatch（Haiku）: PR 受け取り → `gh pr checks` green 確認 → `gh pr merge --squash` → 報告。詳細は `docs/rules/ci-and-merge-workflow.md` を参照。
