...
| **git エラー黙殺禁止** | scripts 内の `git pull` / `git push` は `PIPESTATUS[0]` で捕捉し、失敗時は `BOOTSTRAP_EXIT=1` に流して末尾で exit 1。`\|\| true` + `tail -N` での終了コード黙殺禁止。FUSE noise filter (`_strip_fuse_noise`) は維持 | 観測 (週次 audit) + bootstrap 実装のみ。新規スクリプトへの混入は思想 |
...
### PR作成後のCI確認+マージ（物理ルール・2026-05-03 追加）

**PR を出したコードセッションは即 exit。Dispatch が CI確認+マージ専用 Haiku セッションを起動する。**
- PR作成セッション: branch + PR 作成 → 即 exit
- **Dispatch の義務**: PR完了報告を受けたら Haiku セッション「PR #NNN CI確認+マージ」を起動
  - `gh pr checks NNN` で green/fail/pending を確認 → green なら `gh pr merge NNN --squash --admin` → 完了報告
  - fail: エラー分析 → 修正 → push → 再確認 → マージ
  - pending: 最大2分待機・2回まで再確認。それ以上は Code セッション起動に handoff
- **禁止**: 「p003-haiku に任せる」「bootstrap に任せる」。CI 失敗を誰も確認しない事態を防ぐ
- **禁止**: Monitor ポーリング。1〜2回の直接確認のみ（コスト規律ルール §2 に準拠）
