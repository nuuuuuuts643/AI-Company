...
| ルール | 内容 | 強度 |
|---|---|---|
| **完了 = 動作確認済み + 効果検証済み** | `done.sh <task_id> <verify_target>` + `verify_effect.sh <fix_type>` (`ai_quality` / `mobile_layout` / `freshness` / `empty_topics`) で改善を数値で確認。閾値未達は未完了 | Verified 行は commit-msg hook で物理。done.sh 呼び出し自体はツール（思想） |
| **Verified 行 commit 必須** | `feat:` `fix:` `perf:` には `Verified:` 行（commit-msg hook で reject）。helper: `verified_line.sh` | 物理 |
| **Verified-Effect 行 commit 必須** | `feat:` `fix:` `perf:` には `Verified-Effect:` か `Verified-Effect-Skip:` か `Verified-Effect-Pending:` のどれかを必須（commit-msg hook で reject）。Skip は理由 (build artifact / test fixture 等) 明示。Pending は `Eval-Due` 日付明示 | 物理 |
...
| **deploy.sh は直接実行しない** | デプロイは GitHub Actions 任せ。インフラ新規作成のみ例外でPO確認 | 物理 (pre-commit で reject) |
...
| **main 直 push 禁止** | `git push origin main` は `pre-push` hook で物理拒否。実コード変更は必ず branch + PR。bootstrap sync の chore: commit のみ `ALLOW_MAIN_PUSH=1` で escape。緊急 bypass `git push --no-verify` 使用時は WORKING.md に理由＋`Verified-Effect:` 必須 | 物理 |
| **git エラー黙殺禁止** | scripts 内の `git pull` / `git push` は `PIPESTATUS[0]` で捕捉し、失敗時は `BOOTSTRAP_EXIT=1` に流して末尾で exit 1。`\|\| true` + `tail -N` での終了コード黙殺禁止。FUSE noise filter (`_strip_fuse_noise`) は維持 | 物理 (pre-commit で .sh ファイルの `git pull/push \|\| true` を reject) + 週次 audit |
| **設計記録 + 想定効果が必須** | 新しいモジュール・フィルタ・スキップ条件を追加する場合、`docs/decisions/` に「目的・想定効果・成功基準・変更トリガー」を先に書く。commit-msg hook で `feat:` `perf:` には `Expected-Effect:` 行を必須化。詳細: `docs/rules/design-by-intent.md` | 物理 (commit-msg hook) |
...
| **polling パターン物理 reject** | `sleep N && curl` / `sleep N && aws lambda` / `for i in {1..N}; do curl/aws ... done` を含む code は **pre-commit hook で reject**。1 回確認 + schedule (`p003-haiku` 朝7:08) 委ね。bypass = `git commit --no-verify` (要 WORKING.md 記録) | 単行パターンのみ物理 (pre-commit)。複数行・Python・subshell は思想 |
...
### PR 作成後のマージ（2026-05-03 更新）

**コードセッションは PR 作成時に必ず `gh pr merge --auto --squash` を付けて即 exit。** CI グリーン後に auto-merge.yml が自動 squash merge する。Dispatch のマージセッション起動不要。CI が fail した場合のみ修正 → push → 自動マージ待ちに戻る。
...
**現在着手中** → `WORKING.md` / **完了済み** → `HISTORY.md` / **未着手** → `TASKS.md`（詳細は `docs/system-status.md`）
