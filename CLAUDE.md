...
**作業開始前に必ずやること：**
1. `P003 技術状態スナップショット` テーブルを読む
2. これからやろうとしていることが ✅ になっていないか確認する
3. ✅ なら「すでに完了済み」として**スキップ**し、次の未完了タスクに移る
4. 今セッションでやることをユーザーに宣言してから作業を始める

**作業完了後に必ずやること：**
1. `P003 技術状態スナップショット` テーブルを最新状態に更新する
2. `次フェーズのタスク` の完了済み項目を「完了済み」セクションに移動する
3. CLAUDE.md の変更を含めて `git add -A && git commit && git push` する

**重複作業を避けるルール：**
- 別セッションが同じ作業をしている可能性を常に意識する
- `git pull --rebase` で最新を取り込んでから作業する（スタートアップで済み）
- 同じコンポーネントを短時間に複数セッションで触らない
- **30分ごとに `git pull --rebase && git log --oneline -5` で他セッションの進捗を確認する**
