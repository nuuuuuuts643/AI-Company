...
| processor AI要約 | ✅ 稼働中 | 2026-04-26 | generatedSummary: 95.8%(298/311件)・storyTimeline: 90%(280件)・imageUrl: 80.4%(250件)。4x/day自動実行 |
...
| AI要約カバレッジ | ✅ 77%/68%/67% | 2026-04-26 | summary77%(252/324)・phase68%・imageUrl67%。storyTimelineは個別topic/{id}.jsonに格納。公開トピック数が324件に増加し比率が一時低下。4x/day自動実行で継続改善中 |
...
| アフィリエイト収益化基盤 | ✅ 実装済み | 2026-04-26 | privacy.html更新・topic.htmlウィジェット枠。AFFILIATE_AMAZON_TAG/RAKUTEN_IDをconfig.jsに設定するだけで稼働 |
| お問い合わせフォーム | ✅ 実装済み | 2026-04-26 | contact.html API連携完了（POST /contact → p003-contact Lambda）。SES検証待ち（ナオヤ手動作業必要） |
...
| 静的SEO HTML生成 | ✅ 本番稼働 | 2026-04-26 | topics/{tid}.html 500件生成済み・CF配信確認済み。サイトマップ200件/topics/URLに更新。regenerateStaticHtml/regenerateSitemapモード追加 |
| ジャンル グルメ/ファッション追加 | ✅ 完了 | 2026-04-26 | GENRE_KEYWORDS追加・RSSフィード追加 |
| ジャンル くらし/社会 フィルター | ✅ 完了 | 2026-04-26 | app.js・legacy.html GENRESリストに追加（61件/13件が未表示だった） |
...
| アフィリエイト Yahoo!ショッピング | ✅ 完了 | 2026-04-26 | もしもAFF経由でAmazon/楽天/Yahoo!の3店舗対応。AFFILIATE_MOSHIMO_A_ID設定で即稼働 |
| AI拒否応答フィルタ | ✅ 完了 | 2026-04-26 | generate_titleで40文字超・拒否語句を含む無効応答を除外 |
| モバイル広告CSS修正 | ✅ 完了 | 2026-04-26 | transform-origin:top center+flexbox centeringで広告左はみ出し修正（T005） |
| クラスタリング重複改善 | ✅ 完了 | 2026-04-26 | SYNONYMS拡充（銃撃/発砲等）+5文字以上カタカナ固有名詞1単語共有で結合（T006） |
| AIサマリー固有名詞文脈説明 | ✅ 完了 | 2026-04-26 | 全3プロンプトに「初出時括弧説明」ルール追加（T007）|
| 広告記事フィルタ強化 | ✅ 完了 | 2026-04-26 | ランキング/徹底比較/アフィリエイト/PRパターン追加（T009）|
| 長期停滞トピックarchived化 | ✅ 完了 | 2026-04-26 | lifecycle: 30日超lastArticleAtは速度スコアに関係なく強制inactive（T008）|
| get_topic_detail無制限クエリ修正 | ✅ 完了 | 2026-04-26 | get_item(META)+query(Limit=30/90)に分割。DynamoDB月$10超の読み取りコスト削減（T011）|
| Bluesky 3問題修正 | ✅ 完了 | 2026-04-26 | S3 topics.json読み取り（DynamoDBスキャン廃止）・velocityScore float化・静的URL+存在確認（T013）|
| S3 ETag差分書き込み | ✅ 完了 | 2026-04-26 | proc_storage.py: MD5 ETag比較でJSON/HTML変更なし場合はPUTスキップ。月$1.98のS3コスト削減（T012）|
| クラスタリング高度化 | ✅ 完了 | 2026-04-26 | 3文字以上カタカナ固有名詞1語共有で緩和結合・chunk_sim関数（漢字/カタカナ連続抽出）呼び出し追加|
| fetcher O(n²)削減 | ✅ 完了 | 2026-04-26 | topics_active[:1000]→[:500]・find_related_topics/detect_topic_hierarchy を転置インデックス化（O(n²)→O(n·k)）。Duration 229秒→目標60秒|
