...
| T2026-0430-A | 🔴 高 | AI品質 | **keyPoint 平均長 35.6字 → 200字超化（フェーズ2 最優先）** — 2026-04-30 07:10 JST p003 巡回 (topics.json 公開92件) 実測: keyPoint any-string 100% / **>=100字 充填率 2.2% (2/92)** / **平均長 35.6字** / プロンプト要求 200〜300字に対し1/6。前回 (2026-04-28 PM) 平均43.8字 → 今回35.6字で**さらに悪化**。仮説: ①proc_ai.py で生成失敗時のフォールバックが「title or 1文目」で切り詰めている、②プロンプトの指示が無視され短文応答で確定している、③スキーマ validation が長さチェックしていない。次手: (a) proc_ai.py の keyPoint 抽出フォールバック箇所を grep して短文許容パスを特定、(b) 生成テスト 5件で実応答長を測定、(c) <100字を Claude が返したらリトライ or fallback マーク、(d) skip 条件に lengthShortfall を追加して quality_heal で再処理対象化。**コスト試算必須**: 1068件×Claude Haiku $0.0023/件 ≈ $2.5（一括再処理 1 回）。承認後実施。**フェーズ2 完了条件 70% 超** に対する最大ギャップ。 | lambda/processor/proc_ai.py, lambda/processor/proc_storage.py, scripts/sli_keypoint_fill_rate.py | 2026-04-30 |
| T2026-0430-B | 🟢 観測 | AI品質 | **perspectives 充填率 4.31% → 40.2% に改善（観測のみ・追加対応なし）** — 2026-04-30 07:10 JST 実測 perspectives 充填 37/92=40.2%。前回 (2026-04-28 PM) 4.31% から大幅改善。outlook も 100% 充填で安定。要因仮説: T2026-0429 系の修正で perspectives 抽出が landing。**追加対応なし・次回巡回で 60% 超を確認**。 | (観測のみ) | 2026-04-30 |
