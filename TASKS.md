...
| T2026-0502-SEC4 | 🟡 中 (Anthropic 完了・Notion のみ残存) | **ローカル平文ファイル内 secrets — Anthropic は ✅ rotate 完了 (PO 報告 2026-05-02 PM・SEC1 と同 key)・Notion のみ残存** — ~~①`setup_api_key.sh:5` に Anthropic API key (prefix `sk-ant-api03-sKdL...`)~~ → SEC1 と同じ key で Revoked + 新規発行済み。**残存タスク**: ②`fetch_notion.py:12` の Notion integration token (prefix `ntn_3865...`) — Notion Settings → Integrations で Revoke + 新規発行。本ファイルは PR #200 で env 変数読み込み版に書換済 (current HEAD には full token 無し)。完了条件: 旧 Notion token で API 401 (新 token が必要なら `export NOTION_TOKEN=ntn_xxx` で fetch_notion.py 動作確認)。**Phase-Impact: セキュリティ** / **Eval-Due: 2026-05-09** | Notion Settings | 2026-05-02 |
