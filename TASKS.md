...
| T2026-0429-P | 🔴 高 | コスト | **コスト削減（$8-16/日 → $0.50/日 目標）+ 要約なしトピック下位固定** — (1) proc_ai.py full mode を Sonnet → Haiku に切替。(2) MAX_API_CALLS 30→20。(3) **fetcher→processor 即時トリガーは例外含め完全廃止**（速報例外も実装しない。判定ロジックバグによるコスト爆発リスクを排除）。processor の起動経路は EventBridge 5:30/17:30 のみ。(4) deploy.sh EventBridge cron を `cron(30 20,8 * * ? *)`（5:30/17:30 JST）に変更。(5) app.js sort で `hasAI` (aiGenerated && keyPoint≥50字) でグループ分けし、要約ありが必ず上位、要約なしは下位に固定。各グループ内は既存の decayed score。**T2026-0429-I を吸収**。**手動適用必須**: deploy.sh は CI で実行されないため、merge 後ナオヤが `aws events put-rule --name p003-processor-schedule --schedule-expression "cron(30 20,8 * * ? *)" --state ENABLED --region ap-northeast-1` を一度実行すること（または次回 deploy.sh 実行時に反映）。 | lambda/processor/proc_ai.py, lambda/processor/proc_config.py, lambda/processor/handler.py, lambda/fetcher/handler.py, frontend/app.js, projects/P003-news-timeline/deploy.sh | 2026-04-29 |
