...
| T2026-0501-TS | 🔴 高 | バグ修正 | **detail.js timestamp フォーマット統一 + Unix秒混入 SLI** — POの「スナップショットのタイミングがおかしい (cc7f78cd55b81da6)」報告への根本対応。`new Date(s.timestamp).getTime()` を生で呼ぶと Unix 秒整数 (publishedAt) が 1970 年代に解釈される事故を解消。①`frontend/js/timestamp.js` 新設 (`toMs(v)`：ISO/Unix秒/Unixms/Date を ms に正規化、1e9<n<1e12 を秒として扱う物理境界 + `_warnBadTs(label,raw,ms)`：1970年代/5年超未来を console.warn) ②detail.js の全 `new Date(timestamp系)` を `_toMs()` 経由に置換 (snap.timestamp フィルタ / 集約 / sort / fmtTl / fmtDay / publishedAt → ms / fmtElapsed / iso (JSON-LD) / discovery sort) ③boundary test 40 ケース新設 (null/0/Unix秒/Unixms/ISO/Date/未来/NaN/Infinity/壊れた文字列) ④`scripts/sli_timestamp_check.sh` (jq + curl で公開 topics.json から ISO 想定フィールド (predictionMadeAt/createdAt/storyTimeline.at) に Unix 秒整数が混入していないか検出) ⑤`.github/workflows/timestamp-sli.yml` (週次月曜 08:30 JST cron, bad>=1 で warn / >=5 で error → Slack)。Verified: detail.js:toMs:added (210/210 unit pass)。 | projects/P003-news-timeline/frontend/detail.js, projects/P003-news-timeline/frontend/js/timestamp.js, projects/P003-news-timeline/frontend/topic.html, projects/P003-news-timeline/tests/unit/timestamp.test.js, projects/P003-news-timeline/package.json, scripts/sli_timestamp_check.sh, .github/workflows/timestamp-sli.yml | 2026-05-01 |
| T2026-0501-OL2 | 🔴 高 | AI品質 | **outlook 影響先マッピング + 予想深度改修** — Flotopic ビジョン「情報の地図」の核心。ニュースが世界のどこに飛び火するかの連鎖が見えて初めて読者が「わかった」と感じる。①**影響先マッピング (`_GENRE_IMPACT_TARGETS`)**: ジャンル別の「波及先カテゴリ」を辞書定義し outlook prompt に注入 — 経済/株 → 関連業種・ドル円・長期金利・日銀政策 / 国際 → 日本の輸出企業・円相場・エネルギーコスト / 政治 → 内閣支持率・規制業界・選挙・予算。「このトピックが波及しうる先を1〜2個特定してから予想を書け（例: 円ドル相場・輸出企業株価・与党支持率）」を追加注入。②**予想の連鎖深度強制**: prompt に「明らかな1次効果（誰でもわかること）は省略し、2次・3次の連鎖（日本政府の対応・日銀の判断・関連市場の反応）を具体的に予想せよ」追加。例: 原油高は「物価が上がる」ではなく「日銀が〇〇する可能性、その場合円は〇〇円台」まで踏み込ませる。読者ペルソナ (T2026-0501-G) と組み合わせて「誰に・どう波及して・何が起きるか」の三層を outlook 一文に圧縮。 | projects/P003-news-timeline/lambda/processor/proc_ai.py, projects/P003-news-timeline/tests/test_outlook_prompt.py | 2026-05-01 |
