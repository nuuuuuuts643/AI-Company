...
| T2026-0502-U | 🟡 中 (技術調査・PR は調査結果次第) | アーキ | **fetcher クラスタリングを embedding ベースに移行する技術調査 (Haiku merge judge を hot path から外す)** — 2026-05-02 13:30 JST PO 指示 (「そもそもなんでこんな構成なのか謎」「高すぎる」)。背景: 現状 fetcher は bigram Jaccard クラスタリング + borderline ペア (0.10〜0.35) に対して同期で Haiku 判定 (1100 pairs/run・月$380)。bigram は意味類似の指標として弱いため false split が起き、それを LLM で穴埋めしている構造。**業界標準 (Google News 等) は sentence embedding + コサイン類似度** で LLM を hot path から外す。**調査範囲**: ①Haiku 系列の embedding API (`voyage-3-lite` 等) または日本語 sentence-transformers (multilingual-e5-small) の精度・レイテンシ・コスト比較 (記事1本あたり) ②Lambda layer に sentence-transformers を載せる zip サイズ実測 (250MB 制限) ③DynamoDB に embedding を 768次元 base64 で持たせるストレージコスト試算 ④移行計画の段階分け案 (現行と並走 → A/B → 切替)。**完了条件**: `docs/p003-embedding-migration-research.md` を新設し、上記4項目に数値で答え + 推奨案 + リスク (false merge 増の可能性) を書く。**実装はこの調査結果次第で別タスク起票**。**Phase-Impact: 2 E2-3 クラスタリング品質** / **Eval-Due: 2026-05-09 (調査ドキュメント完成)** | docs/p003-embedding-migration-research.md (新規・調査結果) | 2026-05-02 |
