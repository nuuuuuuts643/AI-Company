...
|---|---|---|---|---|
| T2026-0502-A | 🟠 高 | **freshness-check + fetcher-health-check が false-failure (フェーズ1 観測基盤の劣化)** — fetcher Lambda 自体は健全 (CloudWatch メトリクス FetcherSavedArticles 過去3h 178件 / topics.json 鮮度 27.7分) だが、両 SLI workflow が連続 failure を返す。失敗ステップ: freshness-check「fetch topics.json と updatedAt 解析」 / fetcher-health-check「get-metric-statistics と判定」。ローカル sandbox で同 script を再現実行 → success (DIFF_MIN=2, STATUS=fresh)。手動 dispatch (2026-05-02 08:07 JST = run 25237008550 / 25237009244) でも failure 再現。**ユーザー被害なし** だが「監視を信用できない」状態 = 本物の障害が来たとき見分けがつかない (狼少年問題)。Cowork は proxy allowlist で Azure Blob (CI ログ保管先) アクセス不可 → Eng Claude が `gh run view --log 25237008550` で根本原因特定する必要あり。完了条件: 両 workflow が連続 2 回 success。Verified-Effect: 両 workflow の最新 schedule run の conclusion=success が 2 回続く。 | `.github/workflows/freshness-check.yml`, `.github/workflows/fetcher-health-check.yml`, `scripts/ci_freshness_diff_min.py`, `scripts/ci_cw_sum_datapoints.py` | 2026-05-02 |
| T2026-0502-B | 🟡 高 | **flotopic-lifecycle Lambda SK FilterExpression error** — p003-haiku 22:13 UTC alert: `ValidationException: Filter Expression can only contain non-primary key attributes: Primary key attribute: SK` が直近24h で複数回発生 (`handler.py:64` `delete_snaps` / `:93` `delete_old_snaps`)。SK は `KeyConditionExpression` で指定すべきところ `FilterExpression` に入っている。完了条件: lifecycle Lambda が直近 24h ERROR ゼロ。Verified-Effect: CloudWatch logs の ValidationException カウント=0。 | `lambda/lifecycle/handler.py` (line 64, 93 周辺) | 2026-05-02 |
