...
| T2026-0429-G | 🟡 中 | AI品質 | **storyPhase 発端率 改善観測 (articleCount≥3 で 7.69%・目標達成)** — 14:13 JST p003 巡回: topics.json 109件中 articleCount≥3 は 52件・うち発端 4件 = **7.69%** (目標 10%未満 ✅)。前回 50.96% → 今回 7.69% に劇改善。要因仮説: T2026-0429-H ゴーストID 駆除で公開対象から発端固定の幽霊が除去された + 公開フィルタ articleCount≥3 が効いた。**未解決の懸念**: 全109件で見ると発端 53/109=48.6% と高いが、これは articleCount<3 の単発トピックが「発端」初期値のまま放置されている可能性 (記事追加されないと storyPhase 更新トリガーが走らない)。次手: storyPhase 更新トリガー (proc_storage.py) を再確認し、articleCount=1 のままの発端も時間経過で「-」or「拡散しない」に収束させるべきか方針判断要。優先度を 🔴高→🟡中 に降格。 | lambda/processor/proc_ai.py, lambda/processor/proc_storage.py | 2026-04-29 |
| ~~T2026-0429-I~~ | — | — | ~~keyPoint 100字未満トピックのランキング降格~~ → **T2026-0429-P に吸収・拡張**（hasAI グループ分けで根治） | — | 2026-04-29 |
| T2026-0429-K | 🔴 高 | 体験 | **Bluesky 投稿品質改善** — 同一トピックの重複投稿・古いトピックの投稿が発生している。対応: (a) 過去投稿済みトピックの重複チェックロジック確認（DynamoDB bluesky_posted_at or posted フラグ） (b) 投稿対象の選定ロジック確認（新着性スコアに基づいて選ぶか、単に上位N件か）(c) 「最後に投稿した時刻から X 時間以内に同一 topicId を再投稿しない」ガードを追加 (d) 投稿候補は publishedAt が 24 時間以内 or velocityScore が直近上昇トレンドのもののみに絞る | lambda/processor/bluesky.py or 相当ファイル | 2026-04-29 |
| T2026-0429-L | 🔴 高 | AI品質 | **velocityScore / スコア減衰の検証・修正** — 古いトピックが「急上昇」として上位表示されている（ナオヤ確認）。スコア減衰が機能していない可能性。対応: (a) proc_storage.py or app.js の velocityScore 計算ロジック確認（時間経過でスコアが下がるか） (b) 減衰係数・半減期の設定値確認 (c) 古いトピック（publishedAt > 48h）が上位に出ないよう時間減衰を強化。目標: publishedAt から 48h 経過したトピックの表示順は新着トピックに比べ大幅に下がる | lambda/processor/proc_storage.py, frontend/app.js | 2026-04-29 |
| T2026-0429-M | 🔴 高 | 基盤 | **SLIフィールド ↔ 実装スキーマ 乖離検出CI（物理ガード）** — situation 0% 問題の再発防止。仕組み: `scripts/check_sli_field_coverage.sh` 新設。freshness-check.yml で測定しているフィールド名を抽出し、proc_ai.py の出力スキーマ（実際に生成するフィールド）と突合。不一致があれば CI ERROR。ルールとの二重縛り: bug-prevention.md にも「SLIフィールドは実装スキーマと一致させる・CI物理検証」を追記。ユーザー体験: 「数字は通っているがユーザーには何も届いていない」状態を事前に防ぐ。 | scripts/check_sli_field_coverage.sh, .github/workflows/ci.yml, docs/rules/bug-prevention.md | 2026-04-29 |
| T2026-0429-N | 🔴 高 | 基盤 | **PRテンプレート改善 + 係数・パラメータ根拠必須化** — 背景: 「なぜこの実装が必要か」「このPRで何の問いに答えられるようになるか」「将来これが正しかったと言えるか」を考えずに実装が進む。対応: (a) `.github/pull_request_template.md` に必須フィールド追加: 「このPRで答えられる問い」「将来の検証方法」「係数・パラメータを使った場合は実測根拠」 (b) 係数・閾値を含むPRはCI lint で「実測値コメント」の存在チェック（`# measured:` プレフィックスの行）。目的: 未来の自分or他人が「これは正しかったのか」を判断できるように。ユーザー体験への接続: PR説明が充実することで、正しい問いに答えている実装だけがマージされる。 | .github/pull_request_template.md, scripts/check_param_basis.sh, .github/workflows/ci.yml | 2026-04-29 |
| T2026-0429-P | 🔴 高 | コスト | **コスト削減（$8-16/日 → $0.50/日 目標）+ 要約なしトピック下位固定** — (1) proc_ai.py full mode を Sonnet → Haiku に切替。(2) MAX_API_CALLS 30→20。(3) fetcher→processor 通常時即時トリガー廃止 → 「直近30分で記事3件以上の新規トピック」かつ「AI要約なし」の速報例外時のみ invoke (maxApiCalls=2)。(4) deploy.sh EventBridge cron を `cron(30 20,8 * * ? *)`（5:30/17:30 JST）に変更。(5) app.js sort で `hasAI` (aiGenerated && keyPoint≥50字) でグループ分けし、要約ありが必ず上位、要約なしは下位に固定。各グループ内は既存の decayed score。**T2026-0429-I を吸収**（keyPoint penalty では不十分なためグループ固定で根治）。**手動適用必須**: deploy.sh は CI で実行されないため、merge 後ナオヤが `aws events put-rule --name p003-processor-schedule --schedule-expression "cron(30 20,8 * * ? *)" --state ENABLED --region ap-northeast-1` を一度実行すること（または次回 deploy.sh 実行時に反映）。 | lambda/processor/proc_ai.py, lambda/processor/proc_config.py, lambda/processor/handler.py, lambda/fetcher/handler.py, frontend/app.js, projects/P003-news-timeline/deploy.sh | 2026-04-29 |
...
| ID | 優先 | 内容 | 変更予定ファイル | 追加日 |
|---|---|---|---|---|
| T2026-0428-AV | 中 | **トピックカードに注目スコアを表示する** — ナオヤフィードバック: スコアをカードに表示するアイデアが面白い。トップページのトピックカード (`frontend/app.js` の renderTopicCard 系) に、AI が計算した注目度スコア (`velocityScore` 等) を数値またはバーで表示する。フェーズ1 の足回り安定が完了してから着手 (UI 改善は後回し方針)。実装時は ①どのスコアを使うか (velocityScore / freshness / 記事数) を明確化 ②表示形式 (数値/バー/バッジ) を決定 ③モバイル 375px で崩れないこと を必須。 | `frontend/app.js`, `frontend/style.css` | 2026-04-28 |
