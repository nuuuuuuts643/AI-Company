...
| T2026-0429-H | 🔴 高 | AI品質 | **Lambda proc_ai 停止調査・修正** — 12:05 JST 以降 topics-full.json が更新停止。新規記事の要約（keyPoint 等）が生成されていない（admin.html でも確認）。昨日分の記事も未処理の可能性。対応: (a) CloudWatch Logs で p003-processor の最終 invocation とエラーを確認 (b) EventBridge スケジュール (JST 02:00/08:00) の有効状態を確認 (c) エラー原因を特定して修正 (d) 手動 invoke または次回定時実行で topics-full.json が更新されることを確認。優先度: ユーザー体験直撃のため最高。 | lambda/processor/handler.py, proc_ai.py, proc_storage.py | 2026-04-29 |
