...
| T2026-0430-I | 🟡 中 | ルール物理化 | **CLAUDE.md に Monitor 禁止 / CI 待ち即クローズ ルール追記** — コードセッションが「PR出した後に Monitor で CI 完了を待つ」誤動作を繰り返していた (`docs/rules/global-baseline.md §10` 違反 = 時間待ち確認はスケジューラーに渡す)。CLAUDE.md「Cowork ↔ Code 連携ルール」セクションに「PR を出したらコードセッションを即 exit / Monitor ツールでの CI ポーリング禁止 / 違反例 (CI 待ち中 Monitor / gh pr checks ループ)」を追記。観測のみ (commit-msg hook 対象外)。PR #60。 | CLAUDE.md | 2026-04-30 |
| T2026-0430-J | 🔴 高 | AI品質 | **keyPoint 短文率 77.7% (87/112) の根本原因修正 — fetcher_trigger backfill 末尾追加 bug** — 調査: 新 retry 機構 (T2026-0430-A) は機能しており本日生成分 10/10 は ≥100字。問題は 2026-04-26〜29 の旧 87 件が再処理されないこと。原因: handler.py L205 の T237 backfill が `pending.extend()` で末尾追加、`MAX_API_CALLS=10` (fetcher_trigger override) で 5 topic 処理時 loop break → 末尾 backfill が永久未処理。修正 (handler.py): rescue を `KP_RESCUE_PER_RUN=2` 件固定、pending **先頭** に挿入 (`rescue_added + pending`)。これで 30 分ごとの fetcher_trigger run で 2 件確実に消化 → 87 件は ~1 日で全消化見込み。コスト中立 (既存 budget 内)。tests 7 ケース rewrite + 全 98 ケース pass。 | projects/P003-news-timeline/lambda/processor/handler.py, projects/P003-news-timeline/tests/test_handler_fetcher_backfill.py | 2026-04-30 |
