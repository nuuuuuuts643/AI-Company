...
| T2026-0502-SEC3 | 🟡 中 (rotate 完了・history rewrite のみ残存) | **git history 内 commit 済み live secrets — token rotate は ✅ 完了 (PO 報告 2026-05-02 PM)・git history rewrite のみ残存** — ~~①GitHub PAT (prefix `ghp_LyAq...`) ②別 GitHub PAT (prefix `ghp_JPS5...`) ③Slack Bot Token (prefix `xoxb-89706414...`) ④Slack Webhook (path `T08UJJVCFJ4/B0AUJ9K64KE/...`)~~ → 全て **Revoked + 新規発行済み** (PO 報告 2026-05-02)。**残存タスク**: git history rewrite (`git filter-repo --replace-text` または BFG) で旧 token を history からも消去 + force-push。**重要度低下理由**: 旧 token が API 401 を返す = 攻撃者が history から拾っても利用不可。ただし「過去に流出した事実」記録としては残るため、git filter-repo で消すのが望ましい。完了条件: `git log --all -p \| grep -E "ghp_LyAq\|ghp_JPS5\|xoxb-8970641423616-10995291739264\|B0AUJ9K64KE"` が 0 件。**Phase-Impact: セキュリティ (defense in depth)** / **Eval-Due: 2026-05-16 (緊急性下がった・余裕あるとき)** / **手順例**: `pip install git-filter-repo` → `git filter-repo --replace-text <(echo -e 'ghp_LyAq...==>REDACTED\nghp_JPS5...==>REDACTED\nxoxb-89706414...==>REDACTED')` → `git push --force-with-lease origin main` → 全コラボの `git fetch + reset --hard origin/main` 周知 | `git filter-repo` (history rewrite) | 2026-05-02 |
