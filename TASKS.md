...
| T2026-0502-L | 🟡 中 | **Bluesky 投稿頻度 恒久対処（debut 設計欠陥修正・SSoT 化）** — 5/1 に debut が 48件/日投稿の事故。応急処置 `DEBUT_MAX_PER_RUN=0` キルスイッチを設計レイヤーに昇格: `BLUESKY_POSTING_CONFIG` 辞書を SSoT 化、`_check_rate_limit(mode)` を全 post_xxx() の唯一エントリ化、`enabled` / `cooldown_hours` / `max_per_24h` の3重ガード、テスト14件、lessons-learned 追記。完了条件: PR merge + Lambda deploy 後の `flotopic-bluesky` 起動で debut skip 理由ログ出現。Verified-Effect: 2026-05-03 朝に5/2 投稿件数が ≤4件/日 (PO目標)。**Phase-Impact: 1 運用安定化 / Eval-Due: 2026-05-03 09:00 JST** | `scripts/bluesky_agent.py`, `projects/P003-news-timeline/tests/test_bluesky_rate_limit.py`, `docs/lessons-learned.md` | 2026-05-02 |
...
| T2026-0502-PIPE-FIX-1 | 🔴 高 | **session_bootstrap.sh git pull/push エラー隠蔽パターン修正 (T2026-0502-PIPE-AUDIT 検出)** — grep audit T2026-0502-PIPE-AUDIT で特定された実害パターン。`scripts/session_bootstrap.sh:170/172` で `git pull/push 2>&1 | _strip_fuse_noise | tail -2 || true` により git コマンド error が output 末尾 2 行に入らないと silent に吸収される。5/2 朝の bootstrap 失敗（`git pull failed (rc=128)`）もこのパターンで原因が隠蔽されていた（T2026-0502-M 根本原因）。修正内容: tail 削除 + exit code tee による error tap。修正案: `git pull ... 2>&1 | tee >(head -2) | tail -n+1 || true` または error handler 分岐で rc を参照。完了条件: PR merge 後の `bash scripts/session_bootstrap.sh` で git error が stderr に出現・bootstrap ログに「git failed」が visible化。Verified: `git pull failed` が今後見える状態（エラー追跡可能化）。**Phase-Impact: 1 運用安定化（隠蔽排除）** / **Eval-Due: 2026-05-03** | `scripts/session_bootstrap.sh` | 2026-05-02 |
...
| T2026-0502-J | 🔴 高 (PO 削除判断・Code dispatch 必要) | **fetcher → processor 即時トリガー削除 (コスト 67% 減)** — 2026-05-02 10:38 JST Cowork 実測訂正: 前回 J を「設計通り・close」と片付けたが、コスト観点で再評価。CloudWatch Logs Insights で過去24h (5/1 09:00〜5/2 09:00 JST) の Anthropic API call (`[METRIC] claude_cache` log count) = **164 calls/24h**。内訳: 定期 cron 2回 (5:30/17:30 JST) で 53 calls (32%)・**fetcher trigger 由来 111 calls (68%)**。月コスト試算 +$17/month の想定外支出。**PO 判断 (2026-05-02 10:40 JST)**: 削除確定。レイテンシ trade-off (平均 30min → 平均 6h・最悪 12h) は news 集約サイトとして許容。後でレイテンシ短縮が必要になったら cron を 2x/day → 4x/day に増やす方針。**修正内容**: `lambda/fetcher/handler.py:1498-1517` の `if new_pending: ... _lambda.invoke('p003-processor', ...)` ブロック全体を削除 + 削除理由コメント追加。完了条件: PR merge + 24h 後の API call が <60 calls/24h まで減少 (-63% 想定)。Verified-Effect: 月次 Anthropic 課金が前月比 -$15 以上。**Phase-Impact: 1 運用安定化 (コスト)** / **Eval-Due: 2026-05-03 09:00 JST (24h 実測)** | `lambda/fetcher/handler.py` | 2026-05-02 |
