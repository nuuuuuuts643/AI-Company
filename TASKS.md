...
| T2026-0430-A | 🔴 高 | AI品質 | **keyPoint 平均長 35.6字 → 200字超化（フェーズ2 最優先）** — 2026-04-30 07:10 JST p003 巡回 (topics.json 公開92件) 実測: keyPoint any-string 100% / **>=100字 充填率 2.2% (2/92)** / **平均長 35.6字** / プロンプト要求 200〜300字に対し1/6。前回 (2026-04-28 PM) 平均43.8字 → 今回35.6字で**さらに悪化**。仮説: ①proc_ai.py で生成失敗時のフォールバックが「title or 1文目」で切り詰めている、②プロンプトの指示が無視され短文応答で確定している、③スキーマ validation が長さチェックしていない。次手: (a) proc_ai.py の keyPoint 抽出フォールバック箇所を grep して短文許容パスを特定、(b) 生成テスト 5件で実応答長を測定、(c) <100字を Claude が返したらリトライ or fallback マーク、(d) skip 条件に lengthShortfall を追加して quality_heal で再処理対象化。**コスト試算必須**: 1068件×Claude Haiku $0.0023/件 ≈ $2.5（一括再処理 1 回）。承認後実施。**フェーズ2 完了条件 70% 超** に対する最大ギャップ。**[obs 2026-04-30 19:01 JST 自動巡回]** PR #36 (フェーズ判定方式) マージ後の最初の SLI 観測: topics.json 公開92件で >=100字 充填率 **2.2% (2/92・据え置き)** / 平均長 **35.6字 (据え置き)** / >=50字 6.5% / 空 0%。**🔴 緊急基準 (10%未満) に該当**。PR #36 マージ済だが Lambda 巡回 (毎時 :09) は 19:09 で発火予定 → 反映前の数値の可能性あり。次回観測 (20:09 以降) で >=100字 充填率が変動しなければ T-keypoint-prompt-v2 (CloudWatch logs で新プロンプト適用確認 + プロンプト強度 MUST/REQUIRED 強化 + schema min length 物理制約) を起票する。 | lambda/processor/proc_ai.py, lambda/processor/proc_storage.py, scripts/sli_keypoint_fill_rate.py | 2026-04-30 |
