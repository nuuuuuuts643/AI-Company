...
|---|---|---|---|---|---|
| T263 | 🔴 最優先 | 安定性 | **topics.json 鮮度 SLI モニタ実装** — 本番 topics.json が 14h 鮮度ゼロのままサイクル空振り (2026-04-28 17:15 観測)。CloudWatch アラームは Lambda 個別失敗のみで「success だが publish しなかった」を取れない。**仕組み的対策**: ① 1h ごとに `curl https://flotopic.com/api/topics.json` を取得し `(now - updatedAt) > 90min` なら Slack 警告する独立 Lambda を新設。② 監視は governance/processor とは別系統で外部 GH Actions cron で curl する。③ 警告 Slack に「最終成功時刻・次回スケジュール時刻・直近 invoke ログURL」を貼る。T242 と統合済み。 | 新規 `lambda/freshness_monitor/` または `.github/workflows/freshness-check.yml` | 2026-04-28 |
...
| T251 | 高 | **HSTS / X-Frame-Options / Permissions-Policy が CloudFront response headers に無い** — `curl -I https://flotopic.com/` のヘッダに `strict-transport-security` / `x-frame-options` / `permissions-policy` が全く無い。CloudFront に Response headers policy を新設し、すべてのレスポンスに付与 (Naoya コンソールで設定する案件)。 ※2026-04-28 ラウンド3 で T251 セキュリティヘッダ「全付与済」と書かれていたが要再確認。 | CloudFront 設定 (Naoya 確認案件), `scripts/security_headers_check.sh` 新規 | 2026-04-28 |
...
| ID | 優先 | 内容 | 変更予定ファイル | 追加日 |
|---|---|---|---|---|
| T231 | 中 | **推移グラフの長期ボタンがデータ蓄積前から押せる** — 本番運用 2026-04-20 開始のためデータ実測は 8 日分しかないが、`1ヶ月/3ヶ月/半年/1年/全期間` を押すと「データ蓄積中」が出るだけ。修正方法: トピックの最古 SNAP 日時を取得し、その期間に届いていないボタンは `disabled` グレーアウト or 非表示に。※実装済の可能性あり (HISTORY 確認要)。 | `frontend/topic.html`, `frontend/detail.js` | 2026-04-28 |
| T232 | 中 | **「関連記事」h2 がデータ0件でも表示される** — `frontend/topic.html:374-377` で関連記事 0 件でもカード全体表示。CLAUDE.md ルール「データが0件またはロード失敗の場合、h2 ヘッダーを含むカードごと `display:none`」に違反。※実装済の可能性あり (HISTORY 確認要)。 | `frontend/topic.html`, `frontend/detail.js` | 2026-04-28 |
| T233 | 中 | **AI分析「処理待ち」表示に次回更新予定時刻が無い** — CLAUDE.md「実装前ユーザー文脈チェック」③で「いつ来れば読めるか」を必ず表示するルール明示済みだが未対応。現在時刻と JST 01/07/13/19 の境界を計算し「次の更新: 13:00 JST」を併記。※実装済の可能性あり (HISTORY 確認要)。 | `frontend/detail.js` | 2026-04-28 |
| T234 | 中 | **ヘッダーキャッチコピーがページ間で微妙に違う** — privacy.html h1「📰 Flotopic / 話題の流れをAIで追う」(絵文字付き・古いコピー)、terms.html 同左。全ページのヘッダーロゴ要素を統一テンプレート化し「Flotopic / 大きな流れを、1分で。」に揃える。※実装済の可能性あり (HISTORY 確認要)。 | `frontend/privacy.html`, `frontend/terms.html`, `frontend/about.html` | 2026-04-28 |
...
| ID | 優先 | 内容 | 変更予定ファイル | 追加日 |
|---|---|---|---|---|
| T239 | 中 | **ads.txt と index.html pub-id の整合性 CI チェックが未実装** — CLAUDE.md「ads.txt 欠落 なぜなぜ分析」の仕組み的対策2「CIチェック追加候補」に明記済みだが未実装。`.github/workflows/ci.yml` に shell スクリプトを追加: `index.html` から `client=ca-pub-([0-9]+)` を抽出し、`frontend/ads.txt` に対応行が存在するか grep。忍者AdMax の `data-shinobi-id` も同様。 | `.github/workflows/ci.yml`, `frontend/ads.txt` | 2026-04-28 |
...
| ID | 優先 | 内容 | 変更予定ファイル | 追加日 |
|---|---|---|---|---|
| T243 | 高 | **TASKS.md タスクID 同日衝突問題** — 別セッションがタスクIDを「最大値+1」で振る運用だが、Cowork × Cowork でも 同時刻ほぼ並列に動くため衝突する。`scripts/next_task_id.sh` を作り `git pull` 後に発番させる + CI で重複検出。※実装済 (2026-04-28 ba850ef commit で対応)。HISTORY 確認要。 | `scripts/next_task_id.sh` 新規, `.github/workflows/ci.yml` チェック追加 | 2026-04-28 |
| T244 | 高 | **WORKING.md `needs-push` カラム追加** — 「Cowork で実装→push 失敗→Code 起動まで滞留」の事故を物理ゲートで防ぐ。WORKING.md に `needs-push` カラムを追加し、Cowork は code ファイルを編集する時に必ず `yes` を立てる。Code セッション起動時の起動チェックスクリプト末尾で `grep "needs-push.*yes"` を実行。※実装済 (2026-04-28)。HISTORY 確認要。 | `WORKING.md` 構造変更, `CLAUDE.md` 起動チェック script 拡張 | 2026-04-28 |
...
| ID | 優先 | 内容 | 変更予定ファイル | 追加日 |
|---|---|---|---|---|
| TBD-SLI | 中 | **`docs/sli-slo.md` 新設** — トップレベル SLI 5-7 個 (topics.json 鮮度・トップページ HTTP/2 200 / TTL Cache hit / fetcher Lambda 成功率 / 月次 AdSense クリック率 等) を列挙し、各 SLI の警告閾値・観測コマンド・再現可能 curl/jq を併記。T263 実装と並行で進める。 | `docs/sli-slo.md` 新規 | 2026-04-28 |
