...
| T2026-0502-Y | 🟡 高 (要 Code セッション・コスト規律物理化) | **CLAUDE.md「コスト規律ルール」を思想 → 物理に昇格** — 2026-05-02 14:25 JST PO 指示「物理ガードできないのかそれ」。背景: 本セッションで Cowork が `aws lambda invoke` 2回 + `cloudwatch get-metric-statistics`・`logs get-query-results` 連投 (>15 calls / 30min) で「効果検証 polling 禁止」「AWS list/get も連投禁止」「失敗N回ルール」の思想ルール 3 つを違反。CLAUDE.md「コスト規律ルール」表 4 項目すべて『強度: 思想』のまま。**物理化案**: ①(短期・半物理) `scripts/cowork_session_aws_count.sh` を新設 — session_info MCP の transcript を grep して AWS MCP call 数 / 1セッション を集計。session_bootstrap.sh が起動時に「前回セッション AWS API 呼び出し N 回 (閾値超過)」を ERROR 表示。②(中期・真物理) AWS MCP server (`mcp__awslabs_aws-mcp-server__call_aws`) を fork して rate-limit hook を追加: 同一セッション内 `aws lambda invoke` 2 回目以降を refuse、`logs get-query-results` を 5回 / 30min に throttle。③(おまけ) IAM Policy で Cowork ユーザーに `lambda:InvokeFunction` の Conditions 制限を付ける (rate quota は AWS native ではサポート外なので CloudTrail + EventBridge alarm で sidekick 監視)。**完了条件**: ①が landing して session_bootstrap.sh が前回 violation を ERROR で表示すること。Verified-Effect: 試しに連投して bootstrap が ERROR を出すこと。**Phase-Impact: 1 運用安定 (規律物理化)** / **Eval-Due: 2026-05-09** | `scripts/cowork_session_aws_count.sh` (新設), `scripts/session_bootstrap.sh`, `CLAUDE.md` (コスト規律ルール表の強度を 思想→物理 に書換) | 2026-05-02 |
