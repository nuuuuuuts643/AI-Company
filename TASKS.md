...
| ~~T2026-0502-I~~ | ~~🟢 低 (Code セッション dispatch 必要)~~ | ~~**API Gateway 廃止 route `POST /track` 削除**~~ — 2026-05-02 10:32 JST 調査 (Cowork): `aws apigatewayv2 get-routes --api-id x73mzc0v06` で `POST /track` route が integration `c8yyf01` (target: `arn:aws:lambda:...:function:p003-tracker`) を指しているが、`aws lambda get-function p003-tracker` → ResourceNotFoundException で関数自体が存在しない。frontend `app.js` `/track` 呼び出し 0 件・repo 全体に `p003-tracker` 参照 0 件。dead config。**Cowork からは `delete-*` 系コマンド禁止のため Code セッション dispatch 必要**。手順: ①`aws apigatewayv2 delete-route --api-id x73mzc0v06 --route-id <ID>` ②`aws apigatewayv2 delete-integration --api-id x73mzc0v06 --integration-id c8yyf01` ③IaC があれば該当 route を削除 PR。完了条件: `get-routes` の Items から POST /track が消えること。Verified-Effect: 1h 後の API Gateway 5xx 率に若干の減少 (要観察)。**Phase-Impact: 1 運用安定化** | AWS API Gateway 直接 + 関連 IaC | 2026-05-02 |
