...
| T2026-0502-J | 🟢 低 (観察済 / 設計通り・close 候補) | **p003-processor invocation 過多の真因特定 → バグでなく設計通りと判明** — 2026-05-02 10:32 JST Cowork 観察: CloudWatch metrics で processor が想定 (cron 1日2回) を超え毎時 1-3 回起動。仮説 (a) fetcher invoke / (b) Function URL / (c) S3 trigger / (d) Step Functions を列挙。**観察証拠**: IAM `flotopic-least-privilege` policy 内の `Sid: InvokeProcessor` (Allow `lambda:InvokeFunction` Resource `p003-processor`) を発見、加えて `lambda/fetcher/handler.py:1504` で `_lambda.invoke(FunctionName='p003-processor', InvocationType='Event', Payload={topic_ids,source:fetcher_trigger,maxApiCalls:10})` を確認。**真因確定: 仮説 (a) — fetcher が新規ペンディングトピック発生時に processor を即時非同期トリガーする設計** (コメント明示「処理レイテンシを 6h → 30min に短縮」)。バグではない・課金影響軽微 (maxApiCalls=10 制限あり)。Phase-Impact: なし | (観察済 / コード変更不要) | 2026-05-02 |
