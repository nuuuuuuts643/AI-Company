...
| T2026-0429-E | 🔴 高 | AI品質 | **マージ閾値調整** — T2026-0429-C で `error_merge=84.6%` (parent と child の title 類似度 < 0.2 で 11/13 件) 検出。事象: 別事件・別主役・別地域なのに parentTopicId が立つ。原因仮説: ① article→topic マッチング側 (proc_storage.py) の閾値が緩い ② should_branch ④分岐 (Jaccard 0.35 継続) の閾値判定では救えない事象が上流で混入。対応: (a) `verify_branching_quality.py --verbose` で誤マージ事例を ROOT 別に集約 (b) proc_storage.py の topic マッチ閾値見直し or should_branch に「主役不在 + 低 Jaccard 強制分岐」追加 (c) 修正後に再計測して 15% 以下 | lambda/processor/proc_ai.py, lambda/processor/proc_storage.py, scripts/verify_branching_quality.py | 2026-04-29 |
...
| T2026-0429-I | 🔴 高 | 体験 | **keyPoint 100字未満トピックのランキング降格** — 要約なし（keyPoint が空 or 100字未満）のトピックが上位に浮上している（ナオヤ報告: c0ccd7d87d35f1f1, 369af39fcf0c42b4 等）。原因: ランキングスコアが keyPoint 充填を考慮していない。対応: app.js の sort ロジックに keyPoint 充填ペナルティを追加（keyPoint 空 or 50字未満 → score × 0.3 降格）。T2026-0429-H（Lambda修正）完了後に合わせて検証。 | frontend/app.js | 2026-04-29 |
