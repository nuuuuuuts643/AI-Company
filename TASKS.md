...
| T2026-0430-C | 🔴 高 | 観測 | **freshness SLI に「<24h トピック比率」閾値アラート追加** — fetcher バグが 3 日気づかれなかった根本は age 分布 SLI がなかったため。`<24h 比率 < 10%` で Slack 警告を freshness-check.yml に追加。コスト増なし（GitHub Actions + curl のみ）。 | `.github/workflows/freshness-check.yml` | 2026-04-30 |
