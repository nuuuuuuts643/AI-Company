...
| T035 | 高 | **位置情報ダイアログをログインユーザー限定に変更**。現在 index.html ロード直後に天気ウィジェットが `navigator.geolocation.getCurrentPosition()` を自動呼び出しし、未ログインユーザーにもブラウザの位置情報許可ダイアログが表示される。未ログイン時はウィジェットを非表示にし、ログイン後のみ表示する。app.js の loadWeather() 呼び出しを `firebase.auth().currentUser` チェック後に変更するだけ | frontend/app.js | 2026-04-26 |
