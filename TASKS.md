...
| T2026-0428-PRED | 🟠 高 | AI品質 | **AI 予想 (outlook) の自動当否判定ロジック実装** — T2026-0428-E で `predictionMadeAt` / `predictionResult` フィールドを追加済 (現状 `pending` で固定書込)。次に: ① fetcher か 別 Lambda で「`predictionMadeAt` 以降に追加された記事」と outlook を Claude Haiku に渡し matched/partial/missed/pending を判定、② 結果を DynamoDB に書き戻し、③ frontend は既存バッジ (.ai-prediction-badge) で自動表示。判定 prompt は「outlook 文と新記事タイトル群を渡し、検証可能性を分類」する Tool Use schema 化。**注意**: 判定は記事 5 件以上溜まってから走らせる (1 件で判断するとノイズ)。 | `lambda/processor/proc_ai.py` (judge_prediction 追加), `lambda/processor/proc_storage.py` (update_prediction_result 追加) | 2026-04-28 |
| T2026-0428-GRAPH | 🟡 中 | UX | **グラフのスパイクポイントに「きっかけ記事」アノテーションを追加** — 「この記事がきっかけで一気に関心が上がった」が可視化されると体験価値が大きく上がる。実装案: ① `storyTimeline` の各 SNAP の `articleCount` 差分を計算してスパイク (前 SNAP 比 +N 以上 or +X% 以上) を検出、② スパイク SNAP の articles[] のうち媒体多様性が一番高い (= 一斉に取り上げられた起点) 1 記事を選ぶ、③ Chart.js の annotation plugin か独自オーバーレイで該当 datapoint に「📌 きっかけ記事タイトル」を表示。判定は frontend (detail.js) で完結可能。 | `frontend/detail.js`, `frontend/topic.html` | 2026-04-28 |
...
| T2026-0428-AC | 🟡 中 | **[仕組み] system-status.md 数値の自動更新化** — 「topics/{tid}.html 500/500件生成済み」のような **手書き数値が実態と乖離** する問題 (T2026-0428-AB の根本原因)。governance worker か独立 cron で実測値を取り、`docs/system-status.md` の該当行を sed で書き換える、もしくは別ファイル `docs/system-status-live.md` を自動生成する。**手書きで嘘が書ける構造そのものを排除する** のが目的。Claude が「書いたら機能している」と誤認するバイアスへの物理ゲート。 | `scripts/_governance_check.py` or `scripts/update_system_status_live.py` (新規) / `docs/system-status.md` 構造分離 | 2026-04-28 |
| T2026-0428-AD | 🟢 低 | **[仕組み] 新機能 PR テンプレに「対応 SLI 追記」必須化** — T2026-0428-AB の Why4 (新機能ローンチ時に SLI を追加する習慣が無い) への直接対策。`.github/PULL_REQUEST_TEMPLATE.md` に「□ 対応 SLI を `docs/sli-slo.md` に追記したか (該当なしの場合は理由)」チェック欄を追加。CI で PR 本文を grep して未記入なら警告 (block ではなく warning から開始)。新機能ごとに観測を物理紐付けする仕組み。 | `.github/PULL_REQUEST_TEMPLATE.md`, `.github/workflows/ci.yml` | 2026-04-28 |
