...
| T181 | 中 | **comments/favorites Lambda — topicId形式バリデーション追加（T174と同パターン）** — 根本原因: `POST /comments/{topicId}` がtopicId形式を未検証。①任意文字列topicIdへのコメントがai-company-commentsに書き込まれる ②`increment_topic_comment_count()`がConditionExpressionなしでupdate_itemを呼ぶためp003-topicsに幽霊METAレコードが生成される ③`topic.html?id=任意文字列`を共有すると加工コメントが見えるコンテンツインジェクション攻撃が可能。修正方法: comments handlerのGET/POSTパス先頭に`if not re.match(r'^[0-9a-f]{16}$', topic_id): return resp(400, {'error': 'invalid topicId'})`を追加。`increment_topic_comment_count()`に`ConditionExpression='attribute_exists(topicId)'`を追加。favorites Lambdaの`update_topic_fav_count()`インクリメントパスにも同条件を追加。 | `lambda/comments/handler.py`, `lambda/favorites/handler.py` | 2026-04-27 |
