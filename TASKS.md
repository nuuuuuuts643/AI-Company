...
| T2026-0502-Q | 🔴 最優先 | **Lambda デプロイ workflow が連続失敗 → 直近修正 PR が本番未反映** — 2026-05-02 09:50 JST 網羅調査 (Cowork) で発覚。最後成功 deploy `#423` 09:04 JST (sha=68c7c85e)、以降 `#369`/`#370` (09:20/09:25 JST) が `fetcher Lambda をデプロイ` step で連続失敗 → 後続全関数 skip。**未本番反映**: PR #118 (T2026-0501-M 重複検出マージ・実機で「[ITmedia M」x4・「【ドジャース】大谷翔」x3 で再現確認)、PR #125 (T2026-0502-B lifecycle SK fix)。**根本原因確定 (2026-05-02 10:06 JST Eng Claude 手動 trigger 再現)**: `.github/workflows/deploy-lambdas.yml` 46行目で `cd projects/P003-news-timeline/lambda/fetcher` してから 61行目で相対パス `python3 scripts/ci_lambda_merge_env.py` を呼んでいる。実体は repo root 直下の `scripts/ci_lambda_merge_env.py` なので、cd 後の CWD には存在せず Python が `[Errno 2] No such file or directory` で exit 2 → step 必ず失敗する**構造的バグ**。secret 値の有無とは無関係。直近5回 (#25238986862 / #25238862589 / #25238280057 / #25224080486 / #25217961335) すべて同じ箇所で fail。**T2026-0502-Q 単独では fix せず → 修正は T2026-0502-H で実装、修正 PR merge 後に本タスクの完了条件 (deploy run green + Lambda env 反映 + `[ai_merge_judge]` logs 出現) を再検証する。前置仮説 (ANTHROPIC_API_KEY 欠落) は誤りでした。**Phase-Impact: 1 Dispatch運用安定化** / **Eval-Due: 2026-05-03** / **依存**: T2026-0502-H 先 | (確認のみ・H で復旧) | 2026-05-02 |
...
| T2026-0502-H | 🔴 最優先 | **deploy-lambdas.yml の `ci_lambda_merge_env.py` パス解決バグ修正** — 2026-05-02 10:06 JST Eng Claude が手動 trigger 再現で確定。`.github/workflows/deploy-lambdas.yml` 46→61 行目で `cd projects/P003-news-timeline/lambda/fetcher` 後に相対パス `python3 scripts/ci_lambda_merge_env.py` を呼んでいるが、script の実体は repo root 直下のため `[Errno 2] No such file or directory` で必ず exit 2 → step failure → 後続全関数 deploy が skip される。secret 値とは無関係の構造的バグ。直近5回 (#25239903860 / #25238986862 / #25238862589 / #25238280057 / #25224080486) が全て同じ箇所で fail。**修正案** (Eng 提案 + Cowork 案): (a) 61 行目を `python3 ${GITHUB_WORKSPACE}/scripts/ci_lambda_merge_env.py` に変更 — 最小差分・GH Actions 標準変数で root 解決 / (b) `cd` の前で MERGED_ENV を計算し env 経由で渡す — workflow 内整合性が高い / (c) 61 行目を `python3 ../../../../scripts/ci_lambda_merge_env.py` — 階層 hardcode で fragile・非推奨。**Cowork 推奨は (a)**。**完了条件**: deploy run が green (fetcher step success), `aws lambda get-function-configuration --query 'Environment.Variables | keys(@)'` で `p003-fetcher` env に `ANTHROPIC_API_KEY` キーが含まれる, CloudWatch logs `/aws/lambda/p003-fetcher` に直近 1h 以内 `[ai_merge_judge]` または `[borderline]` の語が出現。Verified: deploy run URL + 上記 keys() 確認時刻 + logs grep ヒット件数。**Phase-Impact: 1 Dispatch運用安定化** / **Eval-Due: 2026-05-02 (本日中)** / **ブロッカー解除先**: T2026-0502-Q (deploy 復旧確認), T2026-0502-R (Haiku 復活), T2026-0502-SEC1 (rotation 後の Lambda env 反映) | `.github/workflows/deploy-lambdas.yml` | 2026-05-02 |
