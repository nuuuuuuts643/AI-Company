...
| T2026-0502-SEC3 | 🟡 中 (rotate 完了・history rewrite のみ残存) | **git history 内 commit 済み live secrets — token rotate は ✅ 完了 (PO 報告 2026-05-02 PM)・git history rewrite のみ残存** — ~~①GitHub PAT (prefix `ghp_LyAq...`) ②別 GitHub PAT (prefix `ghp_JPS5...`) ③Slack Bot Token (prefix `***REDACTED-SEC3***...`) ④Slack Webhook (path `T08UJJVCFJ4/B0AUJ9K64KE/...`)~~ → 全て **Revoked + 新規発行済み** (PO 報告 2026-05-02)。**残存タスク**: git history rewrite (`git filter-repo --replace-text` または BFG) で旧 token を history からも消去 + force-push。**重要度低下理由**: 旧 token が API 401 を返す = 攻撃者が history から拾っても利用不可。ただし「過去に流出した事実」記録としては残るため、git filter-repo で消すのが望ましい。完了条件: `git log --all -p \| grep -E "ghp_LyAq\|ghp_JPS5\|***REDACTED-SEC3***23616-10995291739264\|B0AUJ9K64KE"` が 0 件。**Phase-Impact: セキュリティ (defense in depth)** / **Eval-Due: 2026-05-16 (緊急性下がった・余裕あるとき)** / **手順例**: `pip install git-filter-repo` → `git filter-repo --replace-text <(echo -e 'ghp_LyAq...==>REDACTED\nghp_JPS5...==>REDACTED\n***REDACTED-SEC3***...==>REDACTED')` → `git push --force-with-lease origin main` → 全コラボの `git fetch + reset --hard origin/main` 周知 | `git filter-repo` (history rewrite) | 2026-05-02 |
| T2026-0502-SEC4 | 🔴 緊急 (Notion のみ残存・PO アクション待ち) | **Notion integration token のローテーション (T2026-0502-SEC-AUDIT)** — Anthropic は SEC1 と同 key で ✅ rotate 完了済。**Notion はまだ未対応 (PO 確認 2026-05-02 PM)**。`fetch_notion.py:12` に Notion integration token (prefix `ntn_3865...`) が平文滞留。Cowork チャット表示・スクリーン共有経由で第三者に見える可能性あり。本ファイルは PR #200 で env 変数読み込み版に書換済 (current HEAD には full token 無し) のため新 token 受け入れ準備完了。手順: ①Notion Settings → Integrations で旧 `ntn_3865...` を Revoke ②新 token を発行 ③`export NOTION_TOKEN=ntn_xxx` で動作確認 (任意・fetch_notion.py が gitignore 済 = 自動デプロイなし・手動利用のみ)。完了条件: 旧 Notion token で API 401。**Phase-Impact: セキュリティ最優先** / **Eval-Due: 2026-05-02 (即日)** | Notion Settings | 2026-05-02 |
