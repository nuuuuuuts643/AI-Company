...
|---|---|---|---|---|
| T166 | 高 | **storymap一覧カードの「最新イベント」が常に空白** — 根本原因: `renderStorymapList`が`storyTimeline`（topics.jsonの`_INTERNAL`除外フィールド）から`latestEvent`を取得しようとしているが、topics.jsonには含まれないため常に空。修正方法: フィルター条件の`storyTimeline`分岐を削除（`storyPhase`のみ）・`latestEvent`行を`generatedSummary`先頭55文字スニペットに差し替え | `frontend/storymap.html` | 2026-04-27 |
