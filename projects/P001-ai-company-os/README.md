...
> AI が自走でプロダクトを作って運営して収益を出す。ナオヤは出資者・方針決定者。
...
ナオヤの一言指示と方針 → AI 群が要件定義・実装・デプロイ・監視・改善・収益化までを完遂。
ナオヤの作業時間は「方針決定」「金が必要な意思決定」「最終ジャッジ」のみに収束させる。
...
| 部署 | 主担当 | 起動経路 | 役割 |
|---|---|---|---|
| **CEO（出資者）** | ナオヤ | 直接 | 方針決定・金が動く意思決定（広告予算/契約/サブスク等）・最終ジャッジ |
| **Code（Claude Code/CLI）** | Opus 1M | ローカル CLI | コード実装・テスト・デプロイ確認・PR レビュー |
| **Cowork（Claude スマホ・デスクトップ）** | Sonnet/Opus | クラウド | ナオヤとの会話・分析・計画立案・ドキュメント更新・git push |
...
> 「停止中」マークの部署は schedule をコメントアウトしているが、script は残している。コスト都合で止めているだけで、ナオヤの判断で再開可能。
...
ナオヤがやらないこと: 細かい実装・デバッグ・テスト・デプロイ確認・ドキュメント更新。これらは AI が完了報告まで自走する。
...
- ナオヤの介入頻度を「週 1 回 30 分の方針確認」まで圧縮。
