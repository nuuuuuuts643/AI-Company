...
def _generate_story_minimal(articles: list) -> dict | None:
    ...
    prompt = (
        ...
        + '【出力フォーマット（JSON以外出力禁止）】\n'
        '{\n'
        '  "aiSummary": "**思考フレーム『背景→課題→目的→手段→結果→今後』で内部整理してから、初見ユーザーが文脈ごと理解できる1段落にまとめる**。主語(誰/何が)+動詞+目的語(何を)+目的(何のために)を必ず明示。動詞には{何}を付け『〜をめぐる協議』『〜に向けた会談』形式。途中参加でも『誰が何の問題を解決するために何をしているか』が冒頭1文で分かるように書く。",\n'
