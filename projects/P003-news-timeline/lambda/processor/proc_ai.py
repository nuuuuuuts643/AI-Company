...
def _generate_story_standard(articles: list, cnt: int) -> dict | None:
    ...
    prompt = (
        ...
        '  "perspectives": "メディア間の論点フォーカスを必ず1〜2文で記述する (nullや空文字は禁止)。**ズレがあれば**: 「{社A}は『{論点X}』を強調する一方、{社B}は『{別論点Y}』を中心に扱う」形式。**ズレが見えなくても**: 「{社A}・{社B}・{社C}はいずれも『{経済影響/政治影響/法的論点/国際反応}』を中心に報じる」形式で論点フォーカスを示す。Flotopicの価値は『各社が何を主軸に置いているか』の可視化なので、null は出力しない。",\n'
        '  "outlook": "この先どうなるか。1文で必ず記述。null/空禁止",\n'
        '  "timeline": [\n'
        ...
        'perspectives: **必ず 1〜2 文で記述。null・空文字禁止**。ズレが見えるなら 「{社A}は『{X}』、{社B}は『{Y}』」形式でズレを明示。ズレが見えないなら 「{各社}はいずれも『{論点}』を中心に報じる」形式で論点フォーカスを共有する。判断基準は「読者が『どの記事を読んでも同じか/違うか』を1秒で判断できるか」。\n'
