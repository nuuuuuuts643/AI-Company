...
_STORY_PROMPT_RULES = (
    ...
    '    ② なぜ重要か（読者への影響を明示）\n'
    '    ③ 今後どうなりそうか（「背景には〜」「なぜなら〜」で構造を示す）\n'
    # T2026-0501-K: エンタメ/テク充填率引き上げのため genre 多様な ◎例 2 件に差し替え (2026-05-01)
    '    ◎ 良い例 (エンタメ): 「人気声優・降幡愛への暴行事実が報じられ、SNSで100万件超の投稿が集まった。本人の活動に影響はないと事務所が即日声明を出したが、「なぜ被害が長期間表面化しなかったか」という疑問が業界構造問題として注目を集めている。加害者とされる人物の業界での立場を問う声が拡大している」\n'
    '    × 悪い例: 「この件は多くの人に影響を与えたため注目が集まっている」（一般論・抽象的・何も起きていないように見える）\n'
    '\n'
    ...
    '    ◎ 良い例 (テクノロジー): 「NTTの光電融合AI基盤「AIOWN」が消費電力を従来比10分の1に削減したと実証された。これまでAIデータセンターの電力問題は「解決不可能な制約」とされていたが、光技術との融合で商用化の前提条件が変わりつつある。2027年実用化が実現すれば、AIインフラコストを構造的に下げる初の手段になると専門家は評価する」\n'
