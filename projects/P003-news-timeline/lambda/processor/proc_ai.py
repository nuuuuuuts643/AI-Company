...
def _generate_story_minimal(articles: list) -> dict | None:
    ...
    prompt = (
        ...
        + '【出力フォーマット（JSON以外出力禁止）】\n'
        '{\n'
        '  "aiSummary": "**思考フレーム『背景→課題→目的→手段→結果→今後』で内部整理してから、初見ユーザーが文脈ごと理解できる1段落にまとめる**。主語+動詞+目的語+目的(何のために)を必ず明示。動詞は『〜をめぐる協議』『〜に向けた会談』形式。途中参加でも『誰が何の問題を解決するために何をしているか』が冒頭1文で分かるように書く。**教科書的な分析ではなく読み物として刺さる切り口で**(『実は〜』『ところが〜』『〜にもかかわらず』等の物語的接続詞も活用)。30秒で腹落ちできる文体に。事実は正確に、表現は分かりやすく。",\n'
...
def _generate_story_standard(articles: list, cnt: int) -> dict | None:
    ...
    prompt = (
        ...
        '  "perspectives": "**手段・結果・今後の解釈にメディア間でズレがある場合のみ**文字列で暴く (背景・目的は事実なのでズレない)。比較フォーマット推奨: 「{社A}は『{何を強調}』として{結論Aの方向}と扱う一方、{社B}は『{別の側面}』として{結論Bの方向}と扱う。{社C}は{経済影響/法的論点}を中心に分析」。同じ手段を同じ結論で報じてるならnull。Flotopicの独自価値はこのズレの可視化。",\n'
        '  "outlook": "この先どうなるか。1文で",\n'
        '  "timeline": [\n'
        ...
        'perspectives: 背景・目的は事実なのでメディアでズレないが、**手段(何をしたか)・結果(何が起きたか)・今後(どうなるか) の解釈にメディアごとのズレがある場合**だけ暴く。比較フォーマット: 「{社A}は『{何を強調}』として{結論A方向}で扱う一方、{社B}は『{別側面}』として{結論B方向}で扱う」。同一解釈ならnull。Flotopicの独自価値はこのズレの可視化。\n'
