...
def _generate_story_standard(articles: list, cnt: int) -> dict | None:
    ...
    prompt = (
        ...
        '  "perspectives": "メディア間で**主語の選び方・論点の置き方・着地点(結論の方向性)が異なる**場合のみ文字列で記述（例: 「NHKは『政府が』を主語に事実を中立に報じる一方、産経は『国民の不安』を主語に規制強化の必要性で着地。日経は『市場が』の主語で経済影響を中心に分析」）。全メディアが同じ主語・同じ着地ならnull",\n'
        '  "outlook": "この先どうなるか。1文で",\n'
        '  "genres": ["最も適切なジャンル1つ、または2つ（上のリストから選ぶ）"]\n'
        '}\n\n'
        '【ルール】\n'
        'aiSummary: 改行・箇条書き禁止。1段落。メディア名不要。**冒頭で主語を明示する** (「政府が」「日銀が」「米欧が」「市場が」等)。主語不在の要約は禁止。\n'
        ...
        'phase: 以下のルールで厳格に判定（「発端」を選びすぎないこと。デフォルトは「拡散」）:\n'
        '  - 発端: 記事3件未満かつ最初の記事と最後の記事の時間差が24h以内のみ。それ以外で「発端」は選ばない\n'
        '  - 拡散: 記事3件以上、または時間差が24h超。記事数が増えていく途中。標準的な選択肢\n'
        '  - ピーク: タイムラインで「次々と」「同時に」「一斉に」の動きがあり、複数の主体が同時に動いている\n'
        '  - 現在地: 最新の動きが「対応した」「決定した」など落ち着き始めの兆候。新しい局面入り\n'
        '  - 収束: 結論・判決・決着などの最終的な動きで完結している\n'
        '  判定に迷ったら「拡散」を選ぶこと\n'
        'background: 1〜2文。「なぜ今か」を説明する文脈（過去の経緯・法制度・国際情勢等）。backgroundContextとは異なり、現時点でこの話題が浮上している直接的な理由を書く。\n'
        'perspectives: **メディアごとに主語・論点・着地点が異なる場合のみ**文字列で書く。比較フォーマット推奨: 「{社A}は『{主語A}』を主語に{論点A}で着地。{社B}は『{主語B}』を主語に{論点B}で着地」。同一主語・同一着地なら必ずnull。\n'
...
def _generate_story_full(articles: list, cnt: int) -> dict | None:
    ...
    prompt = (
        ...
        '}\n\n'
        '【各フィールドのルール】\n'
        'aiSummary: 改行・箇条書き・見出し禁止。1段落。メディア名不要。**冒頭で主語を明示する** (「政府が」「日銀が」「米欧が」「市場が」等)。主語不在の要約は禁止。\n'
        ...
        'phase: 以下のルールで厳格に判定（「発端」を選びすぎないこと。デフォルトは「拡散」）:\n'
        '  - 発端（始まったばかり）: 記事3件未満かつ最初の記事と最後の記事の時間差が24h以内のみ。それ以外で「発端」は選ばない\n'
        '  - 拡散（広がっている）: 記事3件以上、または時間差が24h超。記事数が増えていく途中。標準的な選択肢\n'
        '  - ピーク（最も活発）: タイムラインで「次々と」「同時に」「一斉に」「相次いで」のような展開があり、複数主体が同時並行で動いている\n'
        '  - 現在地（落ち着いてきた）: 最新の動きが「対応を発表」「方針を決定」「結論」など落ち着き始めの兆候。新しい局面入り\n'
        '  - 収束（話題が終息した）: 判決・決着・最終決定などで完結し、新しい記事が出にくい状態\n'
        '  判定に迷ったら「拡散」を選ぶこと\n'
        'background: 1〜2文。「なぜ今か」を説明する文脈（過去の経緯・法制度・国際情勢等）。backgroundContextとは異なり、現時点でこの話題が浮上している直接的な理由を書く。\n'
        'perspectives: **メディアごとに主語・論点・着地点が異なる場合のみ**文字列で書く。比較フォーマット推奨: 「{社A}は『{主語A}』を主語に{論点A}で着地。{社B}は『{主語B}』を主語に{論点B}で着地」。同一主語・同一着地なら必ずnull。\n'
