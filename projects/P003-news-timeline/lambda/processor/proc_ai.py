...
def _generate_story_minimal(articles: list) -> dict | None:
    ...
    prompt = (
        ...
        + '【出力フォーマット（JSON以外出力禁止）】\n'
        '{\n'
        '  "aiSummary": "**主語(誰/何が)+動詞+目的語(何を)を必ず明示**して、誰が何を巡って何をして何が起きたかを1段落で。『協議』『会談』『合意』『停戦』『批判』等の動詞は必ず{何}を付けて『〜をめぐる協議』『〜に向けた会談』形式に。読者が冒頭1文で『何を巡る話か』が分かるように書く。",\n'
...
def _generate_story_full(articles: list, cnt: int) -> dict | None:
    ...
    prompt = (
        ...
        + '【出力フォーマット（JSON以外出力禁止）】\n'
        '{\n'
        '  "aiSummary": "①何が起きたか。**主語(誰/何が)+動詞+目的語(何を)を必ず明示**して、誰が何を巡って何をして何が起きたかを1段落で。「協議」「会談」「合意」「停戦」「批判」等の動詞は必ず{何}を付けて『〜をめぐる協議』『〜に向けた会談』形式に。読者が冒頭1文で『何を巡る話か』が分かるように書く。",\n'
