...
function _showFavLoginToast() {
  if (_favLoginToastShown) return;
  _favLoginToastShown = true;
  let el = document.getElementById('flotopic-fav-login-toast');
  if (!el) {
    el = document.createElement('div');
    el.id = 'flotopic-fav-login-toast';
...
async function toggleFavorite(topicId, heartBtn) {
  if (!currentUser) {
    // ログイン未済でもローカル保存し、初回のみログイン誘導トーストを表示
