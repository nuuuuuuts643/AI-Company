...
Token 取得の推奨経路 (T2026-0502-BJ 恒久対処):
  Mac で 1 回だけ:
    gh auth token > ~/ai-company/.cowork-token && chmod 600 ~/ai-company/.cowork-token
  これで Cowork (Linux sandbox) からも FUSE 経由で .cowork-token を読める。
  .gitignore で除外済 (commit リスクなし)。
  この経路が動けば .git/config URL から PAT を剥がしても Cowork は動き続ける。
"""
...
def get_token_and_repo():
    ...
    優先順位 (新→旧):
      0. ワークスペース直下 .cowork-token (推奨経路・T2026-0502-BJ で導入)
    ...
    背景:
      2026-05-02 T2026-0502-SEC2 で .git/config URL から PAT を除去した結果、
      cowork_commit.py が 401 Unauthorized になり Cowork から PR が作れなくなった事故。
      平文 token を保管しなくても認証できるよう多経路化したが、Linux sandbox
      (Cowork) からは Mac の Keychain も gh CLI auth も .netrc も見えないため、
      実質的に経路 1 (.git/config URL 直書き) しか動かない状態だった (T2026-0502-BJ)。

      恒久対処として、ワークスペース直下 (FUSE で sandbox にもマウントされる)
      に `.cowork-token` という単一目的ファイルを置く経路を追加する:
        - Mac で 1 回:  gh auth token > ~/ai-company/.cowork-token && chmod 600
        - .gitignore に登録済 (commit リスクなし)
        - Cowork sandbox からも file system mount 経由で読める
        - Mac 側 .git/config からは token を剥がせる (SEC2 警告解消)
    """
    repo = None
    token = None
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    # 経路 0: workspace .cowork-token (推奨・T2026-0502-BJ で導入)
    try:
        token_file = os.path.join(repo_root, '.cowork-token')
        if os.path.isfile(token_file):
            # mode 0600 推奨だが、FUSE でも secret として扱う前提でゆるく許容
            with open(token_file, 'r') as f:
                file_token = f.read().strip()
            if file_token:
                token = file_token  # repo はまだ未確定のため一旦保持
    except Exception:
        pass
    ...
    try:
        ...
        if m:
            # 経路 0 で token が取れていたらそちらを優先・repo は URL から
            return (token or m.group(1)), m.group(2)
    ...
    # 経路 2: 環境変数 (Mac launchctl setenv 経由含む)
    ...
    # 経路 5: macOS Keychain (Mac 環境のみ・Linux sandbox では到達不能)
    try:
    ...
    if not token:
        sys.stderr.write(
            "ERROR: GitHub token not found in any of the 6 sources:\n"
            "  0. <repo_root>/.cowork-token (推奨)  1. .git/config URL\n"
            "  2. env GITHUB_TOKEN/GH_TOKEN  3. ~/.config/gh/hosts.yml\n"
            "  4. ~/.netrc  5. macOS Keychain\n"
            "  恒久対処 (T2026-0502-BJ): Mac で\n"
            "    gh auth token > ~/ai-company/.cowork-token && chmod 600 ~/ai-company/.cowork-token\n"
            "  詳細: docs/lessons-learned.md「T2026-0502-SEC2-RECURRENCE」「T2026-0502-BJ」\n"
        )
    return token, repo
